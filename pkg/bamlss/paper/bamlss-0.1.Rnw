...
\Abstract{
  ...
  for the estimation of a variety of models and proposes an unified modeling architecture
  ...
  existing software, is relatively straightforward. We further emphasize the usefulness on an
...
asymptotic properties fail. This is particular the case in more complex models, e.g., with response
...
problem. More precisely, we highlight the similarities between estimation algorithms such that
model building is practically based on a ``Lego'' toolbox with repeatedly used ``bricks''.
Hence, the approach supports more than the GAMLSS statistical model class and is sometimes
...
for the implementation are presented. Finally, Section~\ref{sec:softex} introduces the corresponding
\proglang{R} implementation on a very large model for daily precipitation observations, as well as
on the computational challenging Cox-model for continuous time.
...
A common choice of prior for the variance parameter is a weakly informative inverse Gamma hyper-prior
\begin{equation} \label{eqn:ig}
p(\tau_{jk}^2|a_{jk},b_{jk}) = \frac{b_{jk}^{a_{jk}}}{\Gamma(a_{jk})} (\tau_{jk}^2)^{-(a_{jk} + 1)} \exp(-b_{jk} / \tau_{jk}^2).
\end{equation}
with hyper-parameters $a_{jk} = b_{jk} = 0.001$ as a standard option. Small values for $a_{jk}$ and
$b_{jk}$ correspond to an approximate flat prior for $\log(\tau_{jk}^2)$. Setting $b_{jk}=0$ and
$a_{jk}=-1$ or $a_{jk}=-0.5$ yields flat priors for $\tau_{jk}^2$ and $\tau_{jk}$, respectively.
However, the default option of the inverse Gamma prior for variance parameters needs to be
reconsidered in some cases, e.g., it might be problematic if $\tau_{jk}$ is close to zero, since the
results a very sensitive on the choice of $a_{jk}$ and $b_{jk}$. Therefore,
...
\end{equation}
For the hyper-parameter $A_{jk} \rightarrow \infty$ the prior is uniform, hence large values
(e.g., $A_{jk} = 25$) result in weakly informative priors. A desirable property of the half-Cauchy
is that for $\tau_{jk} = 0$ the density is a nonzero constant, whereas the density of the inverse
Gamma for $\tau_{jk} \rightarrow 0$ vanishes (see also \citealp{bamlss:Polson+Scott:2012}). Note
that in our framework the priors for variances $\tau_{jk}^2$ rather than $\tau_{jk}$ are usually
...
parameter that is directly related to the functions $f_{jk}$ (and thus much better interpretable and
accessible for the user) is given in~\citet{bamlss:Klein+Kneib:2016} for several different
hyper-priors for $\tau_{jk}^2$, such as resulting priors from half-Cauchy, half-normal or uniform
priors for $\tau_{jk}$ or resulting penalized complexity
priors~\citep{bamlss:Simpson:Rue:Martins:Riebler:Sorbye:2015}.

Note that using a basis function approach it is possible to construct effects of more than
one covariate by applying tensor products of marginal design matrices
\citep{bamlss:Eilers+Marx:2003, bamlss:Wood:2006}.
The resulting penalty matrix is also constructed by tensor products, e.g., for a two-dimensional
effect the corresponding penalty takes the following form
$$
\boldsymbol{\beta}_{jk}^\top \left( \frac{1}{\tau_{1jk}^2} \mathbf{I}_{2jk} \otimes \mathbf{K}_{1jk} +
  \frac{1}{\tau_{2jk}^2} \mathbf{K}_{2jk} \otimes \mathbf{I}_{1jk} \right) \boldsymbol{\beta}_{jk} =
\boldsymbol{\beta}_{jk}^\top \mathbf{K}_{jk}(\boldsymbol{\tau}_{jk}^2)\boldsymbol{\beta}_{jk},
$$
where $\mathbf{K}_{1jk}$ and $\mathbf{K}_{2jk}$ are the penalty matrices of the marginal effects
and $\boldsymbol{\tau}_{jk}^2 = (\tau_{1jk}^2, \tau_{2jk}^2)^\top$ are variances that control the
smoothness of the effect in each of the two dimensions.
...
structured additive predictor of nested covariates, e.g., including a discrete regional spatial
effect modeled by a Markov random field. This way, potential costly operations during estimation
can be avoided since the number of observation in
...
$\tilde{\boldsymbol{\beta}}_{jk}$ are Gaussian regardless of the response distribution, therefore,
an alternative parametrization can be applied that leads to highly efficient MCMC sampling schemes,
...
a more detailed discussion on variance/hyperparameter estimation. In Section~\ref{sec:varselpostmode},
we briefly discuss a stepwise approach for smoothing parameter selection. In the following we describe
...
derived, e.g., using multi-dimensional tensor product splines (see Section~\ref{sec:smootheffects}),
another strategy is to use slice sampling, since the number of smoothing variances is
usually not very large the computational burden does most of the times not exceed possible benefits.
...
$\hat{r}_i = \Phi^{-1}(\mathbf{\mathcal{D}}(y_i | \hat{\boldsymbol{\theta}}_{i}))$ with the inverse 
cumulative distribution function (cdf) of a standard normal distribution $\Phi^{-1}$ and
$\mathbf{\mathcal{D}}(\cdot | \hat{\boldsymbol{\theta}}_{i})$ denoting cdf with estimated
...
terms of quantile-quantile-plots. Strong deviations from the bisecting line are then a sign for an
...
$u_i = \mathbf{\mathcal{D}}(y_i | \hat{\boldsymbol{\theta}}_i)$ without applying the inverse
...
Information criteria like the Akaike information criterion (AIC), or the corrected AIC, as well as
the Bayesian information criterion (BIC), amongst others, can be used for selecting variables and
smoothing variances $\boldsymbol{\tau}^2_{jk}$. These criteria try to penalize overly complex models,
...
$$
\mathrm{edf}_{jk}(\boldsymbol{\tau}^2_{jk}) = \mathrm{trace}\left\{
  \mathbf{F}_{kk}(\boldsymbol{\beta}_{jk})(
  \mathbf{F}_{kk}(\boldsymbol{\beta}_{jk}) + \mathbf{G}_{jk}(\boldsymbol{\tau}^2_{jk}))^{-1} \right\},
$$
with $\mathbf{F}_{kk}(\boldsymbol{\beta}_{jk}) =
\partial^2 \ell(\boldsymbol{\beta}; \mathbf{y}, \mathbf{X})/
\partial \boldsymbol{\beta}_{jk} \partial
\boldsymbol{\beta}_{jk}^\top$ and matrix $\mathbf{G}_{jk}(\boldsymbol{\tau}^2_{jk})$ is the second
derivate of the shrinkage prior w.r.t.\ $\boldsymbol{\beta}_{jk}$, e.g., using prior
(\ref{eqn:shrinkprior}) the matrix is $\mathbf{G}_{jk}(\tau^2_{jk}) = \tau_{jk}^{-2}\mathbf{K}_{jk}$. The
total degrees of freedom used to estimate the model are then given by
$\sum_{k}\sum_{j} \mathrm{edf}_{jk}(\boldsymbol{\tau}^2_{jk})$.
...
distributional regression models. In each cycle of the backfitting algorithm (\ref{eqn:blocknewton})
or (\ref{eqn:blockbackfit}), the smoothing variances for each model component are optimized holding all
other variances fixed. To further speed up convergence, the search space for smoothing variances can
be based on a small grid of candidate values. This will not lead to a global optimum, however, the
resulting parameters serve as good starting values for full Bayesian inference with MCMC. This is
especially useful for multi-dimensional effects with multiple smoothing variances since convergence
of the MCMC chains will usually take much longer with improper starting values.

Another recently proposed, however computationally intensive, approach for the
types of models presented in this paper is gradient boosting
\citep{bamlss:Messner+Mayr+Zeileis:2016, bamlss:Hofner+Mayr+Schmid:2014}.
The boosting algorithm slightly differs from classical boosting in so far, that in each boosting iteration for each parameter
$\theta_k$, first the model components (base learners) that best fit to the respective negative
gradients are chosen, afterwards, the model component that contributes to the log-likelihood the
most is selected for updating. This way, only one stopping criterion needs to be used, which
makes finding the optimum stopping iteration, e.g., by cross-validation less costly.
...
computational effort. If $\boldsymbol{\vartheta}^{(1)}, \ldots, \boldsymbol{\vartheta}^{(T)}$ is a
MCMC sample from the posterior for the complete parameter vector $\boldsymbol{\vartheta}$, the
DIC is given by
$\overline{D(\boldsymbol{\vartheta})} + \mathrm{pd} = 2 \overline{D(\boldsymbol{\vartheta})} -
D(\overline{\boldsymbol{\vartheta}}) =
\tfrac{2}{T}\sum D(\boldsymbol{\vartheta}^{(t)}) - D(\tfrac{1}{T}\sum\boldsymbol{\vartheta}^{(t)})$
where $D(\boldsymbol{\vartheta}) = -2\log(p(\boldsymbol{\vartheta}; \mathbf{y}, \mathbf{X}))$
is the model deviance and $\mathrm{pd} = \overline{D(\boldsymbol{\vartheta})} -
D(\overline{\boldsymbol{\vartheta}})$ is an effective parameter count.
...
corrected AIC and BIC \citep{bamlss:Belitz+Lang:2008}, see also Section~\ref{sec:varselpostmode}.
...
\fct{beta\_bamlss} & Beta distribution \\
\fct{binomial\_bamlss} & Binomial distribution \\
\fct{cnorm\_bamlss} & Censored normal distribution \\
\fct{cox\_bamlss} & Continuous time Cox-model \\
\fct{gaussian\_bamlss} & Gaussian distribution \\
\fct{gamma\_bamlss} & Gamma distribution \\
\fct{multinomial\_bamlss} & Multinomial distribution \\
\fct{mvn\_bamlss} & Multivariate normal distribution \\
\fct{poisson\_bamlss} & Poisson distribution \\ \hline
\end{tabular}
