...
\begin{enumerate}
\item Minor comments:\\
  page 2, line 13: challgenges\\
  page 11, line -2 and -3: check parentheses and punctuation\\
  page 15, third bullet point: check the reference style.\\
  page 15, line -1: check punctuation\\
  page 16, line 9: a.o.? and the empty space that follows?\\
  page 16, line 2 after the third equation: check punctuation and capital letter\\
  page 16, line 3 after the third equation: remove comma after "is"\\
  page 17, line 7: "is computed" is repeated twice\\
  page 18, line 11: staring values\\
  page 25, line 13: capital letter after colon\\
  page 25, last bullet point: the meaning of "...," is not clear\\
  page 28, line 1 of text: capital letter after colon\\
  page 29, line 16 of text: extra parenthesis after $\nu$\\
  page 29, line -11: capital letter after colon\\
  page 35, line 1: capital letter after colon

  \bigskip

  Needs to be fixed.

\item I run the code available in the \code{bamlss-R2.R} file. I noticed that: \\
...
is different from the code in the paper at page 7 (here \code{_} is used for the
names):
\begin{Schunk}
\begin{Sinput}
...
2) The following code reported at page 30 of the paper is not reported int
he R file:
\begin{Schunk}
\begin{Sinput}
R> set.seed(123)
...
More importantly, there is a problem starting from section 5.3 because the
code
\begin{Schunk}
\begin{Sinput}
resids <- c(
...
\end{Sinput}
\end{Schunk}
use two objects but in the paper only the code for computing
...
\end{Sinput}
\end{Schunk}
but I wasn't able to find the necessary objects for computing resids.

\medskip

Moreover, line 225 of the R code uses "b" in the function pathplot, while in
...
3) In Appendix C: the plots corresponding to the following code do not
appear in the paper:
\begin{Schunk}
\begin{Sinput}
R> par(mfrow = c(1, 3))
