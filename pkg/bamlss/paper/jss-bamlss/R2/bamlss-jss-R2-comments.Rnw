...
\item \emph{I have two main comments. The first is that it would be very useful to have a table
  ...
  \textbf{ich bin weiter nicht sicher, dass das seine Frage beantwortet, denn er meint wohl eher, dass man cox_mcmc ja nur bei Cox modell benutzen kann etc. also Modellklasse verbinden mit moeglichen samplern. man wuerde fuer die betas wohl auch nie den slice sampler verwenden wollen. dieser aspekt scheint mir weder von der tabelle noch der neuen funktion addressiert zu werden.}
...
\item \emph{pg 3, talking about gradient boosting "However, obtaining MCMC samples from the posterior
  ...
  \textbf{glaube weiterhin nicht, dass der referee oder unsere antwort korrekt sind. in welchen modellen bitte kann ich die posterior nicht hinschreiben. schlage daher vor: "We thank the referee for this comment indeed our sentence was wrong because there are Bayesian versions of lasso and ridge and for all these models, the posterior can be written down explicitely (even though not available in closed form but this is not a problem. in MCMC). The issue is that less R packages on the Bayesian side are existing. We hence rephrased the sentence to "However, many packages do not support Bayesian posterior estimation in such models.
  wenn das fuer euch ok ist, bitte auch im Text so aendern!}
...
\item \emph{pg 11 "According the histogram and the quantile-quantile plot of the resulting
  ...
  mortality rates in Austria. The QQ-plot now includes 95\% credible intervals. \textbf{it is not a qq plot but quantile residuals. that is not the same.}
...
\item \emph{pg 13 bottom, the paragraph starting with "Using a basis function approach \dots" needs
  ...
  penalize the complexity using a P-spline representation." \NK{bin weiterhin der meinung, dass es keine p-splines sein muessen.}
...
\item \emph{pg 25 "After 1000 iterations the term \code{s(q\_prof\_PC1).mu} has the highest
  ...
  worked over this example and use a different distribution. We now show only the two most
  important terms.
...
\item \emph{For example in example 3 the distribution used, the zero inflated negative binomial,
  ...
  We now test 3 different distributions (two coming from GAMLSS) for this example.
...
\item \emph{So I am left with wondering how long I have to spend to implement a new distribution
  ...
  from GAMLSS instantaneously! You pass the GAMLSS family as it is into the bamlss model call
...
\item \emph{page 13, l 4 I am assuming that the priors are trying to be uninformative. How
  ...
  variances in the prior distributions for \textbf{\mathbf{\beta}. da sollte dann auch was im text ergaenzt werden?!}
...
\item \emph{page 13, l 6 "Univariate responses of any type \dots". The references have a "Bayesian"
  ...
  Yes that is of course correct, \textbf{and we tried to balance better by adding this reference at appropriate places in the text.}
...
\item \emph{page 21 l 12 What happens for distributions for which the expectation of the
  ...
  in Klein et al 2015 (JASA). We added \textbf{details to Section~4.1 in the revised version}
...
\item \emph{page 24 l 6 "In order to capture the truncation of the data and its overdispersion we
  ...
  We now test the zero-truncated (zt) negative binomial against the zt Sichel and
  zt beta negative binomial. We therefore also adapted the set of explorative  variables.
  The latter shows the most promsing fit---check with
  worm plot---and is now used for the final model. 
...
\item \emph{page 27 l 9 It appears that something is wrong here \code{max(fit\$mu) [1] 2.231437e+14}.
  ...
  beta negative binomial and the predictions are stable.
