...
\item \emph{The main smoothers considered are similar to the ones in package \pkg{mgcv}. It was
  ...
  \citep{bamlss:Wood:2019} and \code{bs = "cc"} specifies a penalized cyclic
...
\item \emph{So I am left with wondering how long I have to spend to implement a new distribution
  ...
  We give an example how to implement a new distribution in Appendix B. The time it takes
  to implement a new distribution heavily depends on the complexity of the density function.
