...
\item \emph{pg 24 the model seems to take 5 + 27 = 32 minutes to compute. How does this compare with, 
  ...
  \fixme{Niki/Thorsten}
  Parts of the building blocks in the family (zero-truncated negative binomial) are written in
  C to speed up computation. However, it is correct that this single abolute number is very
  informative. Thus, we skip this information.
