...
rate. The models are estimated using $\eta_k = f_{k}(\texttt{week})$ for each parameter
...
Family & DIC \\ 
\hline
\fct{NO} 12289.34 51.39274
\fct{GA} 12257.82 51.22522
\fct{JSU} 12194.46 50.87003
\fct{BCPE} 12188.77 50.78029
\fct{BCCG} 12188.27 50.81109
\fct{BCT} 12186.60 50.80115
\hline
\end{tabular}
\caption{\label{tab:DIC} DIC comparison of the 6 distributional cyclical week models.}
\end{table}
Here, the four parameter \fct{BCPE} model has the lowest DIC.
...
and only save every 10th sample. After the estimation algorithms are finished, the estimated
