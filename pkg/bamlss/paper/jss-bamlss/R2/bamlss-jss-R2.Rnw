...
global climate \citep{era5b}. ERA5 provides a globally complete and consistent
...
small subset of parameters available \citep{era5b}. It encloses variables that
are well known to be good candidates for predicting or explaining convective
event that also lead to lightning discharges
\citep[e.g.,][]{bamlss:Simon+Mayr+Umlauf+Zeileis:2019}, in Table~\ref{tab:desc} a
...
distribution. \fixme{Discuss other extremely skewed count data distributions.}
...
distribution regression models. The package is primarily based on the typical \proglang{R}
``look \& feel'', which makes it easy to get started with the package. Furthermore, \pkg{bamlss}
is a modular ``Lego toolbox'', so that users who are interested in extensions can easily write
new families, optimizers or sampling functions.
...
In addition, it is planned to expand the family infrastructure to support, for example, families
of the \proglang{R} package \pkg{VGAM}.
