...
complex space-time interactions, estimation may be challenging. The \proglang{R} package \pkg{gamlss} also
supports flexible additive smooth terms, but cannot easily provide inference for probabilistic
predictions, if at all. In contrast, \pkg{mgcv} excels
