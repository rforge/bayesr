...
for this analysis is to model a reference mortality, which allows one to descibe
...
data at hand. Since there are a number of candidate distributions and the complexity of the data
set and the model in terms of the predictor is relatively low, we use all families for
continuous responses as implemented in the \pkg{gamlss.dist} package \citep{bamlss:gamlss.dist} and 
compare the model fits using the deviance information criterion (DIC, the complete
\proglang{R}-code is available in the supplements). Note that we do not
...
\end{figure}
The histogram of the number of fatalities together with the 10 best fitting
densities and corresponding log-likelihoods is shown in
Figure~\ref{fig:fathist}. One of the most flexible distributions of the
\pkg{gamlss.dist} package, the four parameter skew power exponential type 4
distribution (family \fct{SEP4}, \citealp{bamlss:DiCiccio:2004}), has the best
fit to the data (though only by a small difference). Therefore, we use the
\fct{SEP4} family function and estimate a Bayesian model that takes into
