...
package \citep{bamlss:gamlss.dist} and estimate intercept only models and compare the
log-likelihood and the resulting density functions. Note that we do not use discrete distributions
in this example because the number of fatalities is far away from zero in the thousands, i.e., a
model for count data will most probable not improve the overall fit.
\begin{figure}[!ht]
\centering
...
@
\caption{\label{fig:fathist} Histogram and fitted distributions of the fatalities data
  in Austria.}
\end{figure}
...
the four parameter Box-Cox power exponential distribution (family \code{BCPE}), has the best fit
to the data (though only by a small difference). Therefore, we use the \fct{BCPE} family function
and estimate a model using the weekly information as a cyclical seasonal effect given by
$$
\texttt{num} \sim \mathcal{BCPE}(\log(\mu) = f_{\mu}(\texttt{week}),
...
regression splines. The log-link for parameter $\mu$, $\sigma$ and $\tau$ ensures positivity. We
can use the following model formula for estimation
<<>>=
f <- list(
  num   ~ s(week, bs = "cc"),
  sigma ~ s(week, bs = "cc"),
...
also possible to incorporate user defined model terms.
...
@
\caption{\label{fig:fatmodel_resids} Histogram and quantile-quantile plot of the
...
nd$fit <- sapply(c(0.025, 0.1, 0.5, 0.9, 0.975),
  FUN = function(p) { family(b)$q(p, par) })
d19w <- reshape(d19, idvar = "week", timevar = "year", direction = "wide")
par(mar = c(4.1, 4.1, 0.1, 0.1))
matplot(d19w$week, d19w[, -1], data = d19,
  type = "b", pch = 16, col = gray(0.1, alpha = 0.1),
  xlab = "Week", ylab = "#Fatalities")
matplot(nd$week, nd$fit, type = "l", lty = c(3, 2,  1, 2, 3),
  col = 1, lwd = 3, add = TRUE)
lines(num ~ week, data = d20, col = 2, lwd = 2, type = "b", pch = 16, cex = 1.3)
@
\caption{\label{fig:fatmodel_compare}. Predicted 2.5\%, 50\% and 97.5\% quantiles of
  the cyclic seasonal week model, black dashed lines, and number of fatalities in 2020 up
  to week 45, red solid line. Data before 2020 is shown by the light gray solid lines in the
  background.}
\end{figure}
