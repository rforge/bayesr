...
\code{d2m} & The temperature to which the air, at 2 metres above the surface of the Earth,
...
\code{q\_prof\_PC1} & This variable has been derived from the vertical profile of specific humidity 
                      (labelled \code{q} at the ECMWF) by principal component analysis. This is
                      the first principal component. \\
\code{cswc\_prof\_PC4} & \\
\code{t\_prof\_PC1} & \\
\code{v\_prof\_PC2} & \\
...
\code{sqrt\_lsp} & Accumulated liquid and frozen water, comprising
                   rain and snow, that falls to the Earth's surface and which is generated by
                   the cloud scheme in the ECMWF Integrated Forecasting System (IFS). \\ \hline
\end{tabular}
\caption{\label{tab:desc} Describtions of the parameters taken directly or derived from ERA5.}
\end{table}
\fixme{Thorsten, complete table please!}
%
...
truncation of the count data and its overdispersion. A distribution for extremely
skewed count data is the zero-truncated versions of the Sichel.
...
\fct{SICHEL} are available within
...
(for \fct{ztnbinom\_bamlss} parameters $\mu$ and $\theta$, for \fct{ztSICHEL}
...
only two parameters, hence the first two formulae are used.
...
to \code{maxit = 1000}. For the sampling we allow $1000$ iterations as burn-in
phase, and apply a thinning of the resulting chain of $3$.  Running
\code{n.iter = 4000} iterations leads to $1000$ MCMC samples in the
...
+    thin = 3, burnin = 1000, n.iter = 4000,
+    light = TRUE)        
\end{Sinput}
\begin{Soutput}
