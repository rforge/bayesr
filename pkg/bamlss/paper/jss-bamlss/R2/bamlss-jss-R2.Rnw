...
probability of the sample candidate which is computed in each iteration, and indicate
...
\caption{\label{fig:fatmodel_resids} Histogram and quantile-quantile plot of the
  resulting randomized quantile residuals of the distributional regression model.}
\end{figure}
