...
|********************| 100%  0.00sec 109.24min
|********************| 100%  0.00sec 112.98min
|********************| 100%  0.00sec 115.67min
\end{Soutput}
\end{Schunk}
%
The model was fitted on three Intel(R) Xeon(R) CPU E5-4660 v4 cores with 2.20GHz
on which the boosting took about 28.99~minutes and the average sampling time about 1.9~hours.
Note that for the \fct{ztnbinom\_bamlss} model boosting takes about 7~minutes and average sampling
time is about 9~minutes, only.
The reason is that the \fct{ztSICHEL} family is not yet implemented so efficiently.
...
All models are provided within the \pkg{FlashAustria} package and can be loaded with
<<eval=FALSE>>=
data("FlashAustriaModel", package = "FlashAustria")
@
...
\end{figure}
%
The worm plots (Figure~\ref{fig:appworm}) reveal that \code{ztSICHEL} and \code{ztnbinom} 
underestimate the mass of the tail. The distributional model
based on the zero-truncated Sichel exhibits the best overall calibration, 
therefore, we focus on the \fct{ztSICHEL} model in this illustration.
...
\Sexpr{round(FlashAustriaModel$model.stats$optimizer$boost_summary$summary$mu["s(d2m)",2])}.
...
for two regression coefficients of the term \code{s(q\_prof\_PC1)} for parameter $\mu$ of the
Sichel distribution. The traces
...
reveal quite some autocorrelation after the thinning,
which suggests that one should increase the sampling efforts in a final model run.
...
(Figure~\ref{fig:appboost}) reveals that the terms
\code{s(q_prof_PC1)}, \code{s(sqrt_cape)} adn \code{s(d2m)}
have the largest contribution for improving the fit. 
