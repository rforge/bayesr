...
to stabilize the variance additionally we log-transform the dependent variable.
...
b <- bamlss(f, data = d19, family = NO)
@
After the estimation algorithms are finished, the estimated effects, e.g., for the \fct{NO} 
