...
(3)~Enhanced inference infrastructure, typically Bayesian, broaden classical frequentist methodology.
...
The idea of a Lego toolbox for regression models has of course been around for some time, in some 
implementations, Bayesian and frequentist, there is not only the possibility to easily 
implement new distributions, but also model terms, from splines to neural networks to regression 
trees. In some implementations optimization routines may also be exchanged. Nevertheless, to the
best of our knowledge, the possibility to create new models through a flexible implementation that 
combines all concepts in an unified way is unique.
The following is a list of the best known packages in the \proglang{R} ecosystem,
whose implementations are designed to be extremely flexible.
%
\begin{itemize}
...
of a wide range of response distributions. However, for complex predictor structures, e.g.,
complex space-time interactions, estimation may be challenging. In contrast, \pkg{mgcv} excels
at providing highly-optimized algorithms for general smooth models
\citep{bamlss:Wood+Pya+Saefken:2016} as well as the dedicated \fct{bam} function for big data
that is long and/or wide \citep{bamlss:Wood+Li+Shaddick:2017}.
...
(\url{https://ec.europa.eu/eurostat/}) and includes weekly fatalities in Austria from 2000 to 45
weeks in 2020.
...
data at hand. Therefore, we use the several families as implemented in the \pkg{gamlss.dist}
package \citep{bamlss:gamlss.dist} and compare the model fits using the
...
families <- list("NO" = NO(), "GA" = GA(), "BCCG" = BCCG(mu.link = "log"),
  "BCPE" = BCPE(mu.link = "log"), "BCT" = BCPE(mu.link = "log"))
families <- ls()
if(!file.exists("FatDens.rda")) {
  ...
  for(j in seq_along(families)) {
    b <- bamlss(num ~ 1, data = fatalities, family = families[[j]])
    par <- predict(b, type = "parameter")
    ll <- c(ll, family(b)$loglik(fatalities$num, par))
    dnum <- cbind(dnum, family(b)$d(fatalities$num, par))
  }
  save(ll, dnum, file = "FatDens.rda")
} else {
  load("FatDens.rda")
}
...
hist(fatalities$num, breaks = "Scott", freq = FALSE,
  main = "", xlab = "#Fatalities")
i <- order(fatalities$num)
col <- rev(qualitative_hcl(ncol(dnum), palette = "dynamic"))
...
\end{figure}
The histogram of the number of fatalities together with fitted densities and corresponding
log-likelihoods for several estimated distributions is shown in
Figure~\ref{fig:fathist}. One of the most flexible distributions of the \pkg{gamlss.dist} package,
the four parameter Box-Cox power exponential distribution (family \fct{BCPE}), has the best fit
to the data (though only by a small difference). Therefore, we use the \fct{BCPE} family function
...
$$
\texttt{num} \sim BCPE(\log(\mu) = f_{\mu}(\texttt{week}),\,
...
\caption{\label{fig:fatmodel_effects} Estimated effects of \code{week} on parameter
  $\mu$, $\sigma$, $\nu$ and $\tau$ of the Box-Cox power exponential model. The grey shaded areas
...
relationship for parameter $\mu$, $\sigma$ and $\nu$. The \code{week} effect on parameter $\tau$
...
\caption{\label{fig:fatmodel_compare} Predicted 5\%, 50\% and 95\% quantiles of
  ...
  to week 45, red points and line. Data before 2020 is shown in light gray in the
  background.}
\end{figure}
...
cardiovascular stress. The mortality rate from beginning of 2020 to week 45 is plotted in red and 
