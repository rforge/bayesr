...
\begin{Sinput}
R> set.seed(456)
R> flash_model_ztSICHEL <- bamlss(f, data = FlashAustriaTrain, ## Standard interface.
...
+    light = TRUE)                                             ## Return in light mode.
\end{Sinput}
\begin{Soutput}
logLik -36607.4 eps 0.0003 iteration 1000 qsel 10
elapsed time: 31.94min
Starting the sampler...
|********************| 100%  0.00sec 210.34min
\end{Soutput}
\end{Schunk}
%
The model was fitted in parallel on 50 Intel(R) Xeon(R) CPU E5-4660 v4 cores with 2.20GHz
on which the boosting took about 31.94~minutes and average sampling time about 3.5~hours
...
plot(resids, which = "wp", main = "In sample worm plots", col = hcl.colors(3, "Dark 3"), cex = 0.5) 
@
...
par(mfrow = c(1, 2))
plot(b, model = "mu", term = c("s(sqrt_cape)", "s(q_prof_PC1)"),
