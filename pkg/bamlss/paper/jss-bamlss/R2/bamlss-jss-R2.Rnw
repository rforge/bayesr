...
first element specifies terms for the response \code{counts}, which is named
$\mu$ in the \pkg{bamlss} familiy object.  The second element specifies the
formula for parameter $\theta$.  We adopt P-splines
...
which leads to the following specification of the model (formula):
%
<<eval=FALSE>>=
f <- list(
