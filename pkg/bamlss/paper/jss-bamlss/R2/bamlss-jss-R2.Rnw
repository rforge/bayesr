...
\Abstract{
  ...
  become available, facilitated by more powerful computing infrastructure.
  (2)~Full probabilistic modeling
...
techniques using standard algorithms like
iteratively weighted least squares (IWLS, \citealp{bamlss:Gamerman:1997}) or
...
approximation as implemented in the function \fct{CRPS} in the \pkg{bamlss} package. Since
...
\hline
\end{tabular}
\caption{\label{tab:DIC} DIC, WAIC and CRPS comparison of the 6 distributional cyclical week models.}
\end{table}
Here, the four parameter \fct{BCPE} model has the lowest DIC, WAIC and CRPS, therefore, we
examine this model in more detail in the rest of the analysis. However, the differences
are quite small, i.e., the final results of the three very flexible families \fct{BCT}, \fct{BCCG} 
and \fct{BCPE} are expected to be quite similar.
...
relationship for all parameters, however, 95\% credible intervals for parameter $\nu$
...
\section{A flexible model framework} \label{sec:flexreg}
...
\begin{itemize}
 ...
 \item Multivariate responses such as multivariate normal, multivariate t or Dirichlet regression
   (for analyzing compositional data, \citealp{bamlss:Klein+Kneib+Klasen+Lang:2015, bamlss:Stasinopoulos+Rigby:2019}).
...
e.g., \citealp{bamlss:Klein+Kneib+Lang:2015}).
...
the zero-truncated negative binomial against other distributions that can capture the
...
end. Finally, we set \code{light = TRUE} to return a lighter object, i.e., the model.frame
and design and penalty matrices are not stored:
%
\begin{Schunk}
\begin{Sinput}
R> set.seed(123)
...
As a first diagnostic we check the log-likelihood contributions of the
...
reveal quite some auto-correlation after the thinning, which suggests that one
could increase the sampling efforts.

\begin{figure}[t!]
\centering
<<echo=FALSE, fig=TRUE>>=
b2 <- b
b2$samples[[1]] <- b2$samples[[1]][, c("mu.s.s(sqrt_cape).b1", "mu.s.s(sqrt_cape).b2")]
...
occurs on the north-western corner of the domain, which can be seen at 23~UTC.
The propagation of the front is blocked by the main Alpine ridge located at
$47^\circ~N$. On the subsequent day 2001-09-16 one can see that the probability
...
implementations, \pkg{bamlss} is a modular ``Lego toolbox'', so that users who are interested
in extensions can easily write new families, optimizers or sampling functions, where all
