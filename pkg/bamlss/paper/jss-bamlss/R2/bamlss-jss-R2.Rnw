...
normal distribution (family \fct{NO}), gamma distribution (family \fct{GA}), Box-Cox t
distribution (family \fct{BCT}), Johnson's Su distribution (family \fct{JSU}),
Box-Cox power exponential distribution (family \fct{BCPE}) and the Box-Cox Cole and
Green distribution (family \fct{BCCG}). First, we only estimate the distributional parameters
without covariates, to get a first impression which distribution fits well. We compare visually
...
In the next step we estimate Bayesian models that take into
account the cyclical pattern over the year, in winter there are usually quite a
lot of influenze and other viral infections that lead to a higher mortality
rate. The models are estimated using the predictor $\eta_k = f_{k}(\texttt{week})$ for each parameter
...
if(!file.exists("FatModel.rda")) {
  set.seed(456)
  b <- bamlss(f, data = d19, family = JSU(mu.link = "log"),
...
and only save every 10th sample.


The values of the DIC for the 6 models are given in Table~\ref{tab:DIC}.
\begin{table}[h!]
\centering
\begin{tabular}{lcc}
\hline
Family & DIC & CRPS\\ 
\hline
\fct{NO}   & 12289.582 & 51.39974
\fct{GA}   & 12258.501 & 51.22791
\fct{BCT}  & 12202.326 & 50.83966
\fct{BCPE} & 12189.379 & 50.78673
\fct{BCCG} & 12188.076 & 50.81369
\fct{JSU}  & \textbf{9390.927} & \textbf{50.77544}

\hline
\end{tabular}
\caption{\label{tab:DIC} DIC comparison of the 6 distributional cyclical week models.}
\end{table}
Here, the four parameter \fct{JSU} model has the lowest DIC.

After the estimation algorithms are finished, the estimated
effects can be visualized instantly using the plotting method.
<<eval=FALSE>>=
...
and $\tau$ are larger such that the effects to not cross the horizontal zero line
...
visible increase in the upper quntile, which is probably due to particularly
...
CRPS <- function(object, newdata = NULL) {
  ...
  for(i in 1:n) {
    foo1 <- function(x) {
      fam$p(x, par[i, , drop = FALSE])^2
    }
    foo2 <- function(x) {
      (fam$p(x, par[i, , drop = FALSE]) - 1)^2
    }
    int1 <- integrate(foo1, -Inf, y[i])$value
    int2 <- integrate(foo2, y[i], Inf)$value
    crps[i] <- int1 + int2
  }
  return(crps)
}
