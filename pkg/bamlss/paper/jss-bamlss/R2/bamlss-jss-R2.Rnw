...
distribution \citep{bamlss:DiCiccio:2004}, has the best
fit to the data (though only by a small difference). Hence, with some certainty one of the
chosen distribution models will have a fairly good model fit in further analysis.

In the next step estimate a Bayesian models that take into
...
rate. The models are specified with
$$
\texttt{num} \sim \mathbf{\mathcal{D}}\left(h_{1}(\theta_{1}) = f_{1}(\texttt{week}), \,
  h_{2}(\theta_{2}) = f_{1}(\texttt{week}), \dots, \,\, h_{4}(\theta_{4}) =
  f_{4}(\texttt{week})\right)
$$
