...
\Keywords{\textbf{Backfitting, distributional regression, gradient boosting, MCMC, penalization, probabilistic forecasting, \proglang{R}}
\Plainkeywords{\textbf{Backfitting, distributional regression, gradient boosting, MCMC, penalization, probabilistic forecasting, \proglang{R}}

\Abstract{
  Over the last decades, the challenges in applied regression and in predictive modeling have
  been changing considerably: (1)~More flexible \textbf{regression} model specifications are needed as \textbf{data sizes and available information are steadily increasing, and as a consequence demanding for more powerful computing infrastructure.}
  (2)~Full probabilistic modeling \NK{based on distribution regression}
  rather than predicting \NK{single points such as means or expectations of the underlying distributions} is crucial in many applications.
  (3)~\textbf{Availability of Bayesian inference has gained in importance both as an appealing framework for
  regularizing or penalizing complex models and estimation therein as well as a natural alternative to classical
  frequentist inference.} However, while there has been a lot of research \textbf{on all  three challenges and the development of corresponding software packages},
 a modular software implementation that
  allows to easily combine all three aspects has not yet been available \textbf{for the general framework of distributional regression}. \textbf{To fill} this gap,
  the \proglang{R} package \pkg{bamlss} is introduced for Bayesian additive models for location,
  scale, and shape (and beyond). \textbf{The latter have been from a methodolgical perspective as an important area towards distributional regression in both classical frequenstist but also Bayesian frameworks}. At the core of the package are algorithms for highly-efficient Bayesian
  estimation and inference that can be applied to generalized additive models (GAMs) or
  generalized additive models for location, scale, and shape (GAMLSS). However, its building blocks are designed as ``Lego bricks''
...
(1)~\textbf{Large} data sets that can be both long (many observations) and/or wide (many variables or complex effect types).
...
also known as \textbf{Bayesian structured additive} distributional regression \citep{bamlss:Klein+Kneib+Lang+Sohn:2015} which
encompasses  (generalized) linear models \citep{bamlss:Nelder+Wedderburn:1972} as special cases.
\textbf{Bayesian inference in these models can be seen} as a natural framework
...
when fitting such models to big data -- long and/or wide \textbf{was soll das long and wide hier?} -- classical estimation
techniques using standard algorithms like iteratively weighted least squares \colorbox{yellow}{(IWLS)} \textbf{hier muss ide Referenz mit Gamermann und co.} or
...
might be necessary or \textbf{further advanced} custom algorithms \citep{bamlss:Wood:2017}. Hence,
to facilitate \textbf{addressing all challgenges and needs simultaneously independent of a specific estimation strategy} and fitting algorithms, the \pkg{bamlss} package for the \proglang{R}
...
but also the estimation algorithm \textbf{such as a specific} MCMC sampler.
...
of the best known packages for regression models in the \proglang{R} ecosystem \textbf{was soll hier ecosystem sein?}, whose
...
that is long and/or wide \citep{bamlss:Wood+Li+Shaddick:2017}. \textbf{All these packages rely on frequenstist estimation strategies.} Moreover, the package provides
...
sampling times from these generic MCMC engines can become long, sometimes prohibitively \textbf{long.}
This has been addressed by dedicated packages for Bayesian additive models, e.g., with the standalone
package \pkg{BayesX}~\citep{bamlss:Brezger+Kneib+Lang:2005} \textbf{Referenz falsch hier. es muss ide 2015 mit mehr autoren isnbesondere niki und mir sein!} being the first to provide highly-efficient
sampling schemes for very large data sets as well as spatial/multilevel models \textbf{and structured additive distributional regression}. An \proglang{R}
...
\item \emph{Regularized estimation} \textbf{and explicit variable selection} might be necessary, though, for going beyond the models
...
\pkg{gamboostLSS} \citep{bamlss:Hofner+Mayr+Schmid:2014}. \textbf{However, many packages do not cover the Bayesian posterior estimation parts}.
...
(\code{edf}), the log-likelihood (\code{logLik}), etc.
...
\fct{smoothCon} is called (see also Section~\ref{sec:package}).
...
Moreover, using \pkg{ba mlss}, estimation engines can also be exchanged. To give
...
In this example we will now extend the framework and estimate a distributional regression
...
weeks in 2020. The data is provided in the \pkg{bamlss} package and can be loaded with
<<>>=
data("fatalities", package = "bamlss")
@
The data set contains information on the number of fatalities (variable \code{num}) and
the year and week of recording (variables \code{year} and \code{week}). The motivation
for this analysis is to model a reference mortality, which allows one to describe
the excess mortabilty caused by exceptional events such as pandemics \citep{bamlss:Leon:2020} or
natural catastrophes \citep{fouillet2008}.
...
mortality of the pandemic \citep{bamlss:Leon:2020}.
...
data at hand. The \pkg{gamlss.dist} package \citep{bamlss:gamlss.dist} provides a number
of distributions which can also be used for estimation in \pkg{bamlss}. In this example, we consider
6 distributions with different number of parameters, from 2 to 4. In detail these are the
normal distribution (family \fct{NO}), gamma and generalized gamma distribution (family \fct{GA}
and \fct{GG}), Box-Cox t distribution (family \fct{BCT}), Box-Cox power exponential
distribution (family \fct{BCPE}) and the Box-Cox Cole and
Green distribution (family \fct{BCCG}). First, we only estimate the distributional parameters
without covariates, to get a first idea which distribution fits well. We compare visually
as well as by the deviance information criterion (DIC, the complete \proglang{R}-code is
available in the supplements). Note that we do not
...
legend("topright", paste0(dn, ": ", round(dic$dic1, 2)),
  ...
  title = "Family & DIC")
@
\caption{\label{fig:fathist} Fatalities data in Austria (2000-2019), histogram and
  fitted distributions.}
\end{figure}
The histogram of the number of fatalities together with the fitted
densities and corresponding DIC values is shown in
Figure~\ref{fig:fathist}. One of the most flexible distributions of the
\pkg{gamlss.dist} package, the four parameter Box-Cox t
...
parameters of the selected distributions is 4, we supply 4 formulae. Internally, \pkg{bamlss}
only processes the relevant formulae depending on the family. The \pkg{gamlss.dist} package
...
(but not the response \code{num}). Moreover, note that all smooth
...
\fct{BCPE} family of the \pkg{gamlss.dist} package can be estimated with
<<echo=FALSE, results=hide>>=
...
and only save every 10th sample. We did the same estimation step analogously with the
other distributions.

In addition to the DIC, there is also the Watanabe-Akaike information criterion (WAIC, 
\citealp{watanabe2010asymptotic}) to investigate the goodness of fit.
In \pkg{bamlss}, the WAIC of the fitted model can be computed with
<<>>=
WAIC(b)
@
The function returns two values for the WAIC, depending on the type of the estimated number
of parameters of the model (\code{p1} uses expected differences of log densities, whereas \code{p2}
is computed using the variance in the log density, for details
see \citealp{watanabe2010asymptotic}).

Another criterion for assessing model calibration is the continuous rank probability score (CRPS,
\citealp{bamlss:Gneiting+Raftery:2007}).
The \proglang{R} package \pkg{scoringRules} 
\citep{bamlss:Jordan+Krueger+Lerch:2018} provides tools for model calibration checks using the CRPS.
...
approximation as implemented in the function \fct{CRPS} in the \pkg{bamlss} package. Because
the CRPS is a forecasting score, we computed the CRPS using 10-fold cross validation for
each model. For example, the CRPS for the \fct{BCPE} model is computed with
<<eval=FALSE>>=
set.seed(789)
...
The values of the DIC, WAIC and CRPS for the 6 models are given in Table~\ref{tab:DIC}.
\begin{table}[h!]
\centering
\begin{tabular}{lccc}
\hline
...
\hline
\fct{NO}   & 12291.75 & 12294.20 & 51.89 \\
\fct{GA}   & 12261.06 & 12262.40 & 51.72 \\
\fct{GG}   & 12248.10 & 12249.31 & 51.52 \\
\fct{BCT}  & 12215.85 & 12206.26 & 51.44 \\
\fct{BCCG} & 12190.41 & 12188.05 & 51.31 \\
\fct{BCPE} & \textbf{12188.30} & \textbf{12184.73} & \textbf{51.26} \\
\hline
\end{tabular}
\caption{\label{tab:DIC} DIC, WAIC and CRPS comparison of the 6 distributional cyclical week models. \textbf{For the CRPS, 10-fold cross validation was used.}}
\end{table}
Here, the four parameter \fct{BCPE} model has the lowest DIC, WAIC and CRPS, therefore, we
examine this model in more detail in the rest of the analysis.

After the estimation algorithms are finished, the estimated effects, e.g., for the \fct{BCPE} 
...
\caption{\label{fig:fatmodel_effects} Estimated effects of \code{week} on parameter
  $\mu$, $\sigma$, $\nu$ and $\tau$ of the Box-Cox power exponential model. The grey
...
relationship for all parameters, however, 95\% credible intervals \textbf{NIKI: welche denn, habe ich vorher auch gefragt: pointwise oder simultaneous} for parameter $\nu$
and $\tau$ are larger such that the effects do not cross the horizontal zero line
...
residuals \citep{bamlss:Dunn+Smyth:1996} using histograms or quantile-quantile (Q-Q) plots \textbf{replace by quantile residuals}. \textbf{das stimmt so nicht probability integral transforms (PIT). histograms ist ja nicht klar, bzw. man macht dann die PITs und guckt ob die gleichverteilt sind. fuer PIT muesste man Gneiting zitieren. ausserdem sind es keine QQ plots, glaube ich wie sie standardmaessig berechnet werden. mein Vorschlag ist nach wie vor durchgaengig randomized quantile resdiulas (RQR) zu benutzen als terme} 
...
@
\caption{\label{fig:fatmodel_resids} Histogram and Q-Q plot \textbf{replace by quantile residuals} including 95\% credible
  intervals (dashed lines) of the resulting randomized quantile residuals of the distributional
  regression model.}
\end{figure}
By setting \code{c95 = TRUE}, the Q-Q plot \textbf{replace by quantile residuals} includes 95\% credible intervals.
According the histogram and the Q-Q plot \textbf{replace by quantile residuals} of the resulting randomized
...
\caption{\label{fig:fatmodel_compare} Predicted 5\%, 50\% and 95\% quantiles of
  the cyclic seasonal week model, black lines, and number of fatalities in 2020 up
  to week 46, red points and line. Data before 2020 is shown in light gray in the
  background.}
\end{figure}
The resulting plot is shown in Figure~\ref{fig:fatmodel_compare} and reveals
...
heat waves leading to greater cardiovascular stress \citep{fouillet2008}. The
...
\section{A flexible model framework \textbf{das finde ich jetzt wenig informativ. framework for what?}} \label{sec:flexreg}
...
\textbf{and principled constructions for such priors. die priors sind auch nicht fuer beta!}.
...
\begin{itemize}
 ...
 \item Multivariate responses such as multivariate normal, multivariate t or Dirichlet regression
   (for analyzing compositional data, \citealp{bamlss:Klein+Kneib+Klasen+Lang:2015}). \textbf{Rigby and Stasinopolous 2019 removed here, because they do not do this!}
...
\item \emph{Information criteria} can be used to compare different model specifications.  
  ...
  \citealp{bamlss:Hastie+Tibshirani:1990}) is computed and the total degrees of freedom
...
e.g., \citealp{bamlss:Klein+Kneib+Lang:2015}, \textbf{for the case of the overdispersion parameter of the negative binomial distribution}).
...
\section{Flexible count regression for lightning reanalysis \textbf{in welchem sinne reanalysis?}} \label{sec:application}

\textbf{sample size noch immer nicht angegeben im text}
...
The reanalysis data come from the fifth generation of the ECMWF (European
...
order of a decade, here 2010--2018.  With a statistical model on hand one could
predict lightning counts for the time before 2010 and thus analyze lightning
events in the past for which no observations are available. On the one hand
this will increase our knowledge about physical processes leading to such
events. On the other hand this will enable to quantify the development of these
extreme short-term events under the light of a changing climate \citep{westra2014}.
%
\begin{table}[ht!]
\centering
...
\code{hcc} & High cloud cover. The proportion of a grid box covered by cloud
             occurring in the high levels of the troposphere (approx.~$6km$ and above). \\ 
\code{str} & Surface net thermal radiation. Thermal radiation refers to radiation emitted by the 
             atmosphere, clouds and the surface of the Earth. \code{str} is the
             difference between downward 
...
\code{q\_prof\_PC1} & This variable has been derived from the vertical profile of specific humidity 
             (labelled \code{q} at the ECMWF) by principal component analysis. This is the first 
             principal component. \\ 
...
\end{tabular}
\caption{\label{tab:desc} Describtions of the parameters taken directly or derived from ERA5.}
\end{table}
%
The covariates are taken directly or are derived from ERA5. For this study  only a
small subset of parameters is on hand \citep{era5a, era5b}. It encloses variables that
are well known for being good candidates for predicting or explaining convective
events that also lead to lightning discharges
\citep[e.g.,][]{bamlss:Simon+Fabsic+Mayr+Umlauf+Zeileis:2018}, in Table~\ref{tab:desc} a
description of the physical interpretation of the variables is provided.
...
from the one in \citet{bamlss:Simon+Mayr+Umlauf+Zeileis:2019}. Therefore it is worth testing
the zero-truncated negative binomial against other distributions that could capture the
truncation of the count data and its overdispersion. Two of these distributions for extremely
skewed count data are zero-truncated versions of the Sichel and the beta negative 
binomial.

The zero-truncated negative binomial distribution is implemented as
\fct{ztnbinom\_bamlss} within \pkg{bamlss}.  Implementations of the Sichel
\fct{SICHEL} and the beta negative binomial \code{BNB()} are available within
\pkg{gamlss.dist}.  Using \fct{trun} from \pkg{gamlss.tr} zero-truncated
versions of the two GAMLSS families can be generated that can instantaneously be
plugged into the \code{family} argument of the \fct{bamlss} model call.
...
the zero-truncated Sichel.
To specify smooth terms for all three distributional parameter $\mu$, $\sigma$ and $\nu$,
we set up a \code{list} of formulas with length three:
%
<<eval=FALSE>>=
f <- list(
...
@
%
Here, we adopt P-splines
\citep{bamlss:Eilers+Marx:1996}, which are well known for their sampling
properties.  Specifying smooth terms within \pkg{bamlss} formulae builds on the
\pkg{mgcv} infrastructure \citep{bamlss:Wood:2019} provided by \code{s()}.
...
\code{ztSICHEL}, and a data set \code{FlashAustriaTrain}. Within the
...
\citep{bamlss:Simon+Mayr+Umlauf+Zeileis:2019}.  We set the number of iterations
to \code{maxit = 1000}. For the sampling we allow another $500$ iterations as burn-in
phase, and apply a thinning of the resulting chain of $50$.  Running
\code{n.iter = 1500} iterations over \code{cores = 50} cores leads to $1000$ MCMC samples in the
end (note that parallel chains are started using function \fct{mclapply} of the
base \proglang{R} \pkg{parallel} package). Finally, we set \code{light = TRUE} to return a
lighter object, i.e., the model.frame and design and penalty matrices are not stored:
%
\begin{Schunk}
\begin{Sinput}
R> set.seed(123)
...
\end{Sinput}
\begin{Soutput}
logLik -36607.4 eps 0.0003 iteration 1000 qsel 10
elapsed time: 31.76min
Starting the sampler...
|********************| 100%  0.00sec 502.15min
\end{Soutput}
\end{Schunk}
%
The model was fitted in parallel on 50 Intel(R) Xeon(R) CPU E5-4660 v4 cores with 2.20GHz
on which the boosting took about 31.76~minutes and average sampling time about 8.4~hours
(note that sampling time can usually reduced tremendously by implementing the score
and Hessian functions of the family in \proglang{C} as in \fct{ztnbinom\_bamlss}).

Analogous models have been fitted for the two other distributions, zero-truncated
negative binomial \code{flash_model_ztnbinom} and zero-truncated beta negative binomial
\code{flash_model_ztBNB}.
To select one of the three models, we examine their calibration using a worm plot
...
%
%% TODO: * document c.bamlss.residuals, * ylim for "WP", * default for newdata
<<eval=FALSE>>=
resids <- c(
  "ztnbinom" = residuals(flash_model_ztnbinom),
  "ztBNB"    = residuals(flash_model_ztBNB),
...
\begin{figure}[t!]
\centering
<<fig=TRUE, height=5.5, width=6, echo=FALSE>>=
resids <- c(
...
@
\caption{Worm plots for the three distributional models.}
\label{fig:appworm}
\end{figure}
%
Though, the worm plots (Figure~\ref{fig:appworm}) reveal different tail behavior
of the distributions---\code{ztSICHEL} and \code{ztnbinom} underestimate the mass of
the tail and \code{ztBNB} slightly overestimates the tail---the distributional model
based on the zero-truncated Sichel exhibits the best overall calibration.
Thus, from now on we focus on analyzing this model.
...
These two terms as well as all other terms with a strong contribution
act on the distributional parameter \code{mu}. The contributions for the terms of the
parameter \code{nu} play also an important role. The contributions for the terms of the
distributional parameter \code{sigma} seem negligible.
...
for three regression coefficients of the term \code{s(sqrt_cape)} for parameter $\mu$, $\sigma$
and $\nu$ of the Sichel distribution. The traces
...
for parameter $\mu$ and $\sigma$ reveal quite some auto-correlation after the thinning,
which suggests that one could increase the sampling efforts.
...
splines from the term \code{s(sqrt\_cape)} of the model \code{nu}.}
\label{fig:apptrace}
\end{figure}
...
one can go further and investigate the estimated effects. The boosting summary
(Figure~\ref{fig:appboost}) revealed that the terms
\code{s(q_prof_PC1)}
and
\code{s(sqrt_cape)}
had the largest contribution for improving the fit. 
...
The effects are presented on the scale of the linear predictor of the
...
plot(flash_model_ztSICHEL,
  model = "mu",
  term  = c("s(q_prof_PC1)", "s(sqrt_cape)")
)
@
%
\begin{figure}[t!]
...
<<echo=FALSE, fig=TRUE, height=4.7, width=9, png=TRUE, pdf=FALSE>>=
par(mfrow = c(1, 2))
plot(b, model = "mu", term = c("s(q_prof_PC1)", "s(sqrt_cape)"),
  ask = FALSE, spar = FALSE,
  rug = TRUE, col.rug = "#39393919")
### plot(b, model = "nu", term = "s(q_prof_PC1)",
###   ask = FALSE, spar = FALSE,
###   rug = TRUE, col.rug = "#39393919")
@
\caption{Effect of the terms \code{s(q\_prof\_PC1)} and \code{s(sqrt\_cape)}
from the model \code{mu}. Credible intervals derived from MCMC samples.}
\label{fig:appeffect}
\end{figure}
%
\code{s(q_prof_PC1)} shows a clear monotonic decrease.
...
%
\code{s(sqrt_cape)} reveals a monotonic increasing shape.
...
%%% check the marginal calibration of the distribution. This is commonly done by
%%% displaying a qq-plot or a worm plot \citep{bamlss:Buuren+Miranda:2001}.  For
%%% evaluating count data regression model the hanging rootogram serves as popular
%%% tool \citep{kleiber2016visualizing}. First we predict the distributional
%%% parameter on out-of-sample data \code{FlashAustriaEval} for which lightning
...
Now given good calibration of the count data distribution,
good convergence and sample characteristics of the gradient
boosting optimizer and MCMC sampler, and physically interpretable effects,
we can take our model and predict a case
...
lightning counts within a grid cell and hour. Over night the situation calms down,
and reaches a minimum around 6~UTC on 2001-09-16 (bottom left panel of
Figure~\ref{fig:appcase}). Finally, strong lightning events were reconstructed for the
afternoon of the second day (2001-09-16) in particular on the Southern side of the main
Alpine ridge (bottom right panel of Figure~\ref{fig:appcase}).
...
elaborate data processing infrastructures and extractor functions are completely provided by
the package.
