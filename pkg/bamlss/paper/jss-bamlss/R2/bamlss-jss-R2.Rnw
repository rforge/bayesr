...
\Abstract{
  ...
  generalized additive models for location, scale, and shape (GAMLSS), or more general
...
As noted in the introduction, the \pkg{bamlss} package leverage the infrastructure from the \proglang{R}
package \pkg{mgcv} \citep{bamlss:Wood:2019} for setting up the design
...
(\url{https://ec.europa.eu/eurostat/}) and includes weekly fatalities in Austria from the beginning
of 2000 up to week~46 in 2020. The data is provided in the \pkg{bamlss} package and can be loaded with
<<>>=
...
the excess mortality caused by exceptional events such as pandemics \citep{bamlss:Leon:2020} or
...
As a first distributional model for analysis we use the normal distribution, which seems
plausible, the number of fatalities is far away from zero in the
thousands and the distribution is only moderately skew towards larger rates. Hence,
to stabilize skewness and variance we additionally log-transform the dependent variable.
Moreover, in winter there are usually quite a lot of influenza and other viral
infections, which lead to a higher mortality rate. Therefore, we specify a
a cyclic smooth spline for the \code{week}s of the year to capture the seasonal variation,
i.e., the model is estimated using the predictor $\eta_k = f_{k} (\texttt{week})$ for parameter
$\mu$ and $\sigma$ of the normal distribution and functions $f_{k}(\texttt{week})$ are 
unspecified smooth functions. The model formula is specified by
<<>>=
f <- list(
...
regression spline. Note that model formulae are provided as lists of formulae, i.e., each list
entry represents one parameter of the response distribution. Moreover, note that all smooth
...
In the next step, a full Bayesian semi-parametric distributional regression model using the
\fct{NO} family of the \pkg{gamlss.dist} package can be estimated with
<<echo=FALSE, results=hide>>=
...
b <- bamlss(f, data = d19, family = NO)
@
After the estimation algorithms are finished, the estimated effects can be visualized
instantly using the plotting method.
<<eval=FALSE>>=
...
\caption{\label{fig:fatmodel_effects} Estimated effects of \code{week} on parameter
  $\mu$ and $\sigma$ of the normal location-scale model. The grey
...
relationship for parameter $\mu$ and $\sigma$. One can see a significant increase in mortality 
during the winter months, but uncertainty also increases at this time of year. Likewise, the 
uncertainty increases in the summer months, which is quite natural due to recurrent heat waves that 
can be quite stressful for the cardiovascular system \citep{fouillet2008}.
...
(see Section~\ref{sec:families} for details) and re-transformed to the original scale using
\fct{exp}
<<>>=
...
with a higher number of fatalities in winter. The effect of the standard deviation of the
...
quantile residuals in Figure~\ref{fig:fatmodel_resids}, the model seems to be less
appropriate for very large cases. Therefore, the next step will be to investigate other 
distributions to obtain the best possible model fit.

For the interested reader we have continued the analysis in the vignette for the \pkg{bamlss} 
package with a variety of distributions from the \pkg{gamlss.dist} package, the vignette
is available at \url{http://bamlss.org/articles/fatalities.html}.
...
FlashAustriaModel <- b <- flash_model_ztnbinom
@
%
<<>>=
data("FlashAustria", package = "FlashAustria")
...
\fct{SICHEL} and the beta negative binomial \fct{BNB} are available within
...
of the three distributions. To specify smooth terms for all distributional parameters
(for \fct{ztnbinom\_bamlss} parameters $\mu$ and $\theta$, for \fct{ztSICHEL} and \fct{ztBNB}
parameters $\mu$, $\sigma$ and $\nu$), we set up a \code{list} of formulas with length three:
%
<<eval=FALSE>>=
f <- list(
...
@
%
Internally, only the relevant formulae are processed, i.e., family \fct{ztnbinom\_bamlss} has
only two paremeters, hence the first two formulae are used.
Here, we adopt P-splines \citep{bamlss:Eilers+Marx:1996}, which are well known for their sampling
properties.
...
\fct{ztnbinom\_bamss}, \fct{ztSICHEL}, \fct{ztBNB}, and a data set
...
models with the \fct{ztnbinom\_bamlss} family, the reason being that this family is implemented
much more efficiently and has relatively weak correlation between the distribution parameters in 
comparison, i.e., the mixing of the MCMC chains is significantly better. In addition, the difference 
between the models in terms of prediction is qualitatively very similar.
We set the number of iterations
to \code{maxit = 1000}. For the sampling we allow another $2000$ iterations as burn-in
phase, and apply a thinning of the resulting chain of $10$.  Running
\code{n.iter = 12000} iterations leads to $1000$ MCMC samples in the
...
sampler function \fct{sam\_MCMC}). Finally, we set \code{light = TRUE} to return a
lighter object, i.e., the model.frame and design and penalty matrices are not stored:
%
\begin{Schunk}
\begin{Sinput}
R> set.seed(123)
R> flash_model_ztnbinom <- bamlss(f, data = FlashAustriaTrain,
...
\end{Schunk}
%
The model was fitted on an Intel(R) Xeon(R) CPU E5-4660 v4 with 2.20GHz
on which the boosting took about 31.76~minutes and average sampling time about 8.4~hours.
If the argument \code{binning = TRUE}, this will result in only different observations being used 
...
faster, because fewer floating point operations need to be performed . In addition, if the argument
\code{light = TRUE}, no design or penalty matrices, model frames, results, etc.,
are stored, hence, the returned object requires much less memory.

Analogous models have been fitted for the two other distributions, zero-truncated
Sichel \code{flash_model_ztSICHEL} and zero-truncated beta negative binomial
\code{flash_model_ztBNB}. All models are provided within the \pkg{FlashAustria} package and can be
loaded with
<<eval=FALSE>>=
data("FlashAustriaModel", package = "FlashAustria")
@
The corresponding \proglang{R}-script is provided in the supplemental materials.
...
based on the zero-truncated Sichel exhibits the best overall calibration. However, as
mentioned above the mixing behavior and computational speed of the \fct{ztnbinom\_bamlss} model
is much better and qualitative differences in predictions between the models are very small, 
therefore, we focus on the \fct{ztnbinom\_bamlss} model.
...
%
<<eval=FALSE>>=
pathplot(flash_model_ztnbinom, which = "loglik.contrib")
@
%
\setkeys{Gin}{width=.7\textwidth}
\begin{figure}[t!]
...
and with the autocorrelation function of the chains.
%
...
@
%
Figure~\ref{fig:apptrace} shows the traces and the autocorrelation functions
...
reasonable starting values for the sampling. The autocorrelation functions
for parameter $\mu$ and $\sigma$ reveal quite some autocorrelation after the thinning,
...
<<echo=FALSE, fig=TRUE>>=
b2 <- b
b2$samples[[1]] <- b2$samples[[1]][, c("mu.p.(Intercept)", "theta.p.(Intercept)")]
