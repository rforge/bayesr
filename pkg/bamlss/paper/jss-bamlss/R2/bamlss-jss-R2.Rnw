...
model using electricity load data from the GEFCom2014 forecasting competition
\citep{bamlss:Hong:2016}. A subset of the data is povided in the \pkg{bamlss} package and can
be loaded with
<<>>=
data("GEFCom2014", package = "bamlss")
@
The data set contains information on the electricity load (variable \code{load}) and the
date of measurement (variable \code{Date}). The electricity load data can be visualized with
<<eval=FALSE>>=
GEFCom2014$trend <- 1:nrow(GEFCom2014)
GEFCom2014$yday <- as.POSIXlt(GEFCom2014$date)$yday

par(mfrow = c(1, 2))
plot(load ~ trend, data = GEFCom2014, type = "l")
plot(load ~ yday, data = GEFCom2014)
@
The data in Figure~\ref{fig:GEFCom2014} show a rather strong cyclical trend, i.e., electricity load 
increases in the summer months and also over the turn of the year. In addition, the variation in
summer and winter also seems to be greater than in the transition periods. The overall time trend
is not as pronounced in comparison.
\begin{figure}[!ht]
\centering
\setkeys{Gin}{width=0.4\textwidth}
<<echo=FALSE, fig=TRUE, width=4.5, height=3.8>>=
...
plot(load ~ yday, data = GEFCom2014)
@
\caption{\label{fig:GEFCom2014} GEFCom2014 electricity load data plotted vs.\ day of the year.}
\end{figure}
Therefore, to provide a competitive probabilistic forecast it is
important to account for seasonality (note, for the sake of simplicity, we neglect other
potential covariates). We estimate a location-scale model with
$$
\texttt{load} \sim \mathcal{N}(\mu = f_{\mu,1}(\texttt{trend}) + f_{\mu,2}(\texttt{yday}),\,
  \log(\sigma) = f_{\sigma,1}(\texttt{trend}) + f_{\sigma,2}(\texttt{yday})),
$$
where functions $f_{\mu, j}( \cdot )$ and $f_{\sigma, j}( \cdot )$ are unspecified smooth
functions, which are estimated using regression splines. The log-link for parameter
$\sigma$ ensures positivity. The following model formula is used estimation
<<>>=
f <- list(
  load  ~ s(trend) + s(yday, bs = "cc"),
  sigma ~ s(trend) + s(yday, bs = "cc")
)
@
again, function \fct{s} is the smooth term constructor from the \pkg{mgcv} package
\citep{bamlss:Wood:2019} and with \code{bs = "cc"} a cyclical spline is estimated,
which is a common way to reproduce seasonal effects.
...
also possible to incorporate user defined model terms. In this example we use the normal
distribution family object \fct{NO} from the \pkg{gamlss.dist} package \citep{bamlss:gamlss.dist}
to estimate a full Bayesian semi-parametric distributional regression model with
<<echo=FALSE, results=hide>>=
if(!file.exists("GEFComModel.rda")) {
  set.seed(456)
  b <- bamlss(f, data = GEFCom2014, family = GA)
  save(b, file = "GEFComModel.rda")
} else {
  load("GEFComModel.rda")
}
@
<<eval=FALSE>>=
library("gamlss.dist")
set.seed(456)
b <- bamlss(f, family = NO, data = GEFCom2014)
@
...
<<echo=FALSE, fig=TRUE, width=9, height=3.8>>=
...
@
\caption{\label{fig:GEFCom:effects} Estimated effects of \code{trend} and \code{yday} on
  parameter $\mu$ and $\sigma$ of the normal location-scale model. The grey shaded areas represent
  95\% credible intervals.}
\end{figure}
The estimated effects are shown in Figure~\ref{fig:GEFCom:effects} depicting a clear nonlinear
seasonal effect for parameter $\mu$ and $\sigma$. The estimated overall time trend indicates
that the mean and standard deviation of the modeled normal distribution is slightly increasing.
...
@
\caption{\label{fig:GEFCom_resids} Histogram and quantile-quantile plot of the
...
quantile residuals in Figure~\ref{fig:GEFCom_resids}, the model seems to fit relatively well. 
Only for very low and very high values of \code{Load} the fitted distributions seem
to be less appropriate.

\subsection{Location-scale model v0} \label{sec:locscalemodel:v0}
