...
\shortcites{bamlss:INLA,bamlss:Waldmann+Taylor+Klein+Kneib+Pressler+Schmid+Mayr:2016,bamlss:stan,zeileis2019colorspace,fouillet2008,westra2014}
...
plot2d(p_swiss ~ age, data = nd, ylab = "participation",
  ylim = range(c(nd$p_swiss, nd$p_foreign)),
...
as \code{fatalities}, providing the number (\code{num}) of fatalities in a certain
...
in order to bring out graphically the excess mortality in 2020. Such a strategy is
often employed for assessing the mortality effects of exceptional events such as
pandemics \citep{bamlss:Leon:2020} or natural catastrophes \citep{fouillet2008}.
Therefore, we split the full data into the subsets before and within 2020.
<<eval=TRUE,echo=TRUE>>=
d19 <- subset(fatalities, year <= 2019)
d20 <- subset(fatalities, year >= 2020)
@
To capture the long-term seasonal trend of the fatality numbers, both in terms of
mean and variance, we employ a simple model here: log-fatalities are assumed to be
...
is now a list with elements for the mean (parameter \code{mu}) and standard deviation
(parameter \code{sigma}) of the normal distribution.
<<>>=
f <- list(
...
Finally, a full Bayesian semi-parametric distributional regression model using the
\fct{NO} normal family from the \pkg{gamlss.dist} package can be estimated with
<<echo=FALSE, results=hide>>=
...
relationship for both parameters. The left panel shows that mean log-fatalities are much higher in winter
...
is available at \url{http://www.bamlss.org/articles/fatalities.html}.
