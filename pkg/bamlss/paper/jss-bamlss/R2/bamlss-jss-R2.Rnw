...
<<>>=
data("FlashAustria", package = "FlashAustria")
head(FlashAustriaTrain)[, c("counts", "hcc", "str", "tciw", "q_prof_PC1", "sqrt_cape")]  
...
\end{table}
%
The parameters are taken directly or derived from ERA5 for this study is only a
small subset of parameters available \citep{era5a, era5b}. It encloses variables that
...
truncation of the data and its overdispersion. Two of these distributions for extremely
...
versions of the two GAMLSS families can be generated that can instantenously be
pluged into the \code{family} argument of the \code{bamlss()} model call.
...
To specify smooth terms for all four distributional parameter,
we set up a \code{list} of formulas with length four:
%
<<eval=FALSE>>=
f <- list(
...
Analoguous models have been fitted for the two other distributions, zero truncated
...
plot(flash_model_ztBNB, model = "nu", term = "s(sqrt_cape)", which = "samples")
@
%
...
%
<<fig=FALSE, eval=FALSE>>=
plot(flash_model_ztBNB, model = "nu", term = c("s(sqrt_cape)", "s(q_prof_PC1)"))
@
%
\begin{figure}[t!]
\centering
