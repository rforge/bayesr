...
\Keywords{Backfitting, distributional regression, gradient boosting,
  MCMC, penalization, probabilistic forecasting, \proglang{R}}
\Plainkeywords{Backfitting, distributional regression, gradient boosting,
  MCMC, penalization, probabilistic forecasting, R}
...
\Abstract{
  ...
  as data sizes and available information are steadily increasing, and as a consequence
  demanding for more powerful computing infrastructure.
  (2)~Full probabilistic modeling based on distribution regression
  rather than predicting single points such as means or expectations of the underlying
  ...
  available for the general framework of distributional regression. To fill this gap,
  the \proglang{R} package \pkg{bamlss} is introduced for Bayesian additive models for location,
  scale, and shape (and beyond). The latter have been from a methodological perspective as an 
  important area towards distributional regression in both classical frequentist but also Bayesian 
  frameworks. At the core of the package are algorithms for highly-efficient Bayesian
  estimation and inference that can be applied to generalized additive models (GAMs) or
  generalized additive models for location, scale, and shape (GAMLSS). However, its building
  blocks are designed as ``Lego bricks'' encompassing various distributions (exponential family,
  Cox, joint models, \dots), regression terms (e.g., linear, splines, random effects, tensor
  products, spatial fields), and estimators (MCMC, backfitting, gradient boosting, lasso, and more).
...
to facilitate addressing all challgenges and needs simultaneously independent of a specific 
estimation strategy and fitting algorithms, the \pkg{bamlss} package for the \proglang{R}
...
but also the estimation algorithm such as a specific MCMC sampler.
...
\hline
\end{tabular}
\caption{\label{tab:DIC} DIC, WAIC and CRPS comparison of the 6 distributional cyclical
  week models. For the CRPS, 10-fold cross validation was used.}
\end{table}
...
relationship for all parameters, however, 95\% pointwise credible intervals for parameter $\nu$
