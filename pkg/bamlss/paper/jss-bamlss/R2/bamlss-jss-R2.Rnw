...
mortality of the COVID-19 pandemic \citep{bamlss:Leon:2020}.
...
In addition to the DIC, there is also the continuous rank probability score (CRPS) to investigate 
the goodness of fit of the distribution. The \proglang{R} package \pkg{scoringRules} 
...
approximation as implemented in the function \fct{CRPS} in the \pkg{bamlss} package
<<>>=
CRPS(b)
@

The values of the DIC and CRPS for the 6 models are given in Table~\ref{tab:DIC}.
\begin{table}[h!]
\centering
\begin{tabular}{lcc}
\hline
Family & DIC & CRPS\\ 
\hline
\fct{NO}   & 12291.75 & 51.28 \\
\fct{GA}   & 12261.06 & 51.11 \\
\fct{GG}   & 12248.10 & 51.03 \\
\fct{BCT}  & 12215.85 & 50.69 \\
\fct{BCCG} & 12190.41 & 50.67 \\
\fct{BCPE} & \textbf{12188.30} & \textbf{50.61} \\
\hline
\end{tabular}
\caption{\label{tab:DIC} DIC comparison of the 6 distributional cyclical week models.}
\end{table}
Here, the four parameter \fct{BCPE} model has the lowest DIC and CRPS, therefore, we
...
global climate \citep{era5a, era5b}. ERA5 provides a globally complete and consistent
...
FlashAustriaModel <- b <- flash_model_ztBNB
...
this will increase our knowledge about physical proceses leading to such
...
\end{table}
%
The parameters are taken directly or are derived from ERA5. For this study  only a
...
event that also lead to lightning discharges
...
\code{ztnbinom_bamlss()} within \pkg{bamlss}.  Implementations of the Sichel
\code{SI()} and the beta negative binomial \code{BNB()} are available within
\pkg{gamlss.dist}.  Using \code{trun()} from \pkg{gamlss.tr} zero-truncated
versions of the two GAMLSS families can be generated that can instantaneously be
plugged into the \code{family} argument of the \code{bamlss()} model call.
...
the zero-truncated beta negative binomial.
...
\code{ztBNB}, and a data set \code{FlashAustriaTrain}. Within the
...
\begin{Sinput}
R> set.seed(123)
R> flash_model_ztBNB <- bamlss(f, family = ztBNB, data = FlashAustriaTrain,
+    optimizer = opt_boost, maxit = 1000,     ## Boosting arguments.
+    thin = 5, burnin = 1000, n.iter = 6000)  ## Sampler arguments.
\end{Sinput}
\begin{Soutput}
TODO: Output from Goettingen run?!
logLik -XXXXX.0 eps 0.0003 iteration 1000 qsel 7
elapsed time:  X.XXmin
Starting the sampler...
|********************| 100%  0.00sec XX.XXmin
\end{Soutput}
\end{Schunk}
%
\fixme{The model was fitted on a single core Intel i7-7700 CPU with 3.60GHz and 16~GB memory,
...
negative binomial \code{flash_model_ztnbinom} and zero-truncated Sichel \code{flash_model_ztSI}.
To select one of the three models, we examine their calibration using a worm plot
\citep{bamlss:Buuren+Miranda:2001}. The worm plot is implemnted within \pkg{bamlss}
...
resids <- c(
  ...
  "ztSI"     = residuals(flash_model_ztSI),
  "ztBNB"    = residuals(flash_model_ztBNB)
)
plot(resids, which = "wp", main = "In sample worm plots") 
@
...
resids <- c(
	...
	"ztSI"     = residuals(flash_model_ztSI    , newdata = FlashAustriaTrain),
	"ztBNB"    = residuals(flash_model_ztBNB   , newdata = FlashAustriaTrain)
)
plot(resids, which = "wp", main = "In sample worm plots", col = hcl.colors(3, "Dark 3")) 
@
\caption{Worm plot of the three distributional models.}
\label{fig:appworm}
\end{figure}
%
Though, the worm plots (Figure~\ref{fig:appworm}) reveal different tail behavior
of the distributions---\code{ztSI} and \code{ztnbinom} underestimate the mass of
the tail and \code{ztBNB} slightly overestimates the tail---the distributional model
based on the zero-truncated beta negative binomial exhibits the best overall calibration.
...
%
<<eval=FALSE>>=
pathplot(flash_model_ztBNB, which = "loglik.contrib")
@
%
\setkeys{Gin}{width=.7\textwidth}
\begin{figure}[t!]
...
at_missing <- flash_model_ztBNB$model.stats$optimizer$boost_summary$loglik[1000, missing]
...
\end{figure}
%
After 1000~iterations the term \code{s(sqrt_cape).nu} has the highest contribution
to the log-likelihood with %
\Sexpr{round(FlashAustriaModel$model.stats$optimizer$boost_summary$summary$nu["s(sqrt_cape)",2])} %
followed by \code{s(q_prof_PC1).nu} with %
\Sexpr{round(FlashAustriaModel$model.stats$optimizer$boost_summary$summary$nu["s(q_prof_PC1)",2])}.
These two terms as well as all other terms with a strong contribution
act on the distributional parameter \code{nu}. The contributions for the terms of the
other distributional parameters seems negligible.
...
@
%
\fixme{Interpret samples!} Figure~\ref{fig:apptrace} shows the traces and the auto-correlation functions
...
distributional parameter $\nu$, i.e., the log scale. A smaller
$\log(\nu)$ would result in a higher variance of the count data distribution
and thus a heavier tail.
%
<<fig=FALSE, eval=FALSE>>=
plot(flash_model_ztBNB,
  model = "nu",
  term  = c("s(sqrt_cape)", "s(q_prof_PC1)")
)
@
%
\begin{figure}[t!]
...
## FIXME: plot are vertically displayed, not horizontally !!!
par(mfrow = c(1, 2))
plot(b, model = "nu", term = c("s(sqrt_cape)", "s(q_prof_PC1)"),
...
from the model \code{nu}. Credible intervals derived from MCMC samples.}
\label{fig:appeffect}
\end{figure}
%
\code{s(sqrt_cape)} reveals a monotonic---basically linear---decreasing shape.
This means a higher \textit{convective available potential energy} (CAPE)
decreases $\log(\nu)$, which increases the dispersion of the distribution and
...
\code{s(q_prof_PC1)} shows a clear linear increase.
...
which a source of \emph{latent energy}, energy that becomes free when water
...
# plot(FlashAustriaCase["P10"]) ## FIXME!
