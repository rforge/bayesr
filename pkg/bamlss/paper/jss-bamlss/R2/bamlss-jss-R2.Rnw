...
dic <- dic[dic$dic1 != 0, ]
dic <- dic[dic$dic2 != 0, ]
dic <- dic[order(dic$dic2), ]

dn <- as.character(dic$distribution)[1:10]
dnum <- list()
...
legend("topright", paste0(dn, ": ", round(dic$dic2[1:10], 2)),
...
if(!file.exists("FatModel.rda")) {
  set.seed(456)
  b <- bamlss(f, data = d19, family = ST3C(mu.link = "log"),
    optimizer = opt_boost, sampler = sam_GMCMC,
    n.iter = 2000000, burnin = 100000, thin = 1900)
...
extreme short-term events under the light of a changing climate
\citep{westra2014}.
