...
if(!file.exists("Fat1.rda")) {
  ...
  for(j in names(families)) {
    ...
    b <- try(bamlss(num ~ 1, data = fatalities,
      ...
      optimizer = opt_boost, init = FALSE),
      silent = TRUE)

    if(!inherits(b, "try-error")) {
      par <- predict(b, type = "parameter", drop = FALSE)
      dic[[j]] <- DIC(b)
      cat(".. .. DIC =", dic[[j]]$DIC, "pd =", dic[[j]]$pd, "\n")
  ...
  save(dic, dnum, elapsed, file = "Fat1.rda")
} else {
  load("Fat1.rda")
}
dic <- sapply(dic, function(x) { x$DIC })
dnum <- do.call("cbind", dnum)
i <- order(dic, decreasing = FALSE)
dnum <- dnum[, i][, 1:10]
dic <- dic[i][1:10]
...
legend("topright", paste0(names(dic), ": ", round(dic, 2)),
