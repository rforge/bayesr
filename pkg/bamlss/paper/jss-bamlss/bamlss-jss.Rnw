...
\Abstract{
  ...
  been changing considerably: (1)~More flexible model specifications are needed as big(ger) data as
  well as more powerful computing infrastructure become available. (2)~Full probabilistic modeling
  ...
  scale, and shape. At the core of the package are algorithms for highly-efficient Bayesian
  ...
  regression terms (linear, splines, random effects, tensor products, \dots), and
  estimators (MCMC, backfitting, gradient boosting, lasso, \dots).
...
methods are becoming increasingly complex. Following the recent discussion on p-values
by the American Statistical Association \citep{bamlss:ASA:2016}, models estimated using a Bayesian
framework become will become more and more important, hence, further driving model complexity.
This is not an easy problem to solve and sometimes requires a flexible development environment to
avoid reinventing the wheel. Although some implementations for regression modeling seem to be quite
flexible in the first place, the possibilities are usually limited, e.g., concerning models and
...
maintainers to do so. Another quite flexible Bayesian implementation is the \proglang{R} package
\pkg{INLA} \citep{bamlss:Rue+Martino+Chopin:2009, bamlss:INLA} which uses integrated nested Laplace
approximations to compute the high dimensional integrals, e.g., for posterior mean estimation.
The \pkg{INLA} package infrastructures are in principle flexible, but new models or
features can usually only be implemented by the package authors, too. Recently, the \proglang{R}
package \pkg{brms} \citep{bamlss:Buerkner:2017, bamlss:Buerkner:2018}
