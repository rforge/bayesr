...
since the 95\% credible interval contains zero. Before proceeding the analysis, users usually perform
...
In addition, samples can also be extracted using function \fct{samples}, which returns an object
of class \code{"mcmc"}, a class provided by the \pkg{coda} package
\citep{bamlss:Plummer+Best+Cowles+Vines:2006}.
This package includes a rich infrastructure for further convergence diagnostic checks, e.g.,
Gelman and Rubin's convergence diagnostic \citep{bamlss:Gelman+Rubin:1992, bamlss:Brooks+Gelman:1998} or
