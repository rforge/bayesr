...
\subsection{Normal location-scale model} \label{sec:locscalemodel}
...
Gompertz curve
$$
f(x; \boldsymbol{\beta}) = \beta_1 * \exp(-\beta_2 * \exp(-\beta_3 \cdot x)),
$$
...
the regression coefficients $\boldsymbol{\beta}$ a number of times. For full Bayesian inference
prior distributions $p_{jk}( \cdot )$ are assigned to the individual model components yielding the
log-posterior, which needs to maximized for posterior mode estimation, or for solving
...
\item A unified model description where a \code{formula} specifies how to set up the predictors
  from the \code{data} and \code{family} objects, which provide information about the response
  distribution.
\item A generic method for setting up model terms and a \fct{model.frame} for BAMLSS, the
  \fct{bamlss.frame}, along with the corresponding prior structures. A \fct{transformer} can
...
\begin{tikzpicture}[node distance=1.5cm,
  ...
  \node (trans)  [processing, below of=bframe, yshift=0.3cm] {\texttt{transformer()}};
...
\caption{\label{fig:flowchart} Flow chart of the \pkg{bamlss} modeling architecture. Thick gray
  ...
  function directly.}
\end{figure}
The modular architecture of the \pkg{bamlss} is illustrated in Figure~\ref{fig:flowchart}. As
...
quite long or even lead to memory problems. In summary, the architecture is very flexible
such that users that are interested in implementing new models only need to focus on the
estimation step, i.e., write \fct{optimizer} or \fct{sampler} functions. This way, prototyping
becomes relatively easy, but also the integration of high-performance estimation engines.
