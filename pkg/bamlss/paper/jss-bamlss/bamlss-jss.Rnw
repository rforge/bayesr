...
lasso.plot(b, 2, spar = FALSE,
  name = c("pi.s.la(youngkids).youngkids", "pi.s.la(oldkids).oldkids", "pi.s.la(foreign).foreign"), main = "kids & foreign")
...
\code{income} and \code{education} are shown. Using the fused LASSO estimation some nonlinearities
