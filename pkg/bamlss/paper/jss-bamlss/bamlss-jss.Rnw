...
model is estimated the number of iterations of the MCMC sampler etc.\ is usually set much higher.
...
functions approach. Moreover, using \pkg{bamlss}, estimation engines can also be exchanged. To give
...
page <- predict(b, term = "cage", intercept = FALSE,
  mstop = lasso_stop(b))
@
The figure is then created using the untransformed original covariate
<<eval = FALSE>>=
