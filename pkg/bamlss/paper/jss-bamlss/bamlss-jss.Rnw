...
\Shorttitle{A Lego Toolbox for flexible Bayesian Regression (and Beyond)}
...
based on MCMC simulation, backfitting algorithms, gradient boosting and LASSO-type penalized models.
}
...
\item easy implementation of (new) models, model terms and algorithms.
\end{itemize}
%
...
process, users usually want to obtain first results rather quickly and most commonly, if a final
model is estimated, then the number of iterations of the MCMC sampler etc.\ is set much higher.
...
if(!file.exists("logit_spline.rda")) {
  set.seed(123)
  b <- bamlss(f, family = "binomial", data = SwissLabor,
    n.iter = 12000, burnin = 2000, thin = 10)
  save(b, file = "logit_spline.rda")
} else {
  load("logit_spline.rda")
}
...
\subsection{Flexible model terms} \label{sec:flexm}
...
plot(b, term = "s(age)")
@
The estimated effect based on regression splines is shown if Figure~\ref{fig:logit_effects} and
...
Nonlinear effects of continuous covariates $f(\mathbf{x}) = f(x_1)$ & \code{~ s(x1)} \\
Two-dimensional surfaces\newline $f(\mathbf{x}) = f(x_1, x_2)$ & \code{~ s(x1,x2)}, \code{~ te(x1,x2)} or \code{~ ti(x1,x2)} (higher dimensional terms possible) \\
Spatially correlated effects\newline $f(\mathbf{x}) = f_{spat}(x_s)$ & \code{~ s(xs,bs="mrf",xt=list(penalty=K))}, where \code{xs} is a factor indicating the discrete regional information and \code{K} is a supplied penalty matrix. Other options within the \code{xt} argument are possible, please see the documentation of \code{smooth.construct.mrf.smooth.spec()} \\
Varying coefficients $f(\mathbf{x}) = x_1f(x_2)$ & \code{~ s(x2,by=x1)} \\
Spatially varying effects\newline $f(\mathbf{x}) = x_1f_{spat}(x_s)$ or\newline $f(\mathbf{x}) = x_1f(x_2, x_3)$ & \code{~ s(xs,bs="mrf",xt=list(penalty=K),by=x1)}, \code{~ s(x2,x3,by=x1)} or \code{~ te(x2,x3,by=x1)} \\
Random intercepts with cluster index $c$: $f(\mathbf{x}) = \beta_c$ & \code{~ s(id,bs="re")}, where \code{id} is a factor of cluster indices \\
...
<<echo = FALSE>>=
SwissLabor$cincome <- cut(SwissLabor$income,
...
SwissLabor$ceducation <- with(SwissLabor, cut(education,
...
if(!file.exists("LassoSL.rda")) {
  f <- participation ~ la(cincome,fuse=2) + la(cage,fuse=2) + la(ceducation,fuse=2) +
...
<<>>=
f <- participation ~ la(cincome,fuse=2) + la(cage,fuse=2) +
  la(ceducation,fuse=2) + la(youngkids,fuse=2) + la(oldkids,fuse=2) +
  la(foreign,fuse=2)
@
...
par(mar = c(4.1, 4.1, 4.1, 1.1))
lwd <- 2
lasso_plot(b, 1, spar = FALSE, main = "criterion", lwd = lwd)
@
<<echo = FALSE, fig = TRUE, width = 4, height = 5, results = hide>>=
par(mar = c(4.1, 4.1, 4.1, 4.1))
lasso_plot(b, 2, spar = FALSE, name = "pi.s.la(cincome).cincome", main = "income", lwd = lwd)
@
<<echo = FALSE, fig = TRUE, width = 4, height = 5, results = hide>>=
par(mar = c(4.1, 4.1, 4.1, 4.1))
lasso_plot(b, 2, spar = FALSE, name = "pi.s.la(cage).cage", main = "age", lwd = lwd)
@
\newline
<<echo = FALSE, fig = TRUE, width = 4, height = 5, results = hide>>=
par(mar = c(4.1, 4.1, 4.1, 4.1))
lasso_plot(b, 2, spar = FALSE, name = "pi.s.la(ceducation).ceducation", main = "education", lwd = lwd)
@
<<echo = FALSE, fig = TRUE, width = 4, height = 5, results = hide>>=
par(mar = c(4.1, 4.1, 4.1, 4.1))
lasso_plot(b, 2, spar = FALSE,
...
pincome <- predict(b, term = "cincome", intercept = FALSE, mstop = lasso_stop(b))
page <- predict(b, term = "cage", intercept = FALSE, mstop = lasso_stop(b))
peducation <- predict(b, term = "ceducation", intercept = FALSE, mstop = lasso_stop(b))
...
boost_plot(b, which = "loglik.contrib", intercept = FALSE)
@
...
boost_plot(b, c("loglik.contrib"), intercept = FALSE)
