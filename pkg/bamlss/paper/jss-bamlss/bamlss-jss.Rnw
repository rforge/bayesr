...
\Abstract{
lego+flexibel, weil ich alles austauschen kann, Terme, Schätzer, Verteilungen

2.1 Basic Bayesian regression: Logit Model
2.2 Flexible terms and estimators -> spline + fused LASSO + Boosting hier auf den Appendix verweisen,
  die Terme die wir hier haben gibts schon, aber man kann auch wietere Terme hinzufügen,
  es ist sicher besonders wie einfach und flexibel man diese Terme im Bayes Kontext austauschen kann.

3. A flexible Bayesian model framework/ theoretical background. Wir schreiben das jetzt im
  distreg framework hin, es sind aber auch einfachere Fälle im Spezialfall enthalten.

cut(order.results = TRUE)

pathplot()
lasso_stop()
Immer underscore nehmen!

Families mappen!
Fig. -> Figure

Gerade beim 2. Teil hat sich gegenüber JCGS viel getan.
