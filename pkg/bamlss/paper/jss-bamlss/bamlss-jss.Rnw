...
\subsection{LASSO-type penalization}
...
SwissLabor$c.education <- cut(SwissLabor$education,
  breaks = quantile(SwissLabor$education, prob = seq(0, 1, length = 5)),
  include.lowest = TRUE)
...
for estimation.
The algorithm performs variable selection in combination with factor clustering and can also be used
to identify interpretable nonlinearities. Methodological details on LASSO-type penalization using
\pkg{bamlss} are provided in \citet{bamlss:Groll+Hambuckers+Kneib+Umlauf:2019}.
...
special \fct{la} smooth constructor function
<<>>=
...
b <- bamlss(f, family = "binomial", data = SwissLabor,
  ...
  criterion = "BIC")
@
Note the no MCMC sampling is used after the \fct{lasso} estimation engine is applied. Additionally,
...
lasso.plot(b)
@
and are shown in Figure~\ref{fig:lasso} for variables \code{age}, \code{youngkids} and
\code{oldkids}.
\begin{figure}[!ht]
\centering
...
lasso.plot(b, 2, spar = FALSE,
  name = c("pi.s.la(youngkids).youngkids", "pi.s.la(oldkids).oldkids"), main = "kids")
legend("bottomleft", c("younkids", "oldkids"), lwd = 1, col = rev(rainbow_hcl(2)),
  box.col = NA, bg = NA)
@
\caption{\label{fig:lasso} BIC curve with optimum shrinkage parameter $\lambda$ of the LASSO
  example model, left panel. Coefficient paths for categorized variable \code{age}, middle panel.
  The right panel shows the coefficient paths of the ordered factor variables \code{youngkids} and
  \code{oldkids}.}
\end{figure}
The BIC curves shows a clear minimum. The coefficient paths clearly depict that the algorithm
...
Note again, that no sampling engine is used in this example. However, using the results of the
fused LASSO model, the fully Bayesian LASSO model can be estimated with full MCMC using the
estimates returned from the \fct{lasso} engine as starting values.
<<eval = FALSE>>=
m <- bamlss(f, family = "binomial", data = SwissLabor,
  optimizer = FALSE, sampler = GMCMC,
  n.iter = 12000, burnin = 2000, thin = 10,
...
page <- predict(m, term = "c.age", intercept = FALSE, FUN = c95)
plot2d(page ~ age, data = SwissLabor, fill.select = c(0, 1, 0, 1), scheme = 2)
@
<<echo = FALSE, fig = TRUE, width = 4, height = 4.3, results = hide>>=
pincome <- predict(m, term = "c.income", intercept = FALSE, FUN = c95)
plot2d(pincome ~ income, data = SwissLabor, fill.select = c(0, 1, 0, 1), scheme = 2)
@
<<echo = FALSE, fig = TRUE, width = 4, height = 4.3, results = hide>>=
peducation <- predict(m, term = "c.education", intercept = FALSE, FUN = c95)
plot2d(peducation ~ education, data = SwissLabor, fill.select = c(0, 1, 0, 1), scheme = 2)
@
\caption{\label{fig:lasso_effects} Estimated nonlinear effects of categorized variables
  \code{age}, \code{income} and \code{education}.}
\end{figure}
In Figure~\ref{fig:lasso_effects}, the estimated effects of the categorized variables \code{age}, 
\code{income} and \code{education} are shown. Using the fused LASSO estimation engine in combination
with full MCMC, some nonlinearities can be identified, moreover, compared to the insignificant effect
of variable \code{education} in the summary output of Section~\ref{sec:logitmodel} on
page~\pageref{logitmodel_summary}, the fused LASSO identifies a clear positive effect on labor
\code{participation} for years of education above 12.
