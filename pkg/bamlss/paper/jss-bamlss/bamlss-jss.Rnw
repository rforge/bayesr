...
\Keywords{GAMLSS, distributional regression, backfitting, gradient boosting, LASSO, MCMC, \proglang{R}}
\Plainkeywords{GAMLSS, distributional regression, backfitting, gradient boosting, LASSO, MCMC, R}

\Abstract{
  Over the last decades, there has been an increasing interest in flexible regression models.
  There are several reasons for this development: On the one hand, the number of data sets is
  constantly increasing and thus the number of problems as well as their complexity, on the other hand,
  infrastructures that can deal with ever larger data sets are needed, so that efficient estimation
  algorithms gain more and more importance. Particularly in this environment, models for full
  probabilistic forecasting, that allow to model the entire data distribution conditional on
  covariates, are necessary in a variety of applications. However, the implementation of new models
  is usually time-consuming and complex, especially using Bayesian estimation algorithms. We propose
  an unified modeling architecture, which is implemented in the \proglang{R} package \pkg{bamlss},
  that makes it possible to embed many different approaches suggested in literature and software. We
  show that implementing (new) algorithms, non-standard distributions or effect types, or the
  integration of already existing software, is relatively straightforward in this setting. We
  illustrate the usefulness of the approach by implementing highly efficient algorithms for fully
  Bayesian inference based on MCMC simulation, backfitting algorithms, gradient boosting and
  LASSO-type penalized models.
}
...
%  Over the last decades there has been an increasing interest in distributional
%  regression  models that allow to model the entire data distribution
%  conditional on covariates. In particular, the  framework of structured
%  additive distributional regression models enables to specify  different types
%  of effects such as linear, non-linear or interaction effects on all the 
%  distribution parameters (such as location scale and shape) hence providing a
%  very flexible and generic framework suited for  many complex real data
%  problems. However, the implementation of new models is usually time-consuming
%  and complex, especially using Bayesian estimation algorithms. We propose an
%  unified modeling architecture, which is implemented in the \proglang{R}
%  package \pkg{bamlss}, that makes it possible to embed many different
%  approaches suggested in literature and software. We show that implementing
%  (new) algorithms, non-standard distributions or effect types, or the
%  integration of already existing software, is relatively straightforward in
%  this setting. We illustrate the usefulness of the approach by implementing
%  highly efficient algorithms for fully Bayesian inference based on MCMC
%  simulation, backfitting algorithms, gradient boosting and LASSO-type penalized
%  models.

Nowadays, regression models must be able to capture a variety of different dependencies in order to
provide reliable predictions and inference. Moreover, the great interest in models for full
probabilistic forecasting that alleviate the risk of misspecification, means that the underlying
methods are becoming increasingly complex. This is not an easy problem to solve and sometimes
requires a flexible development environment to avoid reinventing the wheel. Although some
implementations for regression modeling seem to be quite flexible in the first place, the
possibilities are usually limited, e.g., concerning models and estimation algorithms.

Probably the first implementations that allow for some flexibility concerning the choice of response
distribution where developed for generalized linear models
(GLM,~\citealp{bamlss:Nelder+Wedderburn:1972}). The next extension of GLMs was to
allow for different (user specified) smooth model terms within the framework of generalized additive
models (GAM,~\citealp{bamlss:Hastie+Tibshirani:1990}). Further development was the extension away
from the pure exponential family of distributions to the so called generalized additive models for
location, scale and shape (GAMLSS,~\citealp{bamlss:Rigby+Stasinopoulos:2005}), also known as
distributional regression models
\citep{bamlss:Klein+Kneib+Lang+Sohn:2015, bamlss:Klein+Kneib+Klasen+Lang:2015}, which enable
modeling all parameters of a response distribution in terms of covariates. Besides, there are also
some implementations for nonparametric quantile regression models \citep{bamlss:Koenker:2005}, but
with rather limited support for flexible model terms.

The statistical environment \proglang{R} \citep{bamlss:R} provides a rich number of packages
that support a quite flexible regression modeling approach.
...
maintainers to do so. Recently, the \proglang{R} package \pkg{brms}
\citep{bamlss:Buerkner:2017, bamlss:Buerkner:2018}
provides infrastructures for estimating Bayesian multilevel models using \pkg{Stan}. The software
supports a variety of models and is also capable to estimate distributional regression models.
However, as already mentioned above, estimation time is usually quite long and \pkg{Stan}
does not easily handle models with a lot of observations.

To summarize the above: although supposedly quite flexible, most implementations follow a very
specific focus and are therefore not easy to use to implement new methods, e.g., a new efficient
sampling scheme or numerical optimization algorithm. Therefore, we present
the \proglang{R} package \pkg{bamlss}, a ``Lego toolbox'' for flexible Bayesian regression models
(and beyond), which was first introduced in \citet{bamlss:Umlauf+Klein+Zeileis:2018} within
the framework of Bayesian additive model for location, scale and shape (BAMLSS).
Since then, the package has undergone significant further development. The focus has shifted
tremendously from the pure Bayesian viewpoint to virtually a mixture of state-of-the-art estimation
techniques from machine learning and statistics. The main contributions of \pkg{bamlss} are the
following:
%
\begin{itemize}
\item With the usual \proglang{R} modeling ``look \& feel'',
\item estimation of classic (GAM-type) regression models (Bayesian or frequentist),
\item estimation of flexible distributional regression models,
\item easy interchangeability of model terms.
...
b <- bamlss(f, family = "binomial", data = SwissLabor,
  n.iter = 12000, burnin = 2000, thin = 10)
@
using 12000 iterations with a burnin-phase of 2000 and a thinning parameter of 10. Note that
the defaults are 1200 iterations, burnin of 200 and thinning 1. The reason is that during the modeling
process, users usually want to obtain first results rather quickly. Afterwards, if a final
model is estimated the number of iterations of the MCMC sampler is usually set much higher to
get close to i.i.d.\ samples from the posterior distribution.
...
plot2d(pSwiss ~ age, data = nd, ylab = "participation",
  ylim = range(c(nd$pSwiss, nd$pForeign)), lty = c(2, 1, 2))
...
Linear effects: $\mathbf{X}\boldsymbol{\beta}$ & \code{x1 + x2 + x3} \\
...
SwissLabor$cage <- cut(SwissLabor$age,
  ...
  include.lowest = TRUE, order.results = TRUE)
...
lwd <- 2
pathplot(b, 1, spar = FALSE, main = "Criterion", lwd = lwd)
@
\setkeys{Gin}{width=0.36\textwidth}
<<echo=FALSE, fig=TRUE, width=4.5, height=5, results=hide>>=
par(mar = c(4.1, 4.1, 4.1, 7.3))
pathplot(b, 2, spar = FALSE, name = "pi.s.la(cage).cage", main = "Coefficient paths age", lwd = lwd)
@
...
pathplot(b)
@
and are shown in Figure~\ref{fig:lasso}. The BIC curve shows a clear minimum at the vertical
...
As shown in Section~\ref{appendix:specialterms} the \fct{bamlss.frame} function can also
...
pathplot(b, c("loglik.contrib"), intercept = FALSE, spar = FALSE)
# layout(matrix(c(rep(1,2), rep(2,3)), nrow = 1))
# pathplot(b, "loglik")
# pathplot(b, "loglik.contrib")
@
\end{center}
