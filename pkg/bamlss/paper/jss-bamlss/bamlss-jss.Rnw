...
(GAMLSS,~\citealp{bamlss:Rigby+Stasinopoulos:2005}), also called distributional regression models
...
implementing response distributions. Variable selection algorithms based on gradient boosting
were then introduced in the \pkg{gamboostLSS} package
...
The inferential frameowrk of the packages mentioned is based on maximum likelihood, if taken into
account at all.
