...
covariates. While most classical model specifications -- such as linear models, generalized linear models (GLMs),
generalized additive models (GAMs), etc. -- focus on characterization of the mean of the distribution
and ignore changes in other moments, GAMLSS has the clear advantage to alleviate the risk of
model misspecification and therefore provides much more reliable probabilistic forecasts.

Besides the increasing interest in distributional regression, nowadays, regression models are
becoming more and more complex. The reasons are commonly two-fold: because of the complexity of
the problems or because more and more data is available. This is not an easy problem to solve and
sometimes requires a flexible development environment to avoid reinventing the wheel. However,
although some implementations for regression modeling seem to be quite flexible in the first place,
the possibilities are usually limited, e.g., concerning models and estimation algorithms.
...
In this paper, we present the \proglang{R} package \pkg{bamlss}, a ``Lego toolbox''
for flexible Bayesian regression models (and beyond), which was first introduced in
\citet{bamlss:Umlauf+Klein+Zeileis:2018}. Since then, the package has undergone significant further
development. The focus has shifted tremendously from the pure Bayesian viewpoint to virtually a
mixture of state-of-the-art estimation techniques from machine learning and statistics. The main
contributions of \pkg{bamlss} are the following:
%
\begin{itemize}
