...
\Abstract{
  ...
  regression terms (linear, splines, random effects, tensor products, spatial fields, \dots), and
...
(1)~Big data sets that can be both long (many observations) and wide (many variables and/or complex effect types).
...
as proposed in \citep{bamlss:Umlauf+Klein+Zeileis:2018}. In this framework
not only the response distribution or the regression terms are ``Lego bricks''
but also estimation algorithm or the MCMC sampler.
...
models and for penalizing regression effects through suitable prior distributions. Fully
Bayesian approaches using MCMC are particularly attractive for flexible regression models
in order to obtain credible intervals from the posterior samples etc. The \pkg{brms} package
\citep{bamlss:Buerkner:2017} is notable for providing a standard \proglang{R} workflow
for estimating Bayesian multilevel models using \pkg{Stan} \citep{bamlss:stan-software:2016}.
...
However, for complex models -- e.g., using data sets, spatial effects, or higher-order interactions --
sampling times from these generic MCMC engines can become long, sometimes prohibitively so.
This has been addressed by dedicated packages for Bayesian additive models with the standalone
...
interface is available in \pkg{R2BayesX} \citep{bamlss:Umlauf:2015}. Instead of doing
fully Bayesian MCMC another approach to estimate flexible Bayesian regression models is
posterior mean estimation via the integrated nested Laplace approximations. This is provided
...
estimation techniques and fitting algorithms. The package {bamlss}, available from the Comprehensive
\proglang{R} Archive Network at \url{https://CRAN.R-project.org/package=bamlss}, tries to 
fill this gap and provide a modular ``Lego'' approach to flexible Bayesian regression providing:
%
\begin{itemize}
...
alternative optimizers and samplers are also implemented that support Bayesian lasso
or boosting etc. Moreover, the package builds on the well-established \pkg{mgcv} infrastructure
