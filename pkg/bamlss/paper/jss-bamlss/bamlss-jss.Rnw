...
The \proglang{R} package \pkg{bamlss} provides a modular computational framework for
...
\item A unified model description where a \code{formula} specifies how to set up the predictors
  from the \code{data} and \code{family} objects provide information about the response distribution.
\item A generic method for setting up model terms and a \fct{model.frame}, the \fct{bamlss.frame},
  along with the  corresponding prior structures. A \fct{transformer} can optionally set up
  modified terms, e.g., using mixed model representation for smooth terms.
...
\begin{tikzpicture}[node distance=1.5cm,
  ...
  \node (text) [right of=B, xshift=-7.2cm] {Preprocessing};
  \end{pgfonlayer}
  ...
  \node (text) [right of=D, xshift=-7.25cm] {Postprocessing};
  \end{pgfonlayer}
