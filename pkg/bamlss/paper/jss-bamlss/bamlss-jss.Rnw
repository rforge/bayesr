...
estimation \citep{bamlss:Wood:2019} (see also, e.g., the documentation of function \fct{s} and
\fct{te}).
...
\end{Code}
Argument \code{formula} can be a classical model formulae, e.g., as used by the \fct{lm()} function,
or an extended \pkg{bamlss} formula, that is internally parsed by function \fct{bamlss.formula}.
...
The returned object is a named list of class \code{"bamlss.frame"}. The most important elements
used for estimation are:
\begin{itemize}
