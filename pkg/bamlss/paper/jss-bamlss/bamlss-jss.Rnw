...
cases the flexibility of the provided infrastructures.
...
summary(b)
@
and is based on MCMC samples, which suggest ``significant'' effects for all covariates, except for variable \code{education},
since the 95\% credible interval contains zero. The column \code{parameters} shows
respective posterior mode estimates that are calculated by the upstream backfitting
algorithm. Before proceeding the analysis, users usually perform
convergence checks of the MCMC chains by looking at traceplots and auto-correlation.
<<eval = FALSE>>=
...
\end{figure}
These are visualized in Figure~\ref{fig:logit_traceplots} and indicate convergence of
the MCMC chains, i.e., there is no visible trend in the MCMC chains and the very low
...
Note that the function call would show all trace- and auto-correlation plots, however,
...
parameter of the response distribution. Also note that all smooth terms, i.e.,
\fct{te}, \fct{ti()}, etc., are supported by \pkg{bamlss}. This way, it is also possible to
incorporate user defined model terms. A full Bayesian semi-parametric distributional regression
...
\caption{\label{fig:toy_effects} Estimated effects of \code{times} on parameter
  $\mu$ and $\sigma$ of the normal location-scale model.}
\end{figure}
...
residuals \citep{bamlss:Dunn:Smyth:1996} using histograms or quantile-quantile plots. Residuals
can be extracted using function \fct{residuals} and has a plotting method. Alternatively,
residuals can be investigated with
<<eval = FALSE>>=
...
\centering
\setkeys{Gin}{width=0.48\textwidth}
<<echo = FALSE, fig = TRUE, width = 4.3, height = 4.5>>=
