...
regression models with GAM-type predictors, e.g., when using large data sets or modeling complex
higher-order interactions. The first software capable of estimating such GAM-type distributional
regression models using MCMC is the standalone package
\pkg{BayesX}~\citep{bamlss:Belitz+Brezger+Kneib+Lang+Umlauf:2017}, which provides highly
efficient sampling schemes for very large data sets as well as multilevel models. However,
the implementation of new response distributions in \pkg{BayesX} is not straightforward and users
