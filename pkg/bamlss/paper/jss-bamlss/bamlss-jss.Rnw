...
pathplot(b)
@
and are shown in Figure~\ref{fig:lasso} (note that only the coefficient paths for
variable \code{cage} are shown). The BIC curve shows a clear minimum at the vertical
