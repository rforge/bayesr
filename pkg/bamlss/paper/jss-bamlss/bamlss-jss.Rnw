...
%
\begin{itemize}
\item Look \& feel of well-established model fitting functions like \fct{glm},
...
<<echo = FALSE, fig = TRUE, height = 2.7, width = 8>>=
...
Only for very low and very high values of \code{accel} the fitted distributions seems
to be less appropriate.

Besides residuals, users can evaluate the model performance, e.g., based on the deviance
information criterion (DIC), which can be extracted using function \fct{DIC}
<<>>=
DIC(b)
@
and is also reported in the model summary output. Furthermore, statistical calibration can
be assessed by scoring rules \citep{bamlss:Gneiting+Balabdaoui+Raftery:2007}. For example, 
