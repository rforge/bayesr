...
The returned object can be used with the model fitting engines listed in Table~\ref{tab:functions} and
is a named list of class \code{"bamlss.frame"}. The most important elements used for estimation are:
\begin{itemize}
...
@
In this example, the list contains of three smooth term objects for parameter \code{mu} and
\code{sigma}.
...
However, the most important distributions are shipped with \pkg{bamlss}.
Finally, it is also possible to employ \pkg{gamlss}.
