...
smooth terms within bamlss formulae builds on the \pkg{mgcv} infrastructure
...
We set the number of iteration to $1000$. For the sampling we allow another $1000$
iterations as burn-in phase, and apply a thinning of the resulting chain of $5$.
Running \code{n.iter = 6000} iterations in total leads to $1000$ MCMC samples
in the end:
...
reveal samples atound stables means. This suggests that the 1000 boosting
...
Thus, looking at these effects good illustrate how the atmospheric parameters
...
log scale. \code{s(sqrt_cape)} \dots and \code{s(q_prof_PC1)} \dots
...
Another interesting effect to look at, is the one acting on the link scale of
