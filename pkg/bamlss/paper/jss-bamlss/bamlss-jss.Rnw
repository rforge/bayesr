...
\begin{figure}
\begin{center}
<<app-plot-effect1, fig = TRUE>>=
plot(b, model = "mu", term = "s(sqrt_cape)", rug = TRUE)
@
\end{center}
\caption{Effect of the term \code{s(sqrt\_cape)} from the model \code{mu}.
...
\begin{figure}
\begin{center}
<<app-plot-trace, fig = FALSE>>=
plot(b, model = "mu", term = "s(sqrt_cape)", which = "samples")
## How to display only one trace acf pair?
@
\end{center}
\caption{MCMC trace (left panel) and auto-correlation (right panel) for a
single spline from the term \code{s(sqrt\_cape)} of the model \code{mu}.}
\label{fig:apptrace}
\end{figure}
...
@
\end{center}
\caption{A probabilistic reconstruction of lightning counts occured on June 25
2007 between 18 and 19~UTC, i.e., the probability of having observed $10$ or
more counts within one gird box.}
\label{fig:appcase}
\end{figure}
