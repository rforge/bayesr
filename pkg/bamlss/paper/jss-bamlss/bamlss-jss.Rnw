...
In order to depict the generic and flexible modeling possibilities in distributional regression (or
general complex regression modeling), the \proglang{R} package \pkg{bamlss} attempts to follow
a unified architecture made of ``Lego bricks'', such that implementation of new
distributions and estimation algorithms is relatively straightforward. The architecture of the
package is outlined in Figure~\ref{fig:blocks}.
\begin{figure}[ht!]
\centering
\setlength{\unitlength}{1cm}
\setlength{\fboxsep}{0pt}
\begin{tikzpicture}
\node (formula) at (0,0) [draw,minimum height=0.7cm,rounded corners] { \texttt{formula} };
\node (family) at (2.4,0) [draw,minimum height=0.7cm,rounded corners] { \texttt{family} };
\node (data) at (4.5,0) [draw,minimum height=0.7cm,rounded corners] { \texttt{data} };
\node[draw,dashed,rounded corners,fit=(formula) (family) (data),inner xsep=10pt,inner ysep=10pt,label={left:{Inputs\,\,}}](A){};

\node (modelframe) at (0.4,-2) [draw,minimum height=0.7cm,rounded corners] { \texttt{model.frame} };
\node (transformer) at (3.8,-2) [draw,minimum height=0.7cm,rounded corners] { \texttt{transformer} };
\node[draw,dashed,rounded corners,fit=(modelframe) (transformer),inner xsep=10pt,inner ysep=10pt,label={left:{Model components\,\,}}](B){};

\node (optimizer) at (0.4,-4) [draw,minimum height=0.7cm,rounded corners] { \texttt{optimizer} };
\node (sampler) at (3.8,-4) [draw,minimum height=0.7cm,rounded corners] { \texttt{sampler} };
\end{tikzpicture}
