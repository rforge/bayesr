...
becoming more and more complex, because of the complexity of the problem, sometimes because more and
more data is available or because interactions have to be taken into account in many applications,
etc. This is not an easy problem to solve and sometimes requires a flexible development environment
to avoid reinventing the wheel. However, although some implementations for regression modeling seem
to be quite flexible in the first place, the possibilities are usually limited, e.g., concerning
...
\section{Flexible count regression for lightning prediction} \label{sec:application}
