...
In this section we would like to introduce the R package to the reader using two introductory
examples. Readers more interested in advanced topics using \pkg{bamlss} could skip this section
and move to Section~\ref{sec:application} directly. The first example demonstrates that the usual
look and feel when using well-established model fitting functions like \fct{glm} is an elementary
part of \pkg{bamlss}, i.e., first steps and basic handling should be relatively simple. The second
example then explains how full distributional regression models can be estimated.
...
b <- bamlss(f, family = "binomial", data = SwissLabor,
  n.iter = 12000, burnin = 2000, thin = 10)
@
using 12000 iterations with a burnin-phase of 2000 and a thinning parameter of 10. 
Note, to capture nonlinearities, a quadratic term for variable \code{age} is added to the model.
The resulting object \code{b} is of class \code{"bamlss"} for which standard extractor functions
like \fct{summary}, \fct{coef}, \fct{plot}, \fct{predict}, etc.\ are available.
The model summary output is printed by
<<>>=
summary(b)
@
...
\end{figure}
The estimates are shown in Figure~\ref{fig:logit_effects} and suggests a clear difference in the
effect of \code{age} between both cases of factor variable \code{foreign}.
