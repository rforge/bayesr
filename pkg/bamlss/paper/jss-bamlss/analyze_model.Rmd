...
st_crs(nd) <- st_crs(gadm)
xx <- st_difference(st_union(nd), gadm)

plot(nd["P10"], pal = pal, nbreaks = 100)
plot(st_geometry(gadm), border = "white", add = TRUE)
...
library("colorspace")
ggplot() +
  geom_sf(aes(fill = P10), data = nd) +
  scale_fill_continuous_sequential("Inferno", rev = TRUE) +
  theme_minimal() +
  geom_sf(data = xx, col = "white", fill = NA)
```

```{r dynmap, out.width = "100%", eval = TRUE}
library("mapview")
mapview(nd["P10"], col.regions = pal)
```
