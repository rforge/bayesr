...
In the
following commonly used methods used for model choice and variable selection based on the complete response distribution are outlined.
...
the smoothing variances $\boldsymbol{\tau}_{jk}$. For models using a basis function approach,
