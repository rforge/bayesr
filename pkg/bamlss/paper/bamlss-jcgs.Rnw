...
To give a brief insight how we use \pkg{bamlss} we illustrate the basic steps on a small toy
...
\fct{summary}, \fct{plot}, \fct{predict}, etc., are provided. E.g., predictions based on MCMC 
...
plot(b, model = "mu", term = "s(times)", spar = FALSE)
@
<<echo=FALSE, fig=TRUE, width=5, height=4>>=
par(mar = c(4.1, 4.1, 0.1, 1.1))
plot(b, model = "sigma", term = "s(times)", spar = FALSE)
@
