...
\end{equation}
where $\mathbf{X}_{jk}$ ($n \times m_{jk}$) is a design matrix \fixme{Achim, this is what the reviewer did not understand, any suggestions?} and the structure of $\mathbf{X}_{jk}$
