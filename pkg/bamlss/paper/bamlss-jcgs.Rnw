...
dunn  distributions, estimation techniques (posterior mode or posterior mean), and model terms
...
formats, which makes it difficult, e.g., to compare properties of
...
\citet{bamlss:Gelman:2006} proposes to use the half-Cauchy prior which has the desirable property
...
\item \emph{Derivative-based Metropolis-Hastings}: \label{sec:dmh}
  ...
  \citep{bamlss:Gamerman:1997, Klein+Kneib+Lang:2015}.
  The resulting proposal density is multivariate normal (see supplemental material Section~4 for a
  detailed derivation) with precision matrix and mean given by
  $$
  ...
  $q(\boldsymbol{\beta}_{jk}^\star | \boldsymbol{\beta}_{jk}^{(t)}) =
    \mathcal{N}(\boldsymbol{\mu}_{jk}^{(t)}, \boldsymbol{\Sigma}_{jk}^{(t)})$ which is used to compute the usual acceptance probability.

  Again, assuming a basis function approach for functions $f_{jk}( \cdot )$ the precision matrix 
  and mean can be written as
  $$
...
\caption{\label{fig:blocks} Flexible functional BAMLSS architecture. Each building block in the
  ...
  function is adapted for implementing new models.}
\end{figure}
...
$\,$ \code{sigma} $\sim$ \code{x1 + x2 + s(x3)}$\,$\code{)}
...
objects in order to plug in different Lego bricks for \ref{item:1}--\ref{item:8}. Family objects from the \pkg{gamlss} package are readily supported.
...
To give a brief insight how to use \pkg{bamlss} we illustrate the basic steps on a small toy
example using the well-known simulated motorcycle accident data \citep{bamlss:Silverman:1985}, which
was used to test crash helmets. The data set contains measurements of the head acceleration
...
@
where function \fct{s} is the smooth term constructor of the \pkg{mgcv} package. The model is
then fitted by
<<eval=FALSE>>=
...
\centering
<<echo=FALSE, fig=TRUE, width=3, height=2.8>>=
...
types based on Gaussian processes \citep{bamlss:Paciorek:2007}. Straight forward is to include
further model types like state-space models \citep{bamlss:Carter+Kohn:1994}.
