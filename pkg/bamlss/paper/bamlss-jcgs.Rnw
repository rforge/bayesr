...
the distribution parameters and big data sets are a possibly long computing time.  To overcome this
...
usable for for instance a heteroscedastic Gaussian model.

Interesting extensions in the future include efficient matrix transformations for additional effect
types based on Gaussian processes (ADD REF). Straight forward is to include further model
types like state-space models (ADD REF).
