...
\fct{summary}, \fct{plot}, \fct{predict}, etc., are provided. E.g., the posterior mean based on MCMC 
samples for parameter $\mu$ can be computed by
<<eval=FALSE>>=
...
\item[\proglang{R}-package:] \proglang{R}-package \pkg{bamlss} for reproducing the illustrations is available
  at \linebreak \url{http://CRAN.R-project.org/package=bamlss}.

\item[Data-sets:] Data sets used in the illustrations are provided withing the \proglang{R} package
  \pkg{bamlss} \url{http://CRAN.R-project.org/package=bamlss}.
