...
\item \emph{The manuscript is well written, however, in my opinion the authors sometimes provide too
  ...
  and put the score vectors and second derivatives to an online supplement. This makes Section~6
  better understandable for the purpose of the illustration. Interested readers can get detailed
...
\item \emph{The manuscript is clearly too long. It’s perfectly understandable that the authors want to
  ...
  some paragraphs of Section~4 as well. In total, we reduced the initial, submitted version by 4
  pages so that we are now at 34 pages up to the Summary (40 pages including references, before 47
  ...
  is exactly 30 pages. If this is still too long, we could probably drop the last example. However,
  because the last illustration particularly reflects the strengths of this approach, we would prefer
  to keep the section in the manuscript.
...
\item \emph{The paper is too long. It needs to be reduced to 30 pages, by shifting some material to
  ...
  initial manuscript size by 4 pages up to the summary and by 7 pages including the references. If
  we drop the JCGS review \LaTeX-template line spacing of 1.45 the manuscript is exactly 30 pages
  long now. We could drop the last example if necessary, however, we believe this illustration
  particularity presents the strengths of this approach and we would therefore prefer to keep it
  in the manuscript.
...
\item \emph{In your figure captions, add a sentence telling the reader the most important thing to
  ...
  Excellent idea, as suggested, we added a story-telling sentence at certain captions. \fixme{Achim,
  can you look at the captions, too. Maybe you have some more good edits?}
