...
\item \emph{The manuscript is clearly too long. It’s perfectly understandable that the authors want to
  ...
  We had careful reads and have found several places to be more precise and concise. This includes some paragraphs of Section 4 as well. In total, we reduced the initial, submitted version by XXX PAGES so that we are now at XXX pages in total. FIXME SHOULD WE OFFER TO SHORTEN MORE ON REQUEST???
...
\item \emph{I think it is good practice to highlight also the limitations of the approach (e.g.\
  ...
  You are completely right that some critics, placement in a wider context and directions of future research are good practice at the end of the paper. We added possible remedies for problems that require even faster (possible approximative) methods but we also discuss several extensions and directions to broaden the available model class of bamlss in the near future.
