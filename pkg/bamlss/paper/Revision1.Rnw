...
\item \emph{The manuscript is well written, however, in my opinion the authors sometimes provide too
  ...
  Yes indeed, you are completely right. Given the initial length of the paper we followed your advise and put the score vectors and second derivatives to an online supplement. This makes Section 6 better understandable for the purpose of the illustration. Interest readers can get detailed information about implementation in the supplement.
...
\item \emph{The manuscript is clearly too long. It’s perfectly understandable that the authors want to
  ...
  We had several careful reads and have found several places to be more precise and concise. This includes some paragraphs of Section 4 as well. In total, we reduced the initial, submitted version by XXX PAGES so that we are now at XXX pages in total. FIXME SHOULD WE OFFER TO SHORTEN MORE ON REQUEST???

\item \emph{The illustration examples highlight the enormous and versatile capabilities of the bamlss
  framework and are nicely presented. However, I guess an additional smaller and faster toy
  example (perhaps in Section 5 with a few lines of R-code) could help to illustrate how to apply
  \pkg{bamlss} and how to start using it.}
  
  Thank you very much for this comment. We liked the idea of a smaller toy example and added the simple illustration of the popular motorcycle data \texttt{mcycle} from the \texttt{MASS} package in R. This data set with measurements of head acceleration in a simulated motorcycle accident is well suited to demonstrate the benefits of a location-scale model with the bamlss framework. As suggested, we are explicit with lines of code in R and some explanations in Section 5.
...
\item \emph{I think it is good practice to highlight also the limitations of the approach (e.g.\
  ...
  You are completely right that some critics, placement in a wider context and directions of future research are good practice at the end of the paper. We had some careful thoughts and added possible remedies for problems that require even faster (possible approximative) methods but we also discuss several extensions and directions to broaden the available model class of bamlss in the near future.
...
\item \emph{Notation is obviously a mess for this huge framework of different model classes: However,
  ...
  \fixme{Achim, any good suggestion?}
...
\item \emph{The paper is too long. It needs to be reduced to 30 pages, by shifting some material to
  ...
  Following your and the referee's advise, we shortened the paper thoughtfully and removed some derivations to an online supplement. In total, we reduced the initial, submitted version by XXX PAGES so that we are now at XXX pages in total. FIXME SHOULD WE OFFER TO SHORTEN MORE ON REQUEST???
...
\item \emph{In your figure captions, add a sentence telling the reader the most important thing to
  ...
  Excellent idea, As suggest, we added a story-telling sentence at certain captions.
