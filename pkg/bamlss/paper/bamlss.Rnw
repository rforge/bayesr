...
\end{equation}
with possibly linear or nonlinear functions $f_{jk}(\cdot)$, $j = 1, \ldots, J_k$ and $k = 1, \ldots, K$,
of the covariates that are represented by the generic vector $\mathbf{x}$. In
the following, we only assume that each function $f_{jk}(\cdot)$ is a composition of covariate data
$\mathbf{x}$ and regression coefficients $\boldsymbol{\beta}_{jk}$, hence, $f_{jk}(\cdot)$ are not
necessarily linear functions or functions that have a linear representation. More specifically,
a simple linear effect of one continuous covariate could be given by
$f_{jk}(\mathbf{x}, \boldsymbol{\beta}_{jk}) = x_{1}\beta$, whereas
a nonlinear effect $f_{jk}(\mathbf{x}, \boldsymbol{\beta}_{jk}) = f(x_{1})$
could, e.g., be modeled by regression splines or other techniques that are nonlinear in the
parameters. More examples that are covered by this framework are: Spatially correlated effects of some
discrete location index, varying coefficients, spatially varying effects, random intercepts and
slopes, etc.
...
P(enalised)-spline representation of $f_{jk}$ \citep{bamlss:Eilers+Marx:1996}, it is common to add a
...
\text{Mod}(\boldsymbol{\vartheta} | \mathbf{y}) =
  \underset{\boldsymbol{\vartheta}}{\text{arg max }} \log \, p(\boldsymbol{\vartheta} | \mathbf{y})
\end{equation}
and equals the maximum likelihood estimator assigning uniform priors (\ref{eqn:uniprior})
...
\mathbf{s}(\boldsymbol{\beta}) = 
  \frac{\partial \log \, p(\boldsymbol{\vartheta} | \mathbf{y})}{\partial \boldsymbol{\beta}}
...
\frac{\partial^2 \log \, p(\boldsymbol{\vartheta} | \mathbf{y})}{\partial \boldsymbol{\beta}_k \partial \boldsymbol{\beta}_s^\top},
\end{equation}
...
\item \emph{Derivative based Metropolis-Hastings}: \label{sec:dmh} \\
  ...
  $$
  with weights $\mathbf{W}_{kk} = -\mathrm{diag}(\partial^2 \ell(\boldsymbol{\beta} | \mathbf{y}) /
