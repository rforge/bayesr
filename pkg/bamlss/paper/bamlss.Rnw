...
\fct{cox.bamlss}
<<>>=
print(names(cox.bamlss()))
@
Additionally, the family object holds a transformer function that sets up the infrastructures for
computing the integrals for the score, Hessians and the log-likelihood function. Since integrals
need to be approximated for computing survival probabilities, too, a special \fct{predict} function
for this task is also added to the family object.
...
@
where function \fct{Surv} creates the survival object, the response that is used for modeling
(see the documentation of the \pkg{survival} package, \citealp{bamlss:Therneau:2016}). Again,
we apply a decomposition of main effect an spatial-varying effects using the \fct{ti()} model
term constructor function. Finally, the model is estimated with
<<eval=FALSE>>=
firemodel <- bamlss(f, data = LondonFire, family = "cox",
  n.iter = 4000, burnin = 2000, thin = 10, cores = 8)
@
using function \fct{cox.mode} to compute starting values for MCMC sampling with function
\fct{cox.mcmc}. On a Linux system with 8 Intel i7-2600 3.40GHz processors estimation takes
