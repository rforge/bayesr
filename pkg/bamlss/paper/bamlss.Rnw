...
\centering
\begin{tabular}{lll}
\multicolumn{2}{c}{$f_{jk}(\mathbf{x}; \boldsymbol{\beta}_{jk})$} & $p_{jk}(\boldsymbol{\beta}_{jk}; \boldsymbol{\tau}_{jk}, \boldsymbol{\alpha}_{jk})$ \\ \hline
no covariate & $\mathbf{1}\beta_{jk}$ & constant prior \\
scalar covariate(s) & $\mathbf{X}_{jk}\boldsymbol{\beta}_{jk}$ &
\end{tabular}
\end{table}
