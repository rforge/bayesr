...
\end{equation}
and $d_{\boldsymbol{\beta}_{jk}}( \cdot )$ and $d_{\boldsymbol{\tau}_{jk}}( \cdot )$ are prior
densities (or combinations of densities) that depend on the type of covariate and prior
assumptions about $f_{jk}( \cdot )$. In this context, $\boldsymbol{\tau}_{jk}$ are typically
...
interpreted as the inverse smoothing parameters in a penalized regression context. Within this very
general setup $\boldsymbol{\alpha}_{jk} = \{\boldsymbol{\alpha}_{\boldsymbol{\beta}_{jk}},
...
As mentioned above note that from a frequentist perspective (\ref{eqn:logpost}) can be viewed as a
penalized log-likelihood, e.g., if a normal prior is used within $p_{jk}( \cdot )$ for
$\boldsymbol{\beta}_{jk}$ (see Section~\ref{sec:priorsbricks}).
...
\renewcommand\thealgorithm{2\alph{algorithm}}
