...
if(!file.exists("firemodel.rda")) {
  f <- list(
    Surv(arrivaltime) ~ ti(arrivaltime) + ti(arrivaltime,lon,lat,d=c(1,2)),
    gamma ~ s(fsintens) + s(daytime,bs="cc") + s(lon,lat,k=100)
  )

  firemodel <- bamlss(f, data = LondonFire, family = "cox", subdivisions = 50,
    n.iter = 12000, burnin = 2000, thin = 10, nu = 0.1)
  ...
  firemodel_plotdata <- data_basehaz(20, 10)
...
plot_basehaz <- function(data, pos = "topright", mcolor = diverge_hcl, ...)
{
  par(mfrow = c(1, 2))
  ...
  xymap(lon, lat, pspatial, data = data, pos = pos,
    layout = FALSE, map = FALSE, add = TRUE, color = mcolor)
  plot(LondonBoroughs, add = TRUE)
  box()
}

plot_basehaz(firemodel_plotdata, col = rgb(0.1, 0.1, 0.1, alpha = 0.1))
@
