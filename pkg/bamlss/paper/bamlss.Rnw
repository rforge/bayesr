...
model definition is needed, which is described in the next section. Section~\ref{sec:architecture}
then describes a general architecture for implementing BAMLSS and beyond.
