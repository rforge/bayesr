...
identically distributed (i.i.d.) random effects can be incorporated using prior (\ref{eqn:prior}).
...
prior (\ref{eqn:prior}) and the inverse gamma prior (\ref{eqn:ig}), results in the log-posterior
...
combination of all assigned priors. Also note that from a frequentist perspective
(\ref{eqn:posterior}) can be viewed as a penalized log-likelihood using prior (\ref{eqn:prior}) for
fixed variance parameters $\boldsymbol{\tau}_{jk}^2$.
