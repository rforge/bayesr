...
is relatively easy to implement algorithms for a variety of models.
\begin{sidewaystable}
\centering
\begin{tabular}{ll|ll}
\multicolumn{2}{c}{$f_{jk}(\mathbf{x}; \boldsymbol{\beta}_{jk})$} & \multicolumn{2}{c}{$p_{jk}(\boldsymbol{\beta}_{jk}; \boldsymbol{\tau}_{jk}, \boldsymbol{\alpha}_{jk})$} \\
Covariate (subset of $\mathbf{x}$) & Effect types &
  $d_{\boldsymbol{\beta}_{jk}}(\boldsymbol{\beta}_{jk} | \, \boldsymbol{\tau}_{jk};
     \boldsymbol{\alpha}_{\boldsymbol{\beta}_{jk}})$ &
  $d_{\boldsymbol{\tau}_{jk}}(\boldsymbol{\tau}_{jk} | \, \boldsymbol{\alpha}_{\boldsymbol{\tau}_{jk}})$ \\ \hline
No covariate & Intercept $\beta$ & $\propto$ const & \dots \\
Scalar covariates $x$ & Linear effect $x \cdot \beta$ & \dots & \dots \\
                      & Smooth effect $f(x)$ & \dots & \dots \\
Vector of scalars & Linear interaction $x_1 \cdot x_2 \cdot \beta$ & \dots & \dots \\
$(x_1, \ldots, x_L)^\top$ & Varying coefficient $f(x_2) \cdot x_1$  & \dots & \dots \\
                  & Smooth effect $f(x_1, \ldots, x_L)$  & \dots & \dots \\
Indicator covariates $s$ & Random intercept $\beta_s$  & \dots & \dots \\
                     & Spatial effect $f(s)$ & \dots & \dots \\
Scalar and indicator & Random slope $x \cdot \beta_s$ & \dots & \dots \\
                     & Space-time effect $f(s, t)$ & \dots & \dots \\
                     & Functional random intercept $f_s(t)$ & \dots & \dots \\
\end{tabular}
\caption{\label{tab:legobricks} Commonly used ``Lego bricks'' for GAM-type models.}
\end{sidewaystable}
...
While the support for implementing new distributions is existing for all of the above software
packages, the possibilities for implementing new model term types is more limited. The \pkg{mgcv}
...
kinds of different model terms, however, the implementation is usually a bit tedious.
...
paper aims to contribute to algorithmic flexibility by presenting iterative estimation algorithms
with exchangeable updating functions. Moreover, the structures exploited in the last three sections
suggest a unified modeling architecture that is useful for building algorithms and writing
interfaces to existing implementations.

In order to unify the multitude of possibilities, a general software system for BAMLSS should
...
\subsection{Architecture} \label{sec:blocks}
...
\end{figure}
The concept can be divided in three parts: First, functions that describe distribution families,
...
is given by
\begin{eqnarray*}
\texttt{accel} &\sim& \mathcal{N}(\mu, \sigma^2) \\
