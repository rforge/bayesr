...
(LFB) is one of the largest in the world. Each year, the LFB is called thousands of times, in most
...
$$
hence, the baseline hazard is represented by $f_0(t)$ and all functions $f_j(t, \mathbf{x})$
...
$$
\eta_{\lambda}(t) = f_0(\texttt{arrivaltime}) + f_1(\texttt{arrivaltime}, \texttt{lon}, \texttt{lat}).
$$
Here, function $f_0(\texttt{arrivaltime})$ is the baseline hazard modeled by a penalized spline and
$f_1(\texttt{arrivaltime}, \texttt{lon}, \texttt{lat})$ is the space-time varying effect modeling
