...
any functions $f_j(\mathbf{X}_j, \boldsymbol{\beta}_j)$ and prior(s) $p(\boldsymbol{\beta}_j)$ that
are available for the covariate(s).
...
\ell(\boldsymbol{\beta} | \mathbf{y}) =
  \sum_{i = 1}^n \log \, f(\mathbf{y}_i | \theta_{i1} = h_1^{-1}(\eta_{i1}(\boldsymbol{\beta}_1)), \ldots,
...
\log \, p(\boldsymbol{\vartheta} | \mathbf{y}) \propto
  \ell(\boldsymbol{\beta} | \mathbf{y}) +
