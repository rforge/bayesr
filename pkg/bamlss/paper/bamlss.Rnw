...
\end{pmatrix}^{(t)},
$$
with diagonal weight matrix $\mathbf{W}_{kk} = -diag(\partial^2 \ell(\boldsymbol{\beta} | \mathbf{y}) /
...
Fisher information with weights $\mathbf{W}_{kk} = -diag(E(\partial^2 \ell(\boldsymbol{\beta} | \mathbf{y}) /
...
\item \emph{Derivative based Metropolis-Hastings}: \label{sec:dmh} \\
  ...
  $$
  with weights $\mathbf{W}_{kk} = -diag(\partial^2 \ell(\boldsymbol{\beta} | \mathbf{y}) /
\partial \boldsymbol{\eta}_k \partial \boldsymbol{\eta}_k^\top)$ and the mean can be written as
  ...
  $-E(\partial^2 \ell(\boldsymbol{\beta} | \mathbf{y}) /
    \partial \boldsymbol{\eta}_k \partial \boldsymbol{\eta}_k^\top)$,
