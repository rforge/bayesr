...
if(!file.exists("figures/rainmodel-data.png") & FALSE) {
  library("spatstat")
  library("sp")
  library("maptools")
  library("raster")
  ...
  raindata <- model.frame(rainmodel)
  ...
  nd$fmu <- predict(rainmodel, newdata = nd, model = "mu", term = "s(long,lat)", intercept = FALSE)
  nd$fsigma <- predict(rainmodel, newdata = nd, model = "sigma", term = "s(long,lat)", intercept = FALSE)

  coordinates(nd) <- c("long", "lat")
  proj4string(nd) <- proj4string(AustriaTopo)
  gridded(nd) <- TRUE
}
@
