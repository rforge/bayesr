...
In order to construct the backfitting algorithm for posterior mode estimation using iteratively
reweighted least squares, as shown in (\ref{eqn:blockbackfit}), as well as the corresponding
derivative based MCMC sampling scheme presented in Section~\ref{sec:dmh} for estimation of the
posterior mean and inference, the derivatives w.r.t.\ parameters $\mu$ and $\sigma$ need to be
computed.
