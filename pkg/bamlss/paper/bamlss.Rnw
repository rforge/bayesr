...
Assuming a basis function approach with multivariate normal priors given by (\ref{eqn:shrinkprior}),
updating equation (\ref{eqn:blocknewton}) can be written as
$$
\begin{pmatrix}
