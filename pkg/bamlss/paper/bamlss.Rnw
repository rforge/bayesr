...
if(!file.exists("firemodel.rda")) {
  f <- list(
    Surv(arrivaltime) ~ s(arrivaltime,bs="tp") + te(arrivaltime,lon,lat,d=c(1,2),bs=c("tp","tp"),mp=FALSE),
    gamma ~ s(fsintens) + s(daytime,bs="cc")
  )

  firemodel <- bamlss(f, data = LondonFire, family = "cox",
    subdivisions = 10, nu = 0.1, sampler = FALSE)
