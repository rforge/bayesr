...
combination single ``bricks''. Moreover, the framework embeds very general model terms which are not
necessarily a linear combination of a design matrix and regression coefficients. Hence,
...
concept is given in the corresponding \proglang{R} implementation \pkg{bamlss}.
...
Within Bayesian models prior distributions need to be assigned for each parameter vector
...
problem. Within the Bayesian formulation the equivalent is, e.g., to assign a normal prior
on $\boldsymbol{\beta}_{jk}$ (see,~e.g.,~\citealp{bamlss:Fahrmeir+Kneib+Lang+Marx:2013}).
However, within the presented BAMLSS framework we relax this assumption and assign the general prior
...
prior the variances can be interpreted as the inverse smoothing parameters in a penalized regression
context. Within this very general setup
$\boldsymbol{\alpha}_{jk} = \{\boldsymbol{\alpha}_{\boldsymbol{\beta}_{jk}},
\boldsymbol{\alpha}_{\boldsymbol{\tau}_{jk}}\}$ is a set of prespecified and fixed
hyper-parameters, e.g., further controlling the shape of $g_{\boldsymbol{\beta}_{jk}}( \cdot )$ and
$g_{\boldsymbol{\tau}_{jk}}( \cdot )$, or incorporating prior beliefs about
$\boldsymbol{\beta}_{jk}$. Within GAM-type models the choice of $\boldsymbol{\alpha}_{jk}$ is
commonly non-informative.  
