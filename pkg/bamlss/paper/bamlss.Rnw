...
plot(rainmodel)
@
The spatially varying effect of the day of the year is not automatically generated by the plotting
function, in this case we compute the seasonal effect for different locations using the \fct{predict}
function. For example, the estimated seasonal effect for Innsbruck and Vienna for parameter $\mu$
can be computed by
<<eval=FALSE>>=
n <- 365
nd <- data.frame(
...
called thousands of times, in most
cases due to dwelling fires. Since the time it takes until a fire engine arrives at the scene after
an emergency call has been received, an important question is how to reduce the response times in order
to prevent further damage or fatal casualties. Clearly, it mostly depends on the distance of the
responsible fire station to the site of how much damage can be prevented, besides, in some
cases it may happen that fire engines are already in use at another fire scenery and therefore it
also depends on the number of fire stations in the area. This is viewed as an optimization problem
in \citet{bamlss:Taylor:2015}, i.e., they treated the question of possible improvements
of response times according to space in a probabilistic way by accounting for random spatial
variation using a piecewise constant gaussian process.

To contribute to the topic, we use the dwelling fire response time data of 2015. The data is freely
available at the London DataStore (\url{http://data.london.gov.uk/}) with an UK Open Government
Licence (OGL v2). The data set, also containing previous years, can be downloaded from
\begin{center}
...
Moreover, to gain better insight to this problem, they analyzed the response times within a
survival context, which seems natural for this kind of time-to-event data. More precisely, they
described the hazard of an event (the fire engine arrives) at time $t$ with a relative risk model of
the form
$$
...
function of time $t$. Following \citet{bamlss:Taylor:2015}, we set up a similar
model with the a time-constant predictor given by
$$
\eta_{\gamma} = \beta_0 + f_1(\texttt{fsintens}) + f_2(\texttt{daytime}) +
  f_3(\texttt{lon}, \texttt{lat}),
$$
where covariate \code{fsintens} is the fire station intensity at a certain location, computed
with a density estimate from all fire stations in London. I.e., this variable is a proxy
for the distance to the next fire station(s), especially suited for situations when the responsible
fire station already send out all fire engines such that help needs to arrive from another station.
Variable \code{daytime} is the time of the day when the fire occurred and accounts for the effect
that it is more difficult for a fire engine to arrive at the scene in rush hours. To treat the
question of structured spatially driven hazards, a spatial effect of longitude and latitude
coordinates is included in the model. All functions $f_1, \ldots, f_3$ are assumed to be possibly
nonlinear and are modeled using penalized splines.
...
are time-varying possibly nonlinear functions of covariates. This model is a Cox-type
additive model introduced by \citet{bamlss:Kneib+Fahrmeir:2007}. To further investigate if
there is a space-time varying effect, i.e., if the shape of the baseline hazard is dependent on the
location we used the following additive predictor
$$
...
received emergency call. Function $f_0(\texttt{arrivaltime})$ is the baseline hazard modeled by a
penalized spline and $f_1(\texttt{arrivaltime}, \texttt{lon}, \texttt{lat})$ is the space-time
varying effect modeling the deviations from the baseline.
...
The integrals need to be computed numerically, e.g., using the trapezoidal rule we ``only'' need
to set up a time grid, lets say with 100 equidistant points within $[0, t_i]$
$$
\mathbf{G} = \begin{pmatrix}
\mathbf{g}_1^\top \\
\vdots \\
\mathbf{g}_n^\top
\end{pmatrix}, \quad \text{with} \quad \mathbf{g}_i = (0, \ldots, t_i)^{\top},
$$
to construct the evaluated $\lambda(t)$ matrix with
$$
\hat{\eta}_{\lambda}(\mathbf{G}) = \begin{pmatrix}
\sum_{j = 1}^{J_\lambda} f_j(x_{1j}(g_{10})) & \ldots & \sum_{j = 1}^{J_\lambda} f_j(x_{1j}(g_{1t_i})) \\
\vdots & \ddots & \vdots \\
\sum_{j = 1}^{J_\lambda} f_j(x_{nj}(g_{n0})) & \ldots & \sum_{j = 1}^{J_\lambda} f_j(x_{nj}(g_{nt_i}))
\end{pmatrix}\hspace*{-0.2cm}\begin{matrix} \phantom{i} \\ \phantom{i} \\ \phantom{i}.\end{matrix}
$$

Fortunately, the time-constant part is a bit easier. Results in IWLS backfitting/proposal scheme
with
$$
...
Optimizer and sampler implemented in function \code{cox.mode()} and \code{cox.mcmc()}.
