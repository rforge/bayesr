...
\item link functions $h_k( \cdot )$,
\item the first order derivatives $\frac{\partial \log \, p(\boldsymbol{\vartheta} ; \mathbf{y}, \mathbf{X})}{\partial \boldsymbol{\beta}_k}$,
...
the log-likelihood w.r.t.\ to parameters $\boldsymbol{\theta}_k$ (here, $\mu$ and $\sigma$).
...
Precipitation climatology models usually assume a normal distribution and are computed for
...
The censored normal model with latent Gaussian variable $\mathbf{y}^\star$ and
observed response $\mathbf{y}$, the square root of daily precipitation observations, is given by
\begin{eqnarray*}
\mathbf{y}^\star &\sim& N(\boldsymbol{\mu}, \boldsymbol{\sigma}^2) \\
\boldsymbol{\mu} &=& \boldsymbol{\eta}_{\mu} \\
\log(\boldsymbol{\sigma}) &=& \boldsymbol{\eta}_{\sigma} \\
\mathbf{y} &=& max(\mathbf{0}, \mathbf{y}^\star).
\end{eqnarray*}
...
$$
\boldsymbol{\eta} = \beta_0 + f_1(\texttt{alt}) + f_2(\texttt{day}) +
  f_3(\texttt{lon}, \texttt{lat}) + f_4(\texttt{day}, \texttt{lon}, \texttt{lat}),
$$
here function $f_1$ is an altitude effect, $f_2$ the seasonal variation, $f_3$ a spatially
correlated effect and $f_4$ a spatially-varying seasonal effect.

The density function of a left censored Gaussian distribution is given by
\begin{equation}
  f(y; \, \mu = \eta_{\mu}, \, \sigma = \exp(\eta_{\sigma})) = \begin{cases} 
    \frac{1}{\sigma} \phi\left(\frac{y-\mu}{\sigma} \right) & y > 0 \\
    \Phi\left(\frac{c_l-\mu}{\sigma} \right) & \mbox{else.}
  \end{cases}
\end{equation}
