...
\text{Mean}(\boldsymbol{\vartheta} | \mathbf{y}) = E(\boldsymbol{\vartheta} | \mathbf{y}) =
...
In the following we summarize a number of sampling techniques that are especially suited for this
...
\item \emph{Random walk Metropolis}: \label{sec:rwm} \\
  ...
  proceeds by drawing a new $\boldsymbol{\vartheta}_{jk}^{(t+1)}$ from a symmetric jumping
  distribution $q(\boldsymbol{\vartheta}_{jk}^{(t)}, \boldsymbol{\vartheta}_{jk}^{(t+1)})$, the
  candidate is then accepted as the new state of the Markov chain with probability
  \begin{equation} \label{eqn:accprob}
  \alpha\left( \boldsymbol{\vartheta}_{jk}^{(t)}, \boldsymbol{\vartheta}_{jk}^{(t+1)}\right) =
  \text{min} \left( \frac{p(\boldsymbol{\vartheta}_{jk}^{(t+1)} | \mathbf{y})}{
    p(\boldsymbol{\vartheta}_{jk}^{(t)} | \mathbf{y})}, 1 \right).
  \end{equation}
  \citet{bamlss:Gelman+Roberts+Gilks:1996}


\item Derivative based Metropolis-Hastings \label{sec:dmh}

\item Hamiltonian Monte Carlo \label{sec:hmcmc}

\item Slice sampling \label{sec:smcmc}
\end{itemize}
