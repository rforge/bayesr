...
framework thereby leverages repetitive components that arise when applying iterative based
estimation algorithms, either for posterior mode or mean estimation. Moreover, the framework
embeds very general model terms which are not necessarily a linear combination of a design matrix
and regression coefficients. Within this modeling architecture the process of model building
becomes relatively straightforward and supports more than the GAMLSS statistical model class.
However, because of the great similarities with GAMLSS we call the conceptional framework Bayesian 
additive models for location, scale and shape (BAMLSS), and beyond.
...
on the computationally challenging Cox model for continuous time.
...
Since the functions $f_{jk}( \cdot )$ may have a complex nonlinear form, to ensure regularization,
e.g., for penalizing too abrupt jumps using a P(enalised)-spline representation of $f_{jk}( \cdot )$
\citep{bamlss:Eilers+Marx:1996}, in the frequentist setting it is common to add a
penalty $\text{pen}(\mathbf{f}_{jk}) = \text{pen}(\boldsymbol{\beta}_{jk})$ to the regression
problem. Within the Bayesian formulation the equivalent is to put prior distributions
$\pi_{jk}( \cdot )$ on the regression coefficients $\boldsymbol{\beta}_{jk}$.

A simple choice for a non-informative prior is
\begin{equation} \label{eqn:oneprior}
\pi_{jk}(\boldsymbol{\beta}_{jk}) \propto 1.
\end{equation}
Informative priors for $\boldsymbol{\beta}_{jk}$ are usually based on a normal kernel
(see,~e.g.,~\citealp{bamlss:Fahrmeir+Kneib+Lang+Marx:2013}).
In the presented BAMLSS framework we relax this assumption and assign the general prior
\begin{equation} \label{eqn:gprior}
\pi_{jk}(\boldsymbol{\beta}_{jk}; \boldsymbol{\tau}_{jk}) \propto
   g_{\boldsymbol{\beta}_{jk}}(\boldsymbol{\beta}_{jk} | \boldsymbol{\tau}_{jk}) \cdot
   g_{\boldsymbol{\tau}_{jk}}(\boldsymbol{\tau}_{jk}),
\end{equation}
where $g_{\boldsymbol{\beta}_{jk}}( \cdot )$ and $g_{\boldsymbol{\tau}_{jk}}( \cdot )$ are prior
densities that depend on the type of covariate and prior assumptions about $f_{jk}( \cdot )$.
The vector $\boldsymbol{\tau}_{jk}$ represents additional prior parameters,
...
\log \, p(\boldsymbol{\beta}, \boldsymbol{\tau} ; \mathbf{y}, \mathbf{X}) \propto
  \ell(\boldsymbol{\beta} ; \mathbf{y}, \mathbf{X}) +
  \sum_{k = 1}^K\sum_{j = 1}^{J_k} \left\{ \log \,
  \pi_{jk}(\boldsymbol{\beta}_{jk}; \boldsymbol{\tau}_{jk}) \right\},
\end{equation}
...
the vector of all assigned hyper-priors used within prior functions $\pi_{jk}( \cdot )$.
Note that from a frequentist perspective (\ref{eqn:logpost}) can be viewed as a penalized
log-likelihood, e.g., if a normal prior is used for $\pi_{jk}( \cdot )$.
...
\end{equation}
assigning uniform priors (\ref{eqn:oneprior})
