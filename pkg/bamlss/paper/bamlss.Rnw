...
if(!file.exists("firemodel.rda")) {
  ...
  data_basehaz <- function(n = 30, target = 6, k = 20, ...)
  {
    ...
    co <- owin(co["x", ], co["y", ])
    fsppp <- ppp(x = co2[, 1], co2[, 2], window = co)
    ...
    nd$spatial_prob <- predict(firemodel, newdata = nd,
      term = c("ti(arrivaltime)", "ti(arrivaltime,lon,lat)", "s(fsintens)", "s(daytime)", "s(lon,lat)"),
...
if(!file.exists("figures/firemodel-data.png")) {
    ...
    main = NULL, spar = TRUE, ...)
  {
    ...
    if("stations" %in% what) {
      ...
      lines(range(lon) + c(-0.03, 0.03), rep(co["y", 2] + shift, 2))
      lines(rep(min(lon) - 0.03, 2), c(co["y", 2] + shift, ylim[2]))
      wt <- c(min(atimes), 6, 12, 20)
      wt2 <- scale2(wt, co["y", 2] + shift, ylim[2])
      for(j in seq_along(wt)) {
        lines(c(min(lon) - 0.03, min(lon) - 0.04), rep(wt2[j], 2))
        text(min(lon) - 0.04, wt2[j], fmt(wt[j], 2, 0), pos = 2)
      }
      wt2 <- scale2(c(min(atimes), max(atimes)), co["y", 2] + shift, ylim[2])
      text(min(lon) - 0.13, mean(wt2), "Arrivaltime", srt = 90, cex = 0.95)
    }
  }
