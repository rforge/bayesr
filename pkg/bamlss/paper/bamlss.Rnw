...
$\mathbf{y} \sim N(\boldsymbol{\mu}, \boldsymbol{\sigma}^2)$ can then be summarized by the following
pseudo code:
\begin{center}
{ \renewcommand{\arraystretch}{1}
\begin{tabular}{l}
\code{list(} \\
$\quad$ \code{family, names, links,} \medskip \\
$\quad$ \code{d(y, par),} \\
$\quad$ \code{p(y, par),} \\
$\quad$ \code{q(y, par),} \\
$\quad$ \code{r(y, par),} \medskip \\
$\quad$ \code{score = list(} \\
$\quad\quad$ \code{mu(y, par),} \\
$\quad\quad$ \code{sigma(y, par)} \\
$\quad$ \code{),} \\
$\quad$ \code{hess = list(} \\
$\quad\quad$ \code{mu(y, par),} \\
$\quad\quad$ \code{sigma(y, par)} \\
...
the log-likelihood w.r.t.\ to parameters $\boldsymbol{\theta}_k$.
