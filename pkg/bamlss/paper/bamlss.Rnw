...
if(!file.exists("rainmodel.rda") & FALSE) {
  ...
  rainmodel <- bamlss(f, data = homstart2, family = "cnorm",
    ...
    n.iter = 120, burnin = 20, thin = 1, cores = 1)
...
if(!file.exists("figures/rainmodel-effects.png") & FALSE) {
  ...
  png("figures/rainmodel-effects.png", units = "in", res = 120, width = 6, height = 8)
  ...
  matplot(nd2$day, nd2[, grep("fmu", names(nd2))], type = "l", lty = 1,
    ...
    ylim = ylim.mu)
  legend("topright", c("Latitude", paste(latbreaks[3:1], latbreaks[4:2], sep = "-")),
  ...
  plot(Austria)
  xymap(long, lat, rain192, data = nd, pos = "topleft", layout = FALSE, map = FALSE,
    add = TRUE, color = colors.rain, shift = c(0.09, 0.04), distance.labels = 0,
    width = 0.3, symmetric = FALSE, swap = FALSE, digits = 1, range = c(0, 2.2),
    lrange = round(c(0, max(nd$rain192)), 1))
  plot(Austria, add = TRUE)
  box()
  axis(1)
  axis(4)
