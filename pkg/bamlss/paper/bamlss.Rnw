...
\begin{itemize}
\item The log-likelihood function of the continuous time Cox-model is then given by
  $$
  \ell(\boldsymbol{\beta}; \mathbf{y}, \mathbf{X}) =
...
\item For derivative based estimation using Algorithm~\ref{fig:umode} and for MCMC simulation with
  ...
  $$
\item The elements of the Hessian w.r.t.\ $\boldsymbol{\beta}_{\lambda}$ are
  $$
  ...
  \ref{fig:mcmc}.
\item Constructing updating functions for the time-constant part $\eta_{\gamma}$ again yields an
  IWLS updating scheme, see Section~\ref{sec:legobricks}, with working observations given by
  $$
