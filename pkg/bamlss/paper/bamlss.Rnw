...
if(!file.exists("firemodel.rda")) {
  ...
  firemodel <- bamlss(f, data = LondonFire, family = "cox",
    subdivisions = 10, nu = 0.1, n.iter = 1200, burnin = 200, thin = 3, cores = 3)
  ...
  data_basehaz <- function(n = 30, target = 6, ...)
  {
    ...
    nd$pspatial <- predict(firemodel, newdata = nd,
      term = c("s(arrivaltime)", "te(arrivaltime,lon,lat)", "s(daytime)"),
      FUN = mean, intercept = TRUE, type = "prob", time = target, ...)
...
plot_basehaz <- function(data, pos = "top", curve = TRUE, ...)
{
  if(curve) {
    par(mfrow = c(1, 2))
    bamlss_factor2d_plot(data[, c("id", "atime", "p50")], rug = FALSE,
      color = rgb(0.1, 0.1, 0,1, alpha = 0.1), ...)
    abline(v = attr(data, "target"))
  }
  plot(LondonBoroughs)
  xymap(lon, lat, pspatial, data = data, pos = pos,
    layout = FALSE, map = FALSE, add = curve, color = diverge_hcl,
    shift = c(0.08, 0.03), symmetric = FALSE)
