...
<<eval=FALSE>>=
f <- list(
  "mu" = sqrt(raw) ~ s(elevation,k=3) + ti(day,bs="cc",k=10) +
    ti(long,lat,bs="tp",d=2,k=50) +
    ti(day,long,lat,bs=c("cc","tp"),d=c(1,2),k=c(8,30)),
  "sigma" = ~ s(elevation,k=3) + ti(day,bs="cc",k=10) +
    ti(long,lat,bs="tp",d=2,k=50) +
    ti(day,long,lat,bs=c("cc","tp"),d=c(1,2),k=c(8,30))
)
@
...
the model takes approximately 1.5 days.
...
nd <- data.frame(
  "day" = rep(1:365, 2),
  "long" = c(rep(11.404881, 365), rep(16.384151, 365)),
  "lat" = c(rep(47.269362, 365), rep(48.208906, 365)),
  "City" = c(rep("Innsbruck", 365), rep("Vienna", 365))
)
...
The resulting plots of the estimated effects are shown in Figure~\ref{fig:rainmodeleffects}.
