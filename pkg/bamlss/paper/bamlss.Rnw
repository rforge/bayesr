...
(\ref{eqn:gprior}) parameters $\boldsymbol{\tau}$ cannot be estimated by maximization of the
...
$\boldsymbol{\tau}$ represents the so called smoothing parameters.

Therefore, information criteria like the Akaike information criterion (AIC), or the corrected AIC,
...
$y^\star$ from the response distribution
...
\end{equation*}
for each posterior sample $\boldsymbol{\beta}_k^{(t)}$ $t = 1, \ldots, T$.
