...
\item \emph{Derivative based Metropolis-Hastings}: \label{sec:dmh} \\
  ...
  $$
  (\boldsymbol{\Sigma}_{jk}^{-1})^{(t)} = -\mathbf{H}_{kk}\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right),
  ...
  $$
  (\boldsymbol{\Sigma}_{jk}^{-1})^{(t)} = \mathbf{X}_{jk}^\top\mathbf{W}_{kk}\mathbf{X}_{jk} + \mathbf{G}_{jk}
  $$
...
\partial \boldsymbol{\eta}_k \partial \boldsymbol{\eta}_k^\top)$ and the mean can be written as
  ...
  the computation of the mean is equivalent to a full Newton step. This is known as the
