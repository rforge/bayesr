...
excessively demands the computer's storage as well as CPU power. In order to prevent problems
associated with storage and to reduce computational time,
we make use of the fact that, e.g., the effect of the day of year only has 365 unique observations.
This is much smaller than the total number of observations of the data set, i.e., the design matrix
as a lot of duplicated rows. 
