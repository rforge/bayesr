...
\citep{bamlss:Yee:2016, bamlss:Yee:2015} implement all model term infrastructures in their own fashion style. This is
...
\begin{enumerate}[label=\theenumi\theenumii.,ref=\theenumi\theenumii]
  \item  \label{item:6a}w.r.t.\ regression coefficients $\frac{\partial \ell(\boldsymbol{\beta};
...
\begin{enumerate}[label=\theenumi\theenumii.,ref=\theenumi\theenumii]
  \item \label{item:7a} w.r.t.\ regression coefficients $\frac{\partial^2 \ell(\boldsymbol{\beta}; \mathbf{y},
...
\ref{item:6a} and \ref{item:7a} need to be supplied for implementing a new model. As shown in the
upcoming this is relatively straightforward in most cases since \ref{item:6a} and \ref{item:7a} can
