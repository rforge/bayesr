...
\ell(\boldsymbol{\vartheta} | \mathbf{y}) =
  \sum_{i = 1}^n ln \, f(\mathbf{y}_i | \theta_{i1} = h_1^{-1}(\eta_{i1}(\boldsymbol{\vartheta}_1)), \ldots,
  \theta_{iK} = h_K^{-1}(\eta_{iK}(\boldsymbol{\vartheta}_K)))
\end{equation}
a number of times, where the vector
$\boldsymbol{\vartheta} = (\boldsymbol{\vartheta}_1^\top, \ldots, \boldsymbol{\vartheta}_K^\top)^\top$ 
...
(\boldsymbol{\vartheta}_{1k}^\top, \ldots, \boldsymbol{\vartheta}_{p_kk}^\top)^\top =
(\boldsymbol{\beta}_{1k}^\top, \ldots, \boldsymbol{\beta}_{p_kk}^\top)^\top$ for $k = 1, \ldots, K$.
...
a more detailed discussion on variance/hyperparameter estimation). In the following we assume that
variance parameters are held fix and $\boldsymbol{\vartheta}_{jk} = \boldsymbol{\beta}_{jk}$ for
$j = 1, \ldots, p_k$ and $k = 1, \ldots, K$.
Estimation of $\boldsymbol{\beta} = (\boldsymbol{\beta}_1^\top, \ldots, \boldsymbol{\beta}_K^\top)^\top$
requires solving equations
$\partial (ln \, p(\boldsymbol{\beta} | \mathbf{y})) / \partial \boldsymbol{\beta} = \mathbf{0}$
...
\mathbf{s}(\boldsymbol{\beta}) = 
  \frac{\partial ln \, p(\boldsymbol{\beta} | \mathbf{y})}{\partial \boldsymbol{\beta}}
= \frac{\partial \ell(\boldsymbol{\beta} | \mathbf{y})}{\partial \boldsymbol{\beta}} +
    \sum_{k = 1}^K\sum_{j = 1}^{p_k} \left\{ \frac{\partial ln \, p_{jk}(\boldsymbol{\beta}_{jk})}{\partial \boldsymbol{\beta}} \right\}.
\end{equation}
and hessian matrix $\mathbf{H}(\boldsymbol{\beta})$ with components
\begin{equation} \label{eqn:hessian}
\mathbf{H}_{ks}(\boldsymbol{\beta}) = 
\frac{\partial^2 ln \, p(\boldsymbol{\beta})}{\partial \boldsymbol{\beta}_k \partial \boldsymbol{\beta}_s^\top} = \frac{\partial \mathbf{s}(\boldsymbol{\beta}_k)}{\partial \boldsymbol{\beta}_s^\top},
\end{equation}
...
\frac{\partial \ell(\boldsymbol{\beta} | \mathbf{y})}{\partial \boldsymbol{\beta}_k} =
  \frac{\partial \ell(\boldsymbol{\beta} | \mathbf{y})}{\partial \boldsymbol{\eta}_k}
  \frac{\partial \boldsymbol{\eta}_k}{\partial \boldsymbol{\beta}_k} = 
  \frac{\partial \ell(\boldsymbol{\beta} | \mathbf{y})}{\partial \boldsymbol{\theta}_k}
...
the components of $\mathbf{H}_{ks}$ including $\ell(\boldsymbol{\beta} | \mathbf{y})$ can be
written as
\begin{equation} \label{hessian2}
\frac{\partial^2 \ell(\boldsymbol{\beta} | \mathbf{y})}{\partial \boldsymbol{\beta}_k \partial \boldsymbol{\beta}_s^\top} =
\left( \frac{\partial \boldsymbol{\eta}_s}{\partial \boldsymbol{\beta}_s} \right)^\top
\frac{\partial^2 \ell(\boldsymbol{\beta} | \mathbf{y})}{\partial \boldsymbol{\eta}_k\partial \boldsymbol{\eta}_s^\top}
\frac{\partial \boldsymbol{\eta}_k}{\partial \boldsymbol{\beta}_k}
\,\, \underbrace{
  \, + \, \frac{\partial \ell(\boldsymbol{\beta} | \mathbf{y})}{\partial \boldsymbol{\eta}_k}
...
\frac{\partial^2 \ell(\boldsymbol{\beta} | \mathbf{y})}{\partial \boldsymbol{\eta}_k\partial \boldsymbol{\eta}_s^\top} =
  ...
  \frac{\partial^2 \ell(\boldsymbol{\beta} | \mathbf{y})}{\partial \boldsymbol{\theta}_k \partial \boldsymbol{\theta}_s^\top}
  \frac{\partial \boldsymbol{\theta}_k}{\partial \boldsymbol{\eta}_k}
...
\end{pmatrix}^{(t)},
$$
with diagonal weight matrix $\mathbf{W}_{kk} = -diag(\partial^2 \ell(\boldsymbol{\beta}_k | \mathbf{y}) /
...
\vdots \\
\mathbf{X}_{p_kk}^\top \mathbf{u}_k^{(t)} - \mathbf{G}_{p_kk}\boldsymbol{\beta}_{p_kk}^{(t)} \\
\end{pmatrix}
$$
and derivatives $\mathbf{u}_k = \partial \ell(\boldsymbol{\beta}_k | \mathbf{y}) /
...
\ldots + \mathbf{X}_{jk}^\top\mathbf{W}_{kk}\mathbf{X}_{p_kk}\boldsymbol{\beta}_{p_kk}^{(t + 1)} - \\
(\mathbf{X}_{jk}^\top\mathbf{W}_{kk}\mathbf{X}_{jk} + \mathbf{G}_{jk})\boldsymbol{\beta}_{jk}^{(t)} -
\ldots - \mathbf{X}_{jk}^\top\mathbf{W}_{kk}\mathbf{X}_{p_kk}\boldsymbol{\beta}_{p_kk}^{(t)}
...
\mathbf{G}_{jk}(\boldsymbol{\beta}_{jk}^{(t+1)} - \boldsymbol{\beta}_{jk}^{(t)}) +
  ...
  \mathbf{X}_{jk}^\top\mathbf{W}_{kk}\mathbf{X}_{p_kk}\boldsymbol{\beta}_{p_kk}^{(t+1)} - \\
  \mathbf{X}_{jk}^\top\mathbf{W}_{kk}\mathbf{X}_{jk}\boldsymbol{\beta}_{jk}^{(t)} - \ldots -
  \mathbf{X}_{jk}^\top\mathbf{W}_{kk}\mathbf{X}_{p_kk}\boldsymbol{\beta}_{p_kk}^{(t)}
  &=& \mathbf{X}_{jk}^\top \mathbf{u}_k^{(t)} - \mathbf{G}_{jk}\boldsymbol{\beta}_{jk}^{(t)}
\end{eqnarray*}
\begin{eqnarray*}
...
Fisher information with weights $\mathbf{W}_{kk} = -diag(E(\partial^2 \ell(\boldsymbol{\beta}_k | \mathbf{y}) /
...
E(\boldsymbol{\vartheta} | \mathbf{y}) =
  \int \boldsymbol{\vartheta} p(\boldsymbol{\vartheta} | \mathbf{y})d\boldsymbol{\vartheta}.
\end{equation}
...
In the following we summarize four sampling techniques that are especially suited for this
...
\item \emph{Random-walk Metropolis}: \label{sec:rwm} \\
  ...
  proceeds by drawing a candidate $\boldsymbol{\vartheta}_{jk}^{\star}$ from a symmetric jumping
  distribution $q(\boldsymbol{\vartheta}_{jk}^{\star}| \boldsymbol{\vartheta}_{jk}^{(t)})$, the
  candidate is then accepted as the new state of the Markov chain with probability
  $$
  \alpha\left( \boldsymbol{\vartheta}_{jk}^{\star} | \boldsymbol{\vartheta}_{jk}^{(t)}\right) =
  \text{min} \left\{ \frac{p(\boldsymbol{\vartheta}_{jk}^{\star} | \cdot)}{
    p(\boldsymbol{\vartheta}_{jk}^{(t)} | \cdot)}, 1 \right\}
  $$
  with the log-posterior $p(\boldsymbol{\vartheta}_{jk} | \cdot)$ evaluated at the proposed and
  current value.
  Commonly, the jumping distribution is a normal distribution $N(\boldsymbol{\vartheta}_{jk}^{(t)}, \boldsymbol{\Sigma}_{jk})$ centered at the current iterate and fixed covariance matrix. Although this algorithm
  ...
  optimize the acceptance rate to roughly $1/4$ are suggested to obtain a good mixing. Similarly,
...
\item \emph{Derivative based Metropolis-Hastings}: \label{sec:dmh} \\
  ...
  $N(\boldsymbol{\vartheta}_{jk}^{(t)}, \boldsymbol{\Sigma}_{jk})$ is to use the local curvature
  information
  $$
  \boldsymbol{\Sigma}_{jk} = -\left( \frac{\partial^2 p(\boldsymbol{\vartheta} | \mathbf{y})}{
    \partial \boldsymbol{\vartheta}_{jk}\boldsymbol{\vartheta}_{jk}^\top} \right)^{-1}
  $$
  computed at the posterior mode estimate $\hat{\boldsymbol{\vartheta}}_{jk}$. However, fixing
  ...
  $p(\boldsymbol{\vartheta}_{jk} | \cdot)$
  ...
  \begin{eqnarray*}
  p(\boldsymbol{\vartheta}_{jk}^\star | \cdot) &\propto& \exp\left\{
    ln\,p\left(\boldsymbol{\vartheta}_{jk}^{(t)} | \cdot\right) +
    \left(\boldsymbol{\vartheta}_{jk}^\star - \boldsymbol{\vartheta}_{jk}^{(t)}\right)^\top s\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right) + \right.\\
  && \left. \frac{1}{2}\left(\boldsymbol{\vartheta}_{jk}^\star - \boldsymbol{\vartheta}_{jk}^{(t)}\right)^\top
    \mathbf{H}_{kk}\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right)
    \left(\boldsymbol{\vartheta}_{jk}^\star - \boldsymbol{\vartheta}_{jk}^{(t)}\right)\right\} \\
  &\propto& \exp\left\{(\boldsymbol{\vartheta}_{jk}^\star)^\top s\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right) +
    \left(\frac{1}{2}(\boldsymbol{\vartheta}_{jk}^\star)^\top\mathbf{H}_{kk}\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right) - \right.\right. \\
  && \qquad\qquad\left.\left. \frac{1}{2}(\boldsymbol{\vartheta}_{jk}^{(t)})^\top\mathbf{H}_{kk}\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right) \right)\left(\boldsymbol{\vartheta}_{jk}^\star - \boldsymbol{\vartheta}_{jk}^{(t)}\right)\right\} \\
  &\propto& \exp\left\{(\boldsymbol{\vartheta}_{jk}^\star)^\top s\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right) +
    \frac{1}{2}(\boldsymbol{\vartheta}_{jk}^\star)^\top\mathbf{H}_{kk}\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right)\boldsymbol{\vartheta}_{jk}^\star - \right. \\
  && \left. \qquad\qquad \frac{1}{2}(\boldsymbol{\vartheta}_{jk}^\star)^\top\mathbf{H}_{kk}\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right)\boldsymbol{\vartheta}_{jk}^{(t)} -
    \frac{1}{2}(\boldsymbol{\vartheta}_{jk}^{(t)})^\top\mathbf{H}_{kk}\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right)\boldsymbol{\vartheta}_{jk}^\star \right\} \\
  &=& \exp\left\{
    \frac{1}{2}(\boldsymbol{\vartheta}_{jk}^\star)^\top\mathbf{H}_{kk}\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right)\boldsymbol{\vartheta}_{jk}^\star +
    (\boldsymbol{\vartheta}_{jk}^\star)^\top s\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right) -
    (\boldsymbol{\vartheta}_{jk}^\star)^\top\mathbf{H}_{kk}\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right)\boldsymbol{\vartheta}_{jk}^{(t)} \right\} \\
  &=& \exp\left\{
    -\frac{1}{2}(\boldsymbol{\vartheta}_{jk}^\star)^\top-\mathbf{H}_{kk}\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right)\boldsymbol{\vartheta}_{jk}^\star +
    (\boldsymbol{\vartheta}_{jk}^\star)^\top \left(s\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right) - \mathbf{H}_{kk}\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right)\boldsymbol{\vartheta}_{jk}^{(t)}\right) \right\}
  \end{eqnarray*}
  which is proportional to a multivariate normal distribution with precision matrix and mean
  $$
  \left(\boldsymbol{\Sigma}_{jk}^{(t)}\right)^{-1} = -\mathbf{H}_{kk}\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right),
  \qquad \boldsymbol{\mu}_{jk}^{(t)} = \boldsymbol{\Sigma}_{jk}^{(t)}\left\{ s\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right) -
    \mathbf{H}_{kk}\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right)\boldsymbol{\vartheta}_{jk}^{(t)} \right\}.
  $$
  Hence, the proposal density for $\boldsymbol{\vartheta}_{jk}$ is
  $q(\boldsymbol{\vartheta}_{jk}^\star | \boldsymbol{\vartheta}_{jk}^{(t)}) =
    N(\boldsymbol{\mu}_{jk}^{(t)}, \boldsymbol{\Sigma}_{jk}^{(t)})$ and the acceptance probability
  of the candidate is then computed by
  $$
  \alpha\left( \boldsymbol{\vartheta}_{jk}^{\star} | \boldsymbol{\vartheta}_{jk}^{(t)}\right) =
  \text{min} \left\{ \frac{p(\boldsymbol{\vartheta}_{jk}^{\star} | \cdot)q(\boldsymbol{\vartheta}_{jk}^{(t)} | \boldsymbol{\vartheta}_{jk}^\star)}{
    p(\boldsymbol{\vartheta}_{jk}^{(t)} | \cdot)q(\boldsymbol{\vartheta}_{jk}^\star | \boldsymbol{\vartheta}_{jk}^{(t)})  }, 1 \right\}.
  $$
  ...
  $$
  with weights $\mathbf{W}_{kk} = -diag(\partial^2 \ell(\boldsymbol{\beta}_k | \mathbf{y}) /
\partial \boldsymbol{\eta}_k \partial \boldsymbol{\eta}_k^\top)$ and the mean can be written as
  ...
  $-E(\partial^2 \ell(\boldsymbol{\beta}_k | \mathbf{y}) /
    \partial \boldsymbol{\eta}_k \partial \boldsymbol{\eta}_k^\top)$,
...
\item \emph{Hamiltonian Monte Carlo}: \label{sec:hmcmc} \\
