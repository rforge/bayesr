...
calls are dropped, as well as the corresponding \code{terms} object, the \code{model.matrix} of
parametric modeled covariates and the \code{smooth.construct} list of all smoothly modeled
covariates. The elements of \code{smooth.construct} represent one model term each, in this example
...
estimated parameters and other information about the optimizer convergence, information criteria, etc.
<<>>=
print(names(bo))
...
sampler simple trace plots can be created by \code{plot(bs)} or the samples can
...
object of \class{bamlss.results} for which generic plotting and summary functions are provided.
For example, using the samples from the MCMC run we plot the estimated functions by
<<eval=FALSE>>=
bf$samples <- bs
...
and the returning values of such functions to use the \pkg{bamlss} functionalities. E.g., using the
\pkg{JAGS} interface in package \pkg{bamlss}, the same model can be estimated by providing the
sampler function \fct{JAGS}
<<eval=FALSE>>=
...
Like other common \proglang{R} model fitting functions the user must specify the distributions to be
modeled, which are represented as family objects. The basic structure of such an \pkg{bamlss} family
object is described in Section~\ref{sec:families}, but since these objects are simply
passed to the optimizer or sampler functions, the model fitting functions itself must interpret
the information provided by such objects. However, there are minimum requirements to parse the
model frame with \fct{bamlss.frame}:
\begin{itemize}
\item The family object must return a \code{list} of class \class{family.bamlss}.
...
@
Internally, if a family function name ends with \code{.bamlss} it is
...
are provided within the family object, therefore, in \pkg{bamlss} the family objects can hold
...
\end{table}
Additionally, there is also a family transformer function \fct{tF} for family objects created for
the \proglang{R} package \pkg{gamlss}, e.g., the motorcycle model can be estimated with the \fct{NO} function of
\pkg{bamlss} by
<<eval=FALSE>>=
b <- bamlss(f, family = tF(NO), data = mcycle)
@
But note that some of the provided \pkg{gamlss} distributions need good staring value for
convergence.
