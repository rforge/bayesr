...
\item \emph{Random-walk Metropolis}: \label{sec:rwm} \\
  ...
  \text{min} \left\{ \frac{p(\boldsymbol{\vartheta}_{jk}^{\star} | \mathbf{y})}{
    p(\boldsymbol{\vartheta}_{jk}^{(t)} | \mathbf{y})}, 1 \right\}.
  $$
...
\item \emph{Derivative based Metropolis-Hastings}: \label{sec:dmh} \\
  ...
  $\boldsymbol{\Sigma}_{jk}$ during MCMC simulation might still lead to slow convergence of the
  ...
  of the log-posterior around the current iterate
  \begin{eqnarray*}
  ...
  $N(\boldsymbol{\mu}_{jk}^{(t)}, \boldsymbol{\Sigma}_{jk}^{(t)})$. Again, assuming a basis function approach with
  multivariate normal priors (\ref{eqn:shrinkprior}) the precision matrix is
  $$
...
\partial \boldsymbol{\eta}_k \partial \boldsymbol{\eta}_k^\top)$ and the mean can be written as
  ...
  \citep{bamlss:Gamerman:1997, bamlss:Brezger+Lang:2006}.
