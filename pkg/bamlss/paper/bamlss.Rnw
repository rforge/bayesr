...
\Abstract{
  ...
  existing software, is relatively easy.
}
...
bamlss:Belitz+Brezger+Klein+Kneib+Lang+Umlauf:2015}, \pkg{JAGS}~\citep{bamlss:Plummer:2013},
...
for the implemenation are presented. Finally, Section~\ref{sec:softex} introduces the corresponding
...
one covariate by applying tensor products of marginal design matrices \citep{bamlss:Wood:2006}.
...
$$
\boldsymbol{\beta}_{jk}^\top \left( \frac{1}{\tau_{1jk}^2} \mathbf{K}_{1jk} \otimes \mathbf{I}_{2jk} +
  \frac{1}{\tau_{2jk}^2} \mathbf{I}_{1jk} \otimes \mathbf{K}_{2jk} \right) \boldsymbol{\beta}_{jk} =
\boldsymbol{\beta}_{jk}^\top \mathbf{K}_{jk}
  \left(\frac{1}{\tau_{1jk}^2},\frac{1}{\tau_{2jk}^2} \right)\boldsymbol{\beta}_{jk},
$$
where $\mathbf{K}_{1jk}$ and $\mathbf{K}_{2jk}$ are the penalty matrices of the marginal effects
and $\tau_{1jk}^2$ and $\tau_{2jk}^2$ are variances that control the smoothness of the effect
...
structured additive predictor of nested covariates, e.g., including a discrete spatial effect
modeled by a Markov random field. This way, potential costly operations during estimation
...
actual number of observations. Moreover, the full conditionals for
...
see~\citet{bamlss:Klein+Kneib:2015} for details. If a simple Gibbs sampling step cannot be
derived, another strategy is to use slice sampling, since the number of smoothing variances is
...
the Bayesian information criterion (BIC) can be used for selecting variables and smoothing
variances $\boldsymbol{\tau}^2_{jk}$, amongst others. For the computation the effective degrees of
freedom need to be computed. Using nonlinear functions, for each model component these can be
approximated by the equivalent degrees of freedom
$$
\mathrm{edf}_{jk}(\boldsymbol{\tau}^2_{jk}) = \mathrm{trace}\left\{
  \mathbf{F}_{kk}(\boldsymbol{\beta}_{jk}(\boldsymbol{\tau}^2_{jk}))
  \mathbf{H}_{kk}(\boldsymbol{\beta}_{jk}(\boldsymbol{\tau}^2_{jk}))^{-1} \right\},
$$
with $\mathbf{F}_{kk}(\boldsymbol{\beta}_{jk}(\boldsymbol{\tau}^2_{jk})) =
\partial^2 \ell(\boldsymbol{\beta}(\boldsymbol{\tau}^2_{jk}); \mathbf{y}, \mathbf{X})/
\partial \boldsymbol{\beta}_{jk}(\boldsymbol{\tau}^2_{jk}) \partial
\boldsymbol{\beta}_{jk}(\boldsymbol{\tau}^2_{jk})^\top$.
The total degrees of freedom is then given by
...
types of models presented in this paper is gradient boosting. The boosting algorithm slightly
differs from classical boosting in so far, that in each boosting iteration for each parameter
...
model with the a time-constant predictor given by
$$
...
$$
Here, variable \code{arrivaltime} is the waiting time until the first fire engine arrives after the
...
effect modeling the deviations from the baseline. Both functions are modeled using penalized splines.
...
The estimated effects are shown in Figure~\ref{fig:londonfiredeffects}. The upper left panel
...
bounded, i.e., there is a natural limit for the benefit from opening new fire stations in the area.
The effect of the time of the day then indicates that in the morning hours around 4-5 am, as well
as in the afternoon around 14-16 pm, the risk of waiting longer for the fire engine to arrive
is increasing. The time-constant spatial effect clearly shows that the average risk of increased 
waiting times are higher in the city center and some smaller area in southern London. However, the 
estimated probabilities of waiting longer than six minutes around the center show moderate
variation, while the borders of London indicate higher probabilities as well as in the western
parts, most probably because of the lower fire station density in these areas.
