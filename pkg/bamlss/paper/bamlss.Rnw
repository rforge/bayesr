...
observations using censored regression and a Cox model for continuous time with space-time
interactions.
...
\readme{Z: The following last paragraph in this section seems to be more technical to me.
While the other equations above are relatively straightforward and IMO close enough to
``common knowledge'' for readers in the field, I'm not so sure about the next paragraph.
The main idea is also straightforward but the spectral decomposition and null space etc.\
were new to me. Maybe this can be simplified or the technical details deferred?}
...
\readme{Z: Maybe ``Response and posterior distribution''?}

\subsection{Posterior distribution} \label{sec:posterior}

\readme{Z: Possibly using $d_y$ instead of $d$ would be more consistent with the notation
for the priors?}
...
\readme{Z: Added ``either one of'' in the sentence below. OK?}
...
\end{enumerate}
%
\fixme{Z: My impression would be that using E1 and E2 for ``estimation'' than (a) and (b) would
be easier to remember throughout the paper.}
%
...
\end{equation}
%
\readme{Z: Maybe use ``u()'' rather than ``U()'' as most (or all?) functions in the manuscript are
denoted with lower-case letters?}
%
where function $U( \cdot )$ is an updating function, e.g., for generating one Newton-Raphson step
in \ref{item:a} or getting the next step in an MCMC simulation in \ref{item:b}, amongst others. The
...
for different model terms, e.g., in problem \ref{item:a} some updating functions could be based on iteratively
weighted least squares (IWLS) and some on ordinary Newton-Raphson steps (see, e.g.,
example Section~\ref{sec:coxreg}). In problem \ref{item:b} using MCMC simulation it is common to
mix between several sampling methods depending on the type of model term or distribution parameter.

\fixme{Z: The reading flow for the verbal description of Algorithms 1, 2a, 2b could be improved.
Maybe add some more overview description (1--2 sentences) in the next paragraph?}

\fixme{Z: Try to force Algorithms 1, 2a, and 2b onto the same page using p!}
...
Some more brief verbal comments about what the algorithm does and how it proceeds
would be useful in the reading flow. One or two sentences should suffice already, I guess.}
...
\item \label{item:1} The density
  $d(\mathbf{y} | \boldsymbol{\theta}_1, \ldots, \boldsymbol{\theta}_K)$ and respective
...
\ref{item:6a} and \ref{item:7a} need to be supplied for implementing a new model.

\fixme{Z: The sentence above is the story that Klein/Kneib/Lang usually tell because they
convey that all relevant links and model terms are already implemented. While I agree that this
is often true, I don't think that it is really always true and hence I don't think we should
pitch the story this way. Instead, we could say that extending the toolbox can be done in different
directions, e.g.: For a new response distribution, only building block B1 and possibly B6b and
B7b are necessary. For a new model term B3 and B5 are needed. And for a new link function
B2 and B4. And then such new building blocks are straightforward to combine with other
previously available building blocks.}

As shown in the upcoming this is relatively straightforward in most cases since \ref{item:6a} and \ref{item:7a} can
be simplified when fragmenting with the chain rule leading to updating functions that only require
\ref{item:6b} and \ref{item:7b} to be implemented. Moreover, most parts that are used for solving
...
choice, Bayesian inference and prediction. \fixme{Z: Within this last paragraph also try
to highlight which B* building blocks are discussed subsequently. And then in the subsequent
discussions refer back to the B* where appropriate.}
...
considered. Hence, the corresponding priors have to be proposed. \fixme{Z: It is not clear to me what
the sentence before means. Maybe rephrase?} Another question is the actual
...
$y~\sim~\mathcal{N}(\mu = \eta_{\mu}, \log(\sigma) = \eta_{\sigma})$ is then given by
\begin{center}
...
any number of parameters (model parts) that should be modeled by explanatory variables.
...
statistics, visualization, prediction etc.
...
provides infrastructures that builds all necessary model matrices and information from which it is
...
functions only a simple general format of function arguments and return values must be adhered, see
also the documentation of \pkg{bamlss}.
...
efficient algorithm see \citet{bamlss:Lang+Umlauf+Wechselberger+Harttgen+Kneib:2014}. For usage
within \pkg{bamlss} see also the documentation of estimation engines \fct{bfit} and \fct{GMCMC} and
...
the world. Each year, the LFB is
...
area. This is viewed as an optimization problem in \citet{bamlss:Taylor:2015}, i.e., he treats
the question of possible improvements of response times according to space in a probabilistic way.
Therefore, he analyzes the effect of fire station closures in 2014 using a parametric proportional
...
The data is freely available at the London DataStore (\url{http://data.london.gov.uk/}) with a UK
...
As mentioned above, \citet{bamlss:Taylor:2015} analyzes the response times within a
survival context, which seems natural for this kind of time-to-event data. More precisely, he
describes the hazard of an event (the fire engine arrives) at time $t$ with a relative risk model of
the form
$$
...
\end{equation}
which is of major interest in this analysis. Based on (\ref{eqn:surv}), for full Bayesian inference
...
\begin{enumerate}[leftmargin=*,align=left]
\item[\ref{item:1}.] The log-likelihood function of the continuous time Cox-model is then given by
  $$
  \ell(\boldsymbol{\beta}; \mathbf{y}, \mathbf{X}) =
...
\item[\ref{item:7a}.] The elements of the Hessian w.r.t.\ $\boldsymbol{\beta}_{\lambda}$ are
  ...
  effects, see also \citet{bamlss:Hofner:2008} Section~5.2 for a detailed discussion. Therefore
...
time-constant updating functions for $\eta_{\gamma}$. Secondly, a specific hurdle of the continuous
time Cox-model is the computation of the integrals, because these do not have a closed form solution
and need to be approximated numerically, e.g., by the trapezoidal rule or Gaussian quadrature.
Moreover, it is inefficient to compute the integrals a new for every updating step, since for the
