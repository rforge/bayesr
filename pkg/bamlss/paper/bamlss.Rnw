...
if(!file.exists("firemodel.rda")) {
  ...
  firemodel <- bamlss(f, data = LondonFire, family = "cox", subdivisions = 10,
    n.iter = 12000, burnin = 2000, thin = 10, nu = 0.01, maxit = 1000)
  ...
  firemodel_plotdata <- data_basehaz(80, 10)
...
plot_basehaz <- function(data, pos = "topright", mcolor = diverge_hcl, curve = TRUE, ...)
{
  if(curve) {
    par(mfrow = c(1, 2))
  ...
  xymap(lon, lat, pspatial, data = data, pos = pos,
    layout = FALSE, map = FALSE, add = curve, color = mcolor)
