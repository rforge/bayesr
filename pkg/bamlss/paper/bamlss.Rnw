...
\item \emph{Random walk Metropolis}: \label{sec:rwm} \\
  ...
  random walk Metropolis (RWM). The sampler
  proceeds by drawing a candidate $\boldsymbol{\vartheta}_{jk}^{\star}$ from a symmetric jumping
  distribution $q(\boldsymbol{\vartheta}_{jk}^{\star}| \boldsymbol{\vartheta}_{jk}^{(t-1)})$, the
  candidate is then accepted as the new state of the Markov chain with probability
  $$
  \alpha\left( \boldsymbol{\vartheta}_{jk}^{\star} | \boldsymbol{\vartheta}_{jk}^{(t-1)}\right) =
  \text{min} \left\{ \frac{p(\boldsymbol{\vartheta}_{jk}^{\star} | \mathbf{y})}{
    p(\boldsymbol{\vartheta}_{jk}^{(t-1)} | \mathbf{y})}, 1 \right\}.
  $$
  Commonly, the jumping distribution is a normal distribution $N(\boldsymbol{\vartheta}_{jk}^{(t-1)}, \boldsymbol{\Sigma}_{jk})$ centered at the current iterate and fixed covariance matrix. Although this algorithm
...
\item \emph{Derivative based Metropolis-Hastings}: \label{sec:dmh} \\
  ...
  $N(\boldsymbol{\vartheta}_{jk}^{(t-1)}, \boldsymbol{\Sigma}_{jk})$ is to use the local curvature
  information
  $$
  \boldsymbol{\Sigma}_{jk} = \frac{\partial^2 p(\boldsymbol{\vartheta} | \mathbf{y})}{
    \partial \boldsymbol{\vartheta}_{jk}\boldsymbol{\vartheta}_{jk}^\top}
  $$
  ...
  $$
  p(\boldsymbol{\vartheta}_{jk} | \cdot) \propto \ell(\mathbf{y} | \boldsymbol{\vartheta}_{jk}) \cdot
    p_{jk}(\boldsymbol{\vartheta}_{jk})
  $$
  which approximate the posterior at the current iterate. A second order Taylor series expansion of
  the posterior yields
  $$
  p(\boldsymbol{\vartheta}_{jk}) \approx 
  $$
