...
\pkg{bamlss}. Unlike other software packages for regression modeling, the design really focuses on
maximum flexibility and easy integration of new code and/or standalone systems, hence, the basic
idea even goes beyond the concepts presented in the above sections.
