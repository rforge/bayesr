...
(\ref{eqn:logpost}), which has an unique (sometimes analytical) solution in a lot of cases,
...
(\ref{eqn:logpost}) prohibits simultaneous estimation of $\boldsymbol{\beta}_{jk}$ and
$\boldsymbol{\tau}^2_{jk}$ and $\hat{\boldsymbol{\tau}}^2_{jk}$ is usually derived by additionally
...
and a particularly convenient way to maximize (\ref{eqn:logpost}) is the method of scoring
\begin{equation} \label{eqn:scoring}
\boldsymbol{\vartheta}^{(t + 1)} = \boldsymbol{\vartheta}^{(t)} -
...
$\boldsymbol{\eta}_k$ with respect to coefficients $\boldsymbol{\vartheta}_k$. In similar fashion
components of $\mathbf{H}_{ks}$ including $\ell(\boldsymbol{\vartheta} | \mathbf{y})$ can be written as
...
\end{equation}
for $k = 1, \dots, K$ and $s = 1, \dots, K$, where the second term drops if functions
(\ref{eqn:functions}) can be written as a linear combination of a design matrix and coefficients.
