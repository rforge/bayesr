...
posterior mode estimation or MCMC, a simple generic algorithm for BAMLSS models is outlined in
Algorithm~\ref{fig:algodesign}.
\begin{algorithm}
...
\REQUIRE{$\boldsymbol{\beta}$, $\boldsymbol{\eta}$, $\boldsymbol{\tau}$, e.g.,
  $\boldsymbol{\beta} = \mathbf{0}, \boldsymbol{\tau} = 0.001 \cdot \mathbf{1}$, $\Delta = \varepsilon + 1$.}
\WHILE{$(\Delta > \varepsilon) \, \& \, (t < T)$}
  \STATE set $\boldsymbol{\eta}_0 = \boldsymbol{\eta}^{(t)}$.
  \FOR{$k = 1$ to $K$}
    \FOR{$j = 1$ to $J_k$}
    ...
    \ENDFOR
  \ENDFOR
  \STATE compute $\Delta \leftarrow \text{rel.change}(\boldsymbol{\eta}_0, \boldsymbol{\eta}^{(t + 1)})$.
  \STATE increase $t \leftarrow t + 1$.
\ENDWHILE
\renewcommand{\algorithmicensure}{\textbf{Output:}}
\ENSURE{posterior mode estimates $\hat{\boldsymbol{\beta}}$, $\hat{\boldsymbol{\tau}}$ or
  MCMC samples $\boldsymbol{\beta}^{(t)}$, $\boldsymbol{\tau}^{(t)}$, $t = 1, \ldots, T$.}
\end{algorithmic}
\end{algorithm}
