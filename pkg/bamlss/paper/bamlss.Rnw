...
Instead, applying full Bayesian inference is relatively easy by direct computation of desired
...
From the above, it can be recognized that the following ``lego bricks'' are repeatedly used within
BAMLSS candidate algorithms:
\begin{itemize}
\item The density function (\ref{eqn:density}),
\item the first order derivatives of $\boldsymbol{\beta}_k$ the log-likelihood (\ref{eqn:score2}),
\item the second order derivatives of $\boldsymbol{\beta}_k$ the log-likelihood (\ref{eqn:hessian2}),
\item derivatives for log-priors, e.g., $\log \, p(\boldsymbol{\beta}_{jk} | \tau_{jk}^2)$ and $\log \, p(\tau_{jk}^2)$.
\end{itemize}
As shown in Section~\ref{sec:postmode}, first and second order derivatives of $\boldsymbol{\beta}_k$
can in most cases be computed by fragmenting with the chain rule into derivatives of the
log-likelihood with respect to parameters $\boldsymbol{\theta}_k$, derivatives of link functions
with respect to the linear predictors $\boldsymbol{\eta}_k$ and derivatives of $\boldsymbol{\eta}_k$
with respect to the regression coefficients $\boldsymbol{\beta}_k$. Hence, most of the structures
can be reused when implementing new models and/or algorithms, i.e., applying the backfitting
algorithm (\ref{eqn:blockbackfit}) and derivative based MCMC as shown in Section~\ref{sec:dmh}
usually only requires the calculation of derivates of the log-likelihood with respect to
parameters $\boldsymbol{\theta}_k$ anew.
...
a simple generic algorithm for BAMLSS models is outlined by the following pseudo code:
\begin{center}
...
functions could be different for each function $f_{jk}( \cdot )$, incorporating even more flexibility
to the modeling system.
