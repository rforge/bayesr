...
For simple linear effects a common choice of $\pi_{jk}( \cdot )$ is to use a non-informative
...
\pi_{jk}(\boldsymbol{\beta}_{jk}; \boldsymbol{\tau}_{jk}, \boldsymbol{\alpha}_{jk}) \propto
  \exp \left(- \frac{1}{2}(\boldsymbol{\beta}_{jk} -
    ...
    \mathbf{m})\right),
\end{equation}
where $\boldsymbol{\alpha}_{jk} = \{\boldsymbol{\alpha}_{\boldsymbol{\beta}_{jk}} = \{\mathbf{m}\}\}$
...
$\mathbf{m} = \mathbf{0}$ and a large variance.
