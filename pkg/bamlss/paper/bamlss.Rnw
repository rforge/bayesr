...
@
Argument \code{binning} is the switch in function \fct{bamlss.frame} for applying the shrinkage of
the design matrices, as described above, moreover if \code{binning} is an \class{integer} all
covariates in the data set will be rounded to this decimal places.
