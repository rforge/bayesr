...
\section{Lego bricks} \label{sec:algorithms}
...
\item \emph{Slice sampling}: \label{sec:smcmc} \\
  ...
  generated by:
  \begin{enumerate}
  \item Sample $h \sim \mathcal{U}(0, p(\beta_{ijk}^{(t)} | \, \cdot))$.
...
\ENDFOR
\STATE Update $\boldsymbol{\beta}_{jk}^{(t + 1)} \leftarrow U_{jk}(\boldsymbol{\beta}_{jk}^{(t + 1)}, \boldsymbol{\tau}_{jk}^{(t + 1)} | \, \cdot)$.
\renewcommand{\algorithmicensure}{\textbf{Output:}}
...
is relatively easy to implement algorithms for a variety of models. In Table~\ref{tab:bricks}
we give examples of the contents of bricks for a couple of models.
\begin{sidewaystable}
\centering
\begin{tabular}{llllll}
\hline
Name & $\ell(\boldsymbol{\beta}; \mathbf{y}_i, \mathbf{x}_i)$ &
  $\partial \ell(\boldsymbol{\beta}; \mathbf{y}, \mathbf{X}) / \partial \boldsymbol{\beta}_{jk}$ &
  $\partial^2 \ell(\boldsymbol{\beta}; \mathbf{y}, \mathbf{X}) / \partial \boldsymbol{\beta}_{jk}\partial \boldsymbol{\beta}_{jk}^\top$ &
  $\mathbf{u}_k$ & $\mathbf{W}_{kk}$ \\ \hline
Beta \\
Binomial \\
Censored normal \\
Cox-model & $\delta\eta_{\gamma} - \int_0^{t} \exp( \eta_{\lambda}(u)du )$ \\
Gaussian & $-\frac{1}{2}\log(2\pi) - \eta_{\sigma} - \frac{(y - \eta_{\mu})^2}{2\exp(\eta_{\sigma})^2}$ \\
Gamma \\
Multinomial \\
Multivariate normal \\
Poisson \\ \hline
\end{tabular}
\caption{\label{tab:bricks} Examples of ``Lego bricks'' for building BAMLSS and beyond.}
\end{sidewaystable}
...
the density function $g(y ; \theta_{i} = h_1^{-1}(\eta_{1}(\mathbf{x}, \boldsymbol{\beta}_1)), \ldots,
...
As already noted in Section~\ref{sec:algorithms}, e.g., in the case of IWLS backfitting and derivative
based MCMC simulation, implementing a new distribution most of the times only requires to compute
derivatives of the log-likelihood w.r.t.\ to parameters $\boldsymbol{\theta}_k$. Hence, a modular
family description system provides functions for first and second order derivatives which take
...
$\mathbf{y} \sim \mathcal{N}(\boldsymbol{\mu}, \boldsymbol{\sigma}^2)$ can then be summarized in
...
(see Section~\ref{sec:algorithms}~and~\ref{sec:families}) to be used for the IWLS updating
...
As a result, applying the generic algorithm presented in Section~\ref{sec:algorithms} to this type
