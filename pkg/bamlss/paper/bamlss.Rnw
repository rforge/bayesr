...
of models. This huge amount of flexibility is of course not without its drawbacks, e.g., estimation
...
In contrast, this paper aims to contribute to algorithmic flexibility by presenting iterative
estimation algorithms with exchangeable updating functions. Moreover, the structures exploited in
the last three sections suggest a unified modeling architecture that is useful for building
algorithms and writing interfaces to existing implementations.
...
the density function $d(y ; \theta_{i} = h_1^{-1}(\eta_{1}(\mathbf{x}, \boldsymbol{\beta}_1)), \ldots,
...
As already noted in Section~\ref{sec:legobricks}, e.g., in the case of IWLS backfitting and derivative
