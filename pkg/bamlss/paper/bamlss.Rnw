...
scale and shape (BAMLSS) and beyond. The concept can be exploited in two ways: First, to develop
new algorithms and code, secondly, to easily integrate existing implementations. A prove of
concept is given in the corresponding \proglang{R} implementation \pkg{bamlss} which is
available on \pkg{Rforge}.
...
in (a) or getting the next step in a MCMC simulation in (b). The updating scheme can be partitioned
into separate updating equations using leapfrog or zigzag iteration (see, e.g.,
...
updates for $k$-th parameter are computed while holding all other parameters
...
structure, e.g., in MCMC simulation it is common to mix between several sampling methods depending
on the type of model term. Hence, by using highly modular systems like (\ref{eqn:blockupdate}) and
(\ref{eqn:blockblockupdate}) it is possible to develop flexible estimation algorithms for numerous
...
\pi_{jk}(\boldsymbol{\beta}_{jk}; \boldsymbol{\tau}_{jk}, \boldsymbol{\alpha}_{jk}) \propto
  \exp \left(- \frac{1}{2}(\boldsymbol{\beta}_{jk} -
    \mathbf{m})^{\top}diag(\boldsymbol{\tau}_{jk})^{-1}(\boldsymbol{\beta}_{jk} -
    \mathbf{m})\right),
\end{equation}
where $g_{\boldsymbol{\tau}_{jk}}( \cdot ) = 1$ and
$\boldsymbol{\alpha}_{jk} = \{\boldsymbol{\alpha}_{\boldsymbol{\beta}_{jk}} = \{\mathbf{m}\}\}$
with $\mathbf{m}$ as a prior mean for $\boldsymbol{\beta}_{jk}$ and $diag(\boldsymbol{\tau}_{jk})$
is a prior covariance matrix. In a lot of applications a vague prior specification is used with
$\mathbf{m} = \mathbf{0}$ and a large variance (see, e.g., \citealp{bamlss:Fahrmeir+Kneib+Lang+Marx:2013}).
...
the usual choice of prior $\pi_{jk}( \cdot )$ is based on multivariate normal for
...
(see, e.g., \citealp{bamlss:Wood:2006}).
...
$\mathbf{a}_{jk}=-\mathbf{1}$ or $\mathbf{a}_{jk} = -1/2\mathbf{1}$ yields flat priors for
...
\subsection{Response distribution} \label{sec:bricksdensity}
...
For models involving shrinkage priors, e.g., given by (\ref{eqn:mvnormprior}), the posterior mode is
...
priors (\ref{eqn:mvnormprior}), the Hessian matrix in (\ref{eqn:blocknewton}) is given by
$$
...
Similarly to updating scheme (\ref{eqn:blockupdate}) we consider samples that are drawn for blocks of
...
\item \emph{Derivative based Metropolis-Hastings}: \label{sec:dmh} \\
  ...
  function approach with multivariate normal priors (\ref{eqn:mvnormprior}) the precision matrix is
  $$
...
E.g., this is the case when using an inverse Gamma prior (\ref{eqn:igprior}) in combination with
the normal prior (\ref{eqn:mvnormprior}), then $p(\tau_{jk} | \cdot)$ is again an inverse Gamma
distribution with
$$
...
(\ref{eqn:mvnormprior}) the matrix is $\mathbf{G}_{jk}(\tau^2_{jk}) = \tau_{jk}^{-2}\mathbf{K}_{jk}$. The
...
Moreover, using the ideas of partitioned updating (\ref{eqn:blockupdate}) and blockwise MCMC sampling,
