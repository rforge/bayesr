...
climatology for January 10th. The plot clearly shows the influence of elevation, while alpine
regions have an predicted precipitation average of about $1$mm, the lower regions have basically
zero predicted precipitation.
\begin{figure}[t!]
\centering
...
that it is more difficult for a fire engine to arrive at the scene in rush hours. To answer the
question of spatially driven hazards, a spatial effect of longitude and latitude coordinates
is included in the model. All functions $f_1, \ldots, f_3$ are assumed to be possibly nonlinear
and are modeled using penalized splines.
