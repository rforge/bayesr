...
smooth model terms are denoted with \code{s()}. A model with two additional nonlinear modeled terms
...
sampler function for full Bayesian inference with MCMC using using Algorithm~\ref{fig:algodesign}
in combination with \ref{fig:mcmc}, which uses posterior mode estimates as staring values. Note that
each, the optimizer and the sampler function, can be exchanged in this concept, i.e., this way it is
possible to write a number of different optimizer and sampler functions, e.g., for specific
problems.
...
assumed to be more or less stable such that no extra coding is required.
...
Estimation is performed by an \code{optimizer} and \code{sampler} function, which can be provided by
...
\section{Illustratiuons} \label{sec:illustrations}
