...
$\boldsymbol{\tau}_{jk}$ and prior density $g_{\boldsymbol{\tau}_{jk}}( \cdot ) \propto 1$.
Moreover, the structure of (\ref{eqn:logpost}) prohibits simultaneous estimation of
$(\boldsymbol{\beta}_{jk}^\top, (\boldsymbol{\tau}_{jk})^\top)^\top$ and the
estimator $\hat{\boldsymbol{\tau}}_{jk}$ is usually derived by additionally
...
a more detailed discussion on smoothing parameter estimation. In Section~\ref{sec:varselpostmode},
we briefly discuss a stepwise approach for smoothing parameter selection. For developing general
updating functions we describe posterior mode estimation for the case of fixed and known
parameters $\boldsymbol{\tau}_{jk}$, because these updating function form the basis of estimation
algorithms for $\boldsymbol{\tau}_{jk}$. Estimation of
$\boldsymbol{\beta} = (\boldsymbol{\beta}_1^\top, \ldots, \boldsymbol{\beta}_K^\top)^\top$
...
\end{equation}
where the second term cancels out if all functions (\ref{eqn:functions}) can be written as a linear
...
$$
\mathbf{G}_{jk} = \frac{\partial^2 \log \,
  \pi_{jk}(\boldsymbol{\beta}_{jk}; \boldsymbol{\tau}_{jk},
  \boldsymbol{\alpha}_{jk})}{\partial \boldsymbol{\beta} \partial \boldsymbol{\beta}^\top}.
$$
Typically, using priors based on multivariate normal kernels (\ref{eqn:mvnormprior}) matrices
$\mathbf{G}_{jk}$ are a simple product of smoothing variances and penalty matrices.
...
\begin{equation} \label{eqn:blockbackfit}
  ...
  = U_{jk}(\boldsymbol{\beta}_{jk}^{(t)} | \, \cdot ) =
    (\mathbf{X}_{jk}^\top\mathbf{W}_{kk}\mathbf{X}_{jk} +
      \mathbf{G}_{jk})^{-1}\mathbf{X}_{jk}^\top\mathbf{W}_{kk}(
...
$\mathbf{z}_k - \boldsymbol{\eta}_{k, -j}^{(t+1)}$ can be used.
...
\section{Posterior mode updating based on IWLS}
\label{appendix:pmodeiwls}

The following presents the steps needed to derive the iterative updating scheme based on IWLS in
...
\section{Full conditionals for derivative based MCMC}
\label{appendix:fullcond1}
The following the describes the steps to derive a multivariate normal jumping distribution based
