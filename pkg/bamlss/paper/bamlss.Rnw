...
\begin{itemize}
  \item Linear effects of continuous covariates: $f_j(\mathbf{x}) = x_1$.
  \item Nonlinear effects of continuous covariates: $f_j(\mathbf{x}) = f(x_1)$.
  \item Two-dimensional surfaces: $f_j(\mathbf{x}) = f(x_1, x_2)$.
  \item Spatially correlated effects: $f_j(\mathbf{x}) = f_{spat}(x_s)$.
  \item Varying coefficients: $f_j(\mathbf{x}) = x_1f(x_2)$.
  \item Spatially varying effects: $f_j(\mathbf{x}) = x_1f_{spat}(x_s)$ or
    $f_j(\mathbf{x}) = x_1f(x_2, x_3)$.
