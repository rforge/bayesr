...
one parameter is maximized holding the other parameters fixed. Then, applying a Newton-Raphson
algorithm, the updating scheme for parameter $k$ can be written as
\begin{equation} \label{eqn:blocknewton}
\boldsymbol{\vartheta}_k^{(t + 1)} = U_k(\boldsymbol{\vartheta}^{(t)} | \cdot) = \boldsymbol{\vartheta}_k^{(t)} -
  \mathbf{H}_{kk}\left( \boldsymbol{\vartheta}_k^{(t)} \right)^{-1}\mathbf{s}_k\left( \boldsymbol{\vartheta}_k^{(t)} \right).
\end{equation}
Moreover, note that this updating scheme can be further partitioned for each function within
parameter block $k$, leading to a highly modular system.

The hessian matrix in (\ref{eqn:blocknewton}) can be written as
$$
\mathbf{H}_{kk}\left( \boldsymbol{\vartheta}_k^{(t)} \right) =
\begin{pmatrix}
\mathbf{Z}_{1k}^\top\mathbf{W}_{kk}\mathbf{Z}_{1k} + \mathbf{G}_{1k} &
  \cdots & \mathbf{Z}_{1k}^\top\mathbf{W}_{kk}\mathbf{Z}_{p_kk} \\
\vdots & \ddots & \vdots \\
\mathbf{Z}_{p_kk}^\top\mathbf{W}_{kk}\mathbf{Z}_{1k} & \cdots & \mathbf{Z}_{p_kk}^\top\mathbf{W}_{kk}\mathbf{Z}_{p_kk} + \mathbf{G}_{p_kk}
\end{pmatrix},
$$
with diagonal weight matrix $\mathbf{W}_{kk} = -diag(\partial^2 \ell(\boldsymbol{\vartheta}_k | \mathbf{y}) /
\partial \boldsymbol{\eta}_k \partial \boldsymbol{\eta}_k^\top)$ derivatives
$\mathbf{Z}_{jk} = \partial \boldsymbol{\eta}_k / \partial \boldsymbol{\vartheta}_{jk}$, where
$\mathbf{G}_{jk}$ is the derivative resulting from priors $p_{jk}(\cdot)$ and specific functional
types (second term in \ref{hessian2}). Similarly, the score is
$$
\mathbf{s}_k\left( \boldsymbol{\vartheta}_k^{(t)} \right) =
\begin{pmatrix}
\mathbf{Z}_{1k}^\top \mathbf{u}_k^{(t)} + \mathbf{Q}_{1k} \\
\vdots \\
\mathbf{Z}_{p_kk}^\top \mathbf{u}_k^{(t)} + \mathbf{Q}_{p_kk} \\
\end{pmatrix}
$$
and $\mathbf{u}_k = \partial \ell(\boldsymbol{\vartheta}_k | \mathbf{y}) /
\partial \boldsymbol{\eta}_k$ with $\mathbf{Q}_{jk} = \partial p_{jk}(\boldsymbol{\vartheta}_{jk}) /
\partial \boldsymbol{\vartheta}_{jk}$.

\begin{eqnarray*}
(\mathbf{Z}_{1k}^\top\mathbf{W}_{kk}\mathbf{Z}_{1k} + \mathbf{G}_{1k})\boldsymbol{\vartheta}_{1k}^{(t+1)} +
\ldots + \mathbf{Z}_{1k}^\top\mathbf{W}_{kk}\mathbf{Z}_{p_kk}\boldsymbol{\vartheta}_{p_kk}^{(t + 1)} - \\
(\mathbf{Z}_{1k}^\top\mathbf{W}_{kk}\mathbf{Z}_{1k} + \mathbf{G}_{1k})\boldsymbol{\vartheta}_{1k}^{(t)} -
\ldots - \mathbf{Z}_{1k}^\top\mathbf{W}_{kk}\mathbf{Z}_{p_kk}\boldsymbol{\vartheta}_{p_kk}^{(t)}
\end{eqnarray*}
\begin{eqnarray*}
\mathbf{G}_{1k}(\boldsymbol{\vartheta}_{1k}^{(t+1)} - \boldsymbol{\vartheta}_{1k}^{(t)}) +
  \mathbf{Z}_{1k}^\top\mathbf{W}_{kk}\mathbf{Z}_{1k}\boldsymbol{\vartheta}_{1k}^{(t+1)} + \ldots +
  \mathbf{Z}_{1k}^\top\mathbf{W}_{kk}\mathbf{Z}_{p_kk}\boldsymbol{\vartheta}_{p_kk}^{(t+1)} - \\
  \mathbf{Z}_{1k}^\top\mathbf{W}_{kk}\mathbf{Z}_{1k}\boldsymbol{\vartheta}_{1k}^{(t)} - \ldots -
  \mathbf{Z}_{1k}^\top\mathbf{W}_{kk}\mathbf{Z}_{p_kk}\boldsymbol{\vartheta}_{p_kk}^{(t)}
\end{eqnarray*}
\begin{eqnarray*}
\mathbf{G}_{1k}(\boldsymbol{\vartheta}_{1k}^{(t+1)} - \boldsymbol{\vartheta}_{1k}^{(t)}) +
  \mathbf{Z}_{1k}^\top\mathbf{W}_{kk}(\tilde{\boldsymbol{\eta}}_k^{(t+1)} - \tilde{\boldsymbol{\eta}}_k^{(t)}) 
\end{eqnarray*}
...
$$
\begin{pmatrix}
\mathbf{X}_{1k}^\top\mathbf{W}_{kk}\mathbf{X}_{1k} + \frac{1}{\tau_{p_kk}^2}\mathbf{K}_{1k} &
  \cdots & \mathbf{X}_{1k}^\top\mathbf{W}_{kk}\mathbf{X}_{p_kk} \\
\vdots & \ddots & \vdots \\
\mathbf{X}_{p_kk}^\top\mathbf{W}_{kk}\mathbf{X}_{p_kk} & \cdots & \mathbf{X}_{p_kk}^\top\mathbf{W}_{kk}\mathbf{X}_{p_kk} + \frac{1}{\tau_{p_kk}^2}\mathbf{K}_{p_kk}
\end{pmatrix}^{(t)}
\begin{pmatrix}
\boldsymbol{\vartheta}_{1k}^{(t + 1)} - \boldsymbol{\vartheta}_{1k}^{(t)} \\
\vdots \\
\boldsymbol{\vartheta}_{p_kk}^{(t + 1)} - \boldsymbol{\vartheta}_{p_kk}^{(t)}
\end{pmatrix}
= \begin{pmatrix}
\mathbf{X}_{1k}^\top\mathbf{s}_k - \frac{1}{2\tau_{1k}^2}\mathbf{K}_{1k}\boldsymbol{\beta}_{1k}^{(t)} \\
\vdots \\
\mathbf{X}_{p_kk}^\top\mathbf{s}_k - \frac{1}{2\tau_{p_kk}^2}\mathbf{K}_{p_kk}\boldsymbol{\beta}_{p_kk}^{(t)}
\end{pmatrix},
$$
where $\mathbf{s}_k = \partial \ell(\boldsymbol{\vartheta} | \mathbf{y}) / \partial \boldsymbol{\eta}_k$.


Maximum likelihood and other gradient based algorithms require the evaluation of the
...
\begin{equation}
\frac{\partial \ell(\boldsymbol{\vartheta} | \mathbf{y})}{\partial \boldsymbol{\vartheta}_k} =
  \frac{\partial \ell(\boldsymbol{\vartheta} | y_i)}{\partial \boldsymbol{\eta}_k}
  \frac{\partial \boldsymbol{\eta}_k}{\partial \boldsymbol{\vartheta}_k} = 
  \frac{\partial \ell(\boldsymbol{\vartheta} | y_i)}{\partial \boldsymbol{\theta}_k}
  \frac{\partial \boldsymbol{\theta}_k}{\partial \boldsymbol{\eta}_k}
  \frac{\partial \boldsymbol{\eta}_k}{\partial \boldsymbol{\vartheta}_k}.
\end{equation}
...
iteration $t$ an adjusted dependent variable
\begin{equation} \label{eqn:z}
\mathbf{x}_k^{[t]} = \boldsymbol{\eta}_k^{[t]} + \left(\mathbf{W}_{kk}^{[t]}\right)^{-1}\mathbf{s}_k^{[t]}
\end{equation}
is computed, with vector
$\mathbf{s}_k = \partial \ell(\boldsymbol{\vartheta} | \mathbf{y}) / \partial \boldsymbol{\eta}_k$
and weights
$\mathbf{W}_{kk} = - \partial^2 \ell(\boldsymbol{\vartheta} | \mathbf{y}) / \partial \boldsymbol{\eta}_k \partial \boldsymbol{\eta}_k^\top$. Because the working observations follow
\begin{equation}
\mathbf{x}_k^{[t]} \sim N\left(\boldsymbol{\eta}_k^{[t]}, \left(\mathbf{W}_{kk}^{[t]}\right)^{-1}\right),
\end{equation}
...
$\mathbf{W}_{kk} = - E\left(\partial^2 \ell(\boldsymbol{\vartheta} | \mathbf{y}) / \partial \boldsymbol{\eta}_k \partial \boldsymbol{\eta}_k^\top\right)$.
