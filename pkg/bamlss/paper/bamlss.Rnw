...
approximated numerically, e.g., by the trapezoidal rule or Gaussian quadratur. Therefore, it
is inefficient to compute the integrals a new for every update, since for the time-constant part
the integrals given in $\mathbf{P}$ do not change anymore.
...
\fct{bfit} and \fct{GMCMC} can be reused and only need slight adaption. The corresponding
bamlss family object \fct{cox.bamlss} then sets the two estimation engines as the default
model fitters.
...
(see the documentation of the \pkg{survival} package, \citealp{bamlss:Therneau:2016}).
