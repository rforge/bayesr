...
\item \emph{Derivative based Metropolis-Hastings}: \label{sec:dmh} \\
  ...
  $$
  \mathbf{P}_{jk} = -\mathbf{H}_{kk}\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right),
  \qquad \mathbf{m}_{jk} = \mathbf{P}^{-1}_{jk}\left( s\left(\boldsymbol{\vartheta}_{jk}^{(t)}\right) -
  ...
  $N(\mathbf{m}_{jk}, \mathbf{P}_{jk}^{-1})$. Again, assuming a basis function approach with
  multivariate normal priors (\ref{eqn:shrinkprior}) we obtain
  $$
  \mathbf{P}_{jk} = \mathbf{X}_{jk}^\top\mathbf{W}_{kk}\mathbf{X}_{jk} + \mathbf{G}_{jk}
  $$
