...
if(!file.exists("figures/firemodel-data.png")) {
  ...
  hex_plot <- function(n = 600, FUN = NULL,
    color = heat_hcl, symmetric = FALSE, swap = TRUE, ...)
  {
    library("bamlss")
    library("maptools")
    library("rgeos")
    ...
    LBP <- spTransform(LondonBoundaries, CRS("+init=epsg:27700"))
    HexPts <- spsample(LBP, n = n, type = "hexagonal")
    HexPols <- HexPoints2SpatialPolygons(HexPts)
    HexPols <- spTransform(HexPols, CRS("+init=epsg:4326"))
    clip <- gIntersection(LondonBoundaries, HexPols, byid = TRUE, drop_lower_td = TRUE)
    ...
    plot(agg, col = colors$map(agg$arrivaltime), add = TRUE)
