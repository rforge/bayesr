...
responsible fire station to the site of how much damage can be prevented, moreover, in some
cases it may happen that fire trucks are already in use at another fire scenery and therefore it
also depends on the number of fire stations in the area. This is viewed as an optimization problem
in \citet{bamlss:Taylor:2015}, i.e., they treated the question if there is possible improvement
of response times according to space in a probabilistic way.

They analyzed the response times using a survival model
