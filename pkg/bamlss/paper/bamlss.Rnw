...
Using the flexible partitioned updating systems like (\ref{eqn:blockblockupdate}), from
Section~\ref{sec:priorsbricks} to \ref{sec:infpred}, it can be recognized that the following
...
after samples for $\boldsymbol{\beta}_{jk}$.
\begin{algorithm}
