...
\Abstract{
  ...
  existing software, is relatively easy. The examples illustrate the usefulness of the approach on
  a very large climatology model for daily precipitation observations, as well as on a computational
  intensive spatial Cox-model for continuous time with over five thousand ``individuals''.
}
...
, i.e., are trying to prevent overfitting. Using nonlinear functions, for each model component, the
degrees of freedom used to estimate the function can be approximated by the equivalent degrees of
freedom (edf) calculated by
$$
...
(BAMLSS). We highlight the similarities between optimization and sampling concepts and outline
a generic architecture that utilizes repeated building blocks in order to facilitate solutions
