...
p(\tau_{jk}) = \frac{2}{\pi \, A(1 + (\tau_{jk} / A)^2) }, \quad A > 0.
\end{equation}
For $A \rightarrow \infty$ the prior is uniform, hence large values result in weakly informative
priors for $\tau_{jk}$. Note that within this notion a ridge penalty
...
if(!file.exists("rainmodel.rda") & FALSE) {
  ...
  rainmodel <- bamlss(f, data = homstart2, family = "cnorm",
    ...
    binning = TRUE, before = FALSE, cores = 7, sampler = FALSE)
