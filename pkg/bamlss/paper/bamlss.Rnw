...
the density function $f(\mathbf{y} | \boldsymbol{\theta}_1, \ldots, \boldsymbol{\theta}_K)$ or its
logarithm given the parameters $\boldsymbol{\theta}_k$ as well as the involved link functions.
