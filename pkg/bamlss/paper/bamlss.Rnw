...
\item \emph{Random walk Metropolis}: \label{sec:rwm} \\
  Probably the most important algorithm is random walk Metropolis (RWM). The sampler
  ...
  \begin{equation} \label{eqn:accprob}
  ...
  the scaling of $\boldsymbol{\Sigma}_{jk}$. Therefore, numerous methods that try to optimize the
  behavior of the Markov chain in an adaptive phase (burnin phase) have been developed. In the
  seminal paper of \citet{bamlss:Gelman+Roberts+Gilks:1996}, strategies that optimize the acceptance
  rate to roughly $1/4$ are suggested to obtain a good mixing. Similarly, within the presented
  modeling framework and a basis function approach with multivariate normal prior
  (\ref{eqn:shrinkprior}), a convenient way is to set
