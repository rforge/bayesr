...
area. This is viewed as an optimization problem in \citet{bamlss:Taylor:2015}, i.e., they treated
the question of possible improvements of response times according to space in a probabilistic way.
Therefore, they analyzed the effect of fire station closures in 2014 using a parametric proportional
hazards model and identified regions of possible concern about the number of available fire stations.
...
As mentioned above, \citet{bamlss:Taylor:2015} analyzed the response times within a
...
received emergency call. Function $f_0(\texttt{arrivaltime})$ represents the baseline hazard and is
modeled by a penalized spline and $f_1(\texttt{arrivaltime}, \texttt{lon}, \texttt{lat})$ is the
space-time varying effect modeling the deviations from the baseline.
...
$$
For the posterior mode estimation and MCMC simulation we need the log-likelihood of the continuous
time Cox-model, \fixme{stopped here!}
$$
\ell(\boldsymbol{\beta}; \mathbf{y}, \mathbf{X}) =
