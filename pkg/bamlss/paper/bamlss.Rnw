...
\ell(\mathbf{y} | \boldsymbol{\vartheta}) =
...
Estimation of $\boldsymbol{\vartheta}$ requires solving equations
...
= \frac{\partial \ell(\mathbf{y} | \boldsymbol{\vartheta})}{\partial \boldsymbol{\vartheta}} +
...
\frac{\partial \ell(\mathbf{y} | \boldsymbol{\vartheta})}{\partial \boldsymbol{\vartheta}_k} =
  \frac{\partial \ell(\mathbf{y} | \boldsymbol{\vartheta})}{\partial \boldsymbol{\eta}_k}
  \frac{\partial \boldsymbol{\eta}_k}{\partial \boldsymbol{\vartheta}_k} = 
  \frac{\partial \ell(\mathbf{y} | \boldsymbol{\vartheta})}{\partial \boldsymbol{\theta}_k}
...
the components of $\mathbf{H}_{ks}$ including $\ell(\mathbf{y} | \boldsymbol{\vartheta})$ can be
written as
\begin{equation} \label{hessian2}
\frac{\partial^2 \ell(\mathbf{y} | \boldsymbol{\vartheta})}{\partial \boldsymbol{\vartheta}_k \partial \boldsymbol{\vartheta}_s^\top} =
\left( \frac{\partial \boldsymbol{\eta}_s}{\partial \boldsymbol{\vartheta}_s} \right)^\top
\frac{\partial^2 \ell(\mathbf{y} | \boldsymbol{\vartheta})}{\partial \boldsymbol{\eta}_k\partial \boldsymbol{\eta}_s^\top}
\frac{\partial \boldsymbol{\eta}_k}{\partial \boldsymbol{\vartheta}_k}
\,\, \underbrace{
  \, + \, \frac{\partial \ell(\mathbf{y} | \boldsymbol{\vartheta})}{\partial \boldsymbol{\eta}_k}
...
\frac{\partial^2 \ell(\mathbf{y} | \boldsymbol{\vartheta})}{\partial \boldsymbol{\eta}_k\partial \boldsymbol{\eta}_s^\top} =
  ...
  \frac{\partial^2 \ell(\mathbf{y} | \boldsymbol{\vartheta})}{\partial \boldsymbol{\theta}_k \partial \boldsymbol{\theta}_s^\top}
  \frac{\partial \boldsymbol{\theta}_k}{\partial \boldsymbol{\eta}_k}
...
\item \emph{Derivative based Metropolis-Hastings}: \label{sec:dmh} \\
  ...
  of the log-posterior centered at the latest position
  \begin{eqnarray*}
  ...
  $q(\boldsymbol{\vartheta}_{jk}^\star | \boldsymbol{\vartheta}_{jk}^{(t)}) =
    N(\boldsymbol{\mu}_{jk}^{(t)}, \boldsymbol{\Sigma}_{jk}^{(t)})$. Again, assuming a basis
  ...
  $$
  with weights $\mathbf{W}_{kk} = -diag(\partial^2 \ell(\boldsymbol{\vartheta}_k | \mathbf{y}) /
\partial \boldsymbol{\eta}_k \partial \boldsymbol{\eta}_k^\top)$ and the mean can be written as
  ...
  \mathbf{X}_{jk}^\top\mathbf{u}_k^{(t)} - \mathbf{G}_{jk}\boldsymbol{\vartheta}_{jk}^{(t)} +
  (\mathbf{X}_{jk}^\top\mathbf{W}_{kk}\mathbf{X}_{jk} + \mathbf{G}_{jk})\boldsymbol{\vartheta}_{jk}^{(t)}\right\} \\
  &=& \boldsymbol{\Sigma}_{jk}^{(t)}\left\{
  \mathbf{X}_{jk}^\top\mathbf{u}_k^{(t)} +
  \mathbf{X}_{jk}^\top\mathbf{W}_{kk}\mathbf{X}_{jk}\boldsymbol{\vartheta}_{jk}^{(t)}\right\} \\
  &=& \boldsymbol{\Sigma}_{jk}^{(t)}\left\{
  \mathbf{X}_{jk}^\top\mathbf{u}_k^{(t)} +
  ...
  the computation of the mean is equivalent to a full Newton step for parameter
  $\boldsymbol{\vartheta}_{jk}$ in each iteration of the MCMC sampler, which is known as the
  iteratively reweighted least squares (IWLS) proposal
  \citep{bamlss:Gamerman:1997, bamlss:Brezger+Lang:2006, bamlss:Klein+Kneib+Klasen+Lang:2014}.
  The acceptance probability of the candidate is then computed by
  $$
  \alpha\left( \boldsymbol{\vartheta}_{jk}^{\star} | \boldsymbol{\vartheta}_{jk}^{(t)}\right) =
  \text{min} \left\{ \frac{p(\boldsymbol{\vartheta}_{jk}^{\star} | \cdot)q(\boldsymbol{\vartheta}_{jk}^\star | \boldsymbol{\vartheta}_{jk}^{(t)})}{
    p(\boldsymbol{\vartheta}_{jk}^{(t)} | \cdot)q(\boldsymbol{\vartheta}_{jk}^{(t)} | \boldsymbol{\vartheta}_{jk}^\star)  }, 1 \right\}
  $$
...
\item The first order derivatives
  $\partial \ell(\mathbf{y} | \boldsymbol{\vartheta}) / \partial \boldsymbol{\theta}_k$,
...
\item Second order derivatives
  $\partial^2 \ell(\mathbf{y} | \boldsymbol{\vartheta}) / \partial \boldsymbol{\eta}_k \partial \boldsymbol{\eta}_k^\top$.
