...
\end{equation}
Clearly, the problem in deriving the expectation relies in the computation of the usually
...
\subsubsection{* Random walk Metropolis-Hastings} \label{sec:rwmh}
One of the most popular sampling schemes is random walk Metropolis-Hastings (RWMH). The sampler
proceeds by sampling a new $\boldsymbol{\vartheta}_{jk}^{(t+1)}$ from a symmetric jumping
distribution $q(\boldsymbol{\vartheta}_{jk}^{(t)}, \boldsymbol{\vartheta}_{jk}^{(t+1)})$ which
is accepted with probability
\begin{equation} \label{eqn:accprob}
\alpha\left( \boldsymbol{\vartheta}_{jk}^{(t)}, \boldsymbol{\vartheta}_{jk}^{(t+1)}\right) =
\text{min} \left( \frac{p(\boldsymbol{\vartheta}_{jk}^{(t+1)})}{p(\boldsymbol{\vartheta}_{jk}^{(t)})}, 1 \right).
\end{equation}

\subsubsection{* Derivative based Metropolis-Hastings} \label{sec:dmh}

\subsubsection{* Hamiltonian Monte Carlo} \label{sec:hmcmc}

\subsubsection{* Slice sampling} \label{sec:smcmc}
