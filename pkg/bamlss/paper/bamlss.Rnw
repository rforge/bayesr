...
same environment, e.g., the \proglang{R} packages \pkg{mgcv} \citep{bamlss:Wood:2016},
\pkg{gamlss} \citep{bamlss:Stasinopoulos+Rigby:2016} and \pkg{VGAM} \citep{bamlss:Yee:2016}
implement all model terms in their own fashion style. This is particular problematic if all
packages are loaded into \proglang{R}'s global environment, because
some functions that are supposed to fulfill the same purpose have different interpretations.

This article examines the structures that are required for the estimation of a variety of models
and combines the building blocks to a kind of unified conceptional ``Lego toolbox''. The presented
framework thereby leverages
