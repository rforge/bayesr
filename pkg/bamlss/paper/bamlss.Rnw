...
if(!file.exists("figures/rainmodel-effects.png") & FALSE) {
  ...
  for(i in 1:nrow(co)) {
    ...
    psigma <- predict(rainmodel, model = "sigma", newdata = nd2, intercept = TRUE, trans = exp, FUN = foo)
    pred <- NULL
    for(j in 1:ncol(pmu))
  ...
  main <- function(text) {
    mtext(text, side = 3, line = 0.5, cex = 0.8)
  }

  stations <- unique(homstart[, c("long", "lat")])
  orange <- rgb(242, 146, 0, maxColorValue = 255)

  png("figures/rainmodel-data.png", units = "in", res = 120, width = 9, height = 3)
  par(mfrow = c(1, 2), mar = c(4.1, 4.1, 0.5, 1.1))
  plot(Austria, col = gray(0.9))
  ...
  png("figures/rainmodel-effects.png", units = "in", res = 300, width = 6, height = 5.5)
  ...
  axis(2)
  box()
  main(expression(paste("Seasonal effect on ", mu)))

  plot(Austria, xlab = "Longitude [deg]", ylab = "Latitude [deg]")
  ...
  axis(1)
  axis(2)
  main(expression(paste("Spatial effect on ", mu)))
  ...
  axis(2)
  box()
  main(expression(paste("Seasonal effect on ", log(sigma))))
  ...
  axis(2)
  box()
  main(expression(paste("Spatial effect on ", log(sigma))))
  dev.off()

  png("figures/rainmodel-predict.png", units = "in", res = 300, width = 7, height = 4.5)
  ...
  xymap(long, lat, rain10, data = nd, pos = "topleft", layout = FALSE, map = FALSE,
    ...
    width = 0.3, symmetric = FALSE, swap = FALSE, digits = 1, range = c(0, 0.5),
    lrange = round(c(0, max(nd$rain10)), 1))
  plot(Austria, add = TRUE)
  box()
  axis(1)
  axis(2)
  main("Predicted precipitation [mm] January 10th")
  box()
  dev.off()
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.92\textwidth]{figures/rainmodel-effects}
\caption{\label{fig:rainmodeleffects} Estimated effects of the rainmodel.}
\end{figure}

\begin{figure}[t!]
\centering
\includegraphics[width=0.92\textwidth]{figures/rainmodel-predict}
\caption{\label{fig:rainmodelpredict} Predicted mean precipitation for January 10th.}
\end{figure}
    ...
    "fsintens", "daytime"), spar = TRUE, ...)
  {
    ...
    if("daytime_curves" %in% what & !is.null(data$daytime)) {
      ...
      legend("bottomright", c("Mean daytime effect", "Spatial-varying effect"), lwd = c(2, 1),
        col = c(rainbow_hcl(1), "black"), box.col = NA, bg = NA, cex = 0.95)
      main("Spatial-varying daytime effect")
    }
    if("spatial_prob" %in% what) {
      ...
      axis(1)
      axis(2)
      main(paste("Prob(T > ", data$target, ")", sep = ""))
    }
    if("fsintens" %in% what) {
      ...
      main("Effect of fire station intensity")
    }
    if("spatial_td" %in% what) {
      ...
      axis(1)
      axis(2)
      main(paste("Time-dependent spatial effect (t = ", data$target, ")", sep = ""))
    }
    if("daytime" %in% what) {
    ...
    if("spatial_tc" %in% what) {
      ...
      axis(1)
      axis(2)
      main("Time-constant spatial effect")
    }
    ...
    if("stations" %in% what) {
      ...
      main(paste(paste(rep(" ", 70), collapse = "", sep = ""), "Arrivaltime [min]"))
    }
  }
