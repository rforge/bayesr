...
if(!file.exists("figures/firemodel-data.png")) {
  ...
  grid_agg_plot <- function(n = 1000, FUN = NULL,
    ...
    main = NULL, ...)
  {
    ...
    plot(LondonBoundaries, col = gray(0.9), xlab = "Longitude [deg]", ylab = "Latitude [deg]",
      main = main)
  ...
  fire.data.plot <- function()
  {
    par(mfrow = c(1, 3))
    plot(LondonBoroughs, xlab = "Longitude [deg]", ylab = "Latitude [deg]", col = gray(0.9),
      main = "Fires and fire stations")
    ...
    grid_agg_plot(type = "hexagonal", at = c(0, 0.5, 1), labels = c("0.0", "0.5", "1.0"),
      main = "% Arrivaltime > 6min")
    hist(LondonFire$arrivaltime, breaks = 50, col = gray(0.9), freq = FALSE,
      xlab = "Arrivaltime", main = "Histogram of arrivaltime")
    abline(v = 6, lyy = 2, col = "blue")
    box()
  }

  png("figures/firemodel-data.png", units = "in", res = 120, width = 6 * 1.5, height = 8 / 3)
  fire.data.plot()
  dev.off()
...
\begin{figure}[t!]
\centering
\includegraphics[width=1\textwidth]{figures/firemodel-data}
\caption{\label{fig:londonfiredata} Distribution of dwelling fires and fire stations in London (2015).}
\end{figure}

\begin{figure}[t!]
\centering
\includegraphics[width=0.6666667\textwidth]{figures/firemodel-effects}
