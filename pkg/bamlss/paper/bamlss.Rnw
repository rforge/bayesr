...
and similarly $\boldsymbol{\alpha}$ is the set of all fixed prior specifications. For example,
$f_{jk}( \cdot )$ is modeled as a penalized spline \citep{bamlss:Eilers+Marx:1996}, then
$\boldsymbol{\tau}_{jk}$ represents smoothing variances (parameters) and $\boldsymbol{\alpha}_{jk}$
is a set of penalty matrices. Also note that from a
...
\end{algorithmic}
\end{algorithm}
Note again the general structure of sampling Algorithm~\ref{fig:mcmc}, i.e., the proposal function
$q_{jk}( \cdot )$ is left unspecified up to this point.
