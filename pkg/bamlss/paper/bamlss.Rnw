...
if(!file.exists("firemodel.rda")) {
  ...
  data_basehaz <- function(n = 30, target = 6, k = 20, ...)
  {
    ...
    for(i in 1:nrow(co)) {
      td <- data.frame("arrivaltime" = atime)
      td$lon <- co$lon[i]
      td$lat <- co$lat[i]
      td$id <- i
      nd <- rbind(nd, td)
    ...
    nd$p50 <- predict(firemodel, newdata = nd, model = "lambda", FUN = mean, intercept = FALSE)
    nd$atime <- nd$arrivaltime

    fbh <- NULL
    for(i in unique(nd$id))
      fbh <- cbind(fbh, nd$p50[nd$id == i])
    fbh <- cbind(atime, fbh)
    ...
    nd$spatial_prob <- predict(firemodel, newdata = nd,
      term = c("ti(arrivaltime)", "ti(arrivaltime,lon,lat)", "s(daytime)", "s(lon,lat)"),
      intercept = TRUE, type = "prob", time = target, ...)
    nd$spatial_tc <- predict(firemodel, newdata = nd, model = "gamma",
      term = "s(lon,lat)", intercept = FALSE)
    nd$spatial_td <- predict(firemodel, newdata = nd, model = "lambda",
      term = "ti(arrivaltime,lon,lat)", intercept = FALSE)

    b <- gam(log(arrivaltime) ~ s(lon, lat, k = 100), data = LondonFire)
    nd$spatial_mtime <- exp(predict(b, newdata = nd))

    return(list("curves" = fbh, "spatial" = nd, "target" = target))
  }

  firemodel_plotdata <- data_basehaz(80, 6, subdivisions = 15)

  save(firemodel, firemodel_plotdata, file = "firemodel.rda")
}
...
if(!file.exists("figures/firemodel-data.png")) {
    ...
    main = NULL, spar = TRUE, ...)
  {
    ...
    if("spatial_prob" %in% what) {
      ...
      xr <- c(0, quantile(data$spatial$spatial_prob, probs = 0.97))
      xymap(lon, lat, spatial_prob, data = data$spatial, pos = "bottomright",
        ...
        side.legend = 2, digits = 2, range = xr, lrange = round(lr, 2),
    ...
    if("fsintens" %in% what) {
      plot(firemodel, model = "gamma", term = "s(fsintens)", spar = FALSE,
    ...
    if("daytime" %in% what) {
      plot(firemodel, model = "gamma", term = "s(daytime)", spar = FALSE, rug = FALSE,
    ...
    if("stations" %in% what) {
      ...
      plot(LondonBoroughs, main = "Dwelling fires and fire stations",
        xlab = "Longitude [deg]", ylab = "Latitude [deg]", ylim = ylim)
      cfun <- function(n) { heat_hcl(n, alpha = 0.9) }
      pal <- make_pal(col = cfun, ncol = 99, data = LondonFire$arrivaltime, symmetric = FALSE,
        range = c(4, 8), swap = TRUE)
      points(LondonFire, pch = 4, col = pal$map(LondonFire$arrivaltime), cex = 0.5)
      points(LondonFStations, pch = 16, col = "blue", cex = 0.6)
      points(LondonFStations, cex = 0.6)
      box()
      axis(1)
      axis(2)
      ...
      i <- order(LondonFire$lon)
      atimes <- LondonFire$arrivaltime[i]
      lon <- LondonFire$lon[i]
      shift <- scale * 0.1 * abs(diff(co["y", ]))
      y <- scale2(atimes, co["y", 2] + shift, ylim[2])
      rect(lon - 0.002, rep(co["y", 2] + shift,length = length(y)), lon + 0.002, y,
        col = pal$map(atimes), border = NA)
      lines(range(lon), rep(co["y", 2] + shift, 2))


      x <- c(lon, rev(lon))
      y <- c(rep(co["y", 2] + shift, length = length(y)), y)
      polygon(x, y)
    }
    if("mtime" %in% what) {
      par(mfrow = c(1, 2))
      i <- order(LondonFire$lat)
      atimes <- LondonFire$arrivaltime[i]
      lat <- LondonFire$lat[i]
      plot(lat, atimes, type = "h", col = rgb(0.1, 0.1, 0.1, alpha = 0.01), lwd = 4)

      i <- order(LondonFire$lon)
      atimes <- LondonFire$arrivaltime[i]
      lon <- LondonFire$lon[i]
      plot(lon, atimes, type = "h", col = rgb(0.1, 0.1, 0.1, alpha = 0.01), lwd = 4)
    }
  }

  png("figures/firemodel-data.png", units = "px", res = 120, width = 720, height = 960 / 3,
    pointsize = 8)
  par(mfrow = c(1, 2))
  plot.firemodel(firemodel_plotdata, what = c("stations", "mtime"), spar = FALSE)
  dev.off()
