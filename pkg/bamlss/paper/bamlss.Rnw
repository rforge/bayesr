...
if(!file.exists("figures/firemodel-data.png")) {
  ...
  grid_plot <- function(n = 300, FUN = NULL,
    color = heat_hcl, symmetric = FALSE, swap = TRUE, eps = 0.0001, ...)
  {
    ...
    plot(LondonBoroughs)
    LBP <- unionSpatialPolygons(LondonBoroughs, rep(1L, length = length(LondonBoroughs)))
    LBP <- spTransform(LBP, CRS("+init=epsg:27700"))
    HexPts <- spsample(LBP, n = n, type = "hexagonal")
    HexPols <- HexPoints2SpatialPolygons(HexPts)
    HexPols <- spTransform(HexPols, CRS("+init=epsg:4326"))
    LBP <- spTransform(LBP, CRS("+init=epsg:4326"))
    clip <- gIntersection(LBP, HexPols, byid = TRUE, drop_lower_td = TRUE)
    plot(clip, add = TRUE)

    val <- over(clip, LondonFire[, "arrivaltime"], fn = mean)
    clip <- as(clip, "SpatialPolygonsDataFrame")
