...
Since the HOMSTART data set has over $1$ million observations, the resulting design matrices
...
year only has 365 unique observations. This is much smaller than the total number of observations
...
<<eval=FALSE>>=
f <- list(
  "mu" = sqrt(raw) ~ elevation + ti(day,bs="cc",k=10) +
    ti(long,lat,bs="tp",d=2,k=50) +
    ti(day,long,lat,bs=c("cc","tp"),d=c(1,2),k=c(8,30)),
  "sigma" = ~ elevation + ti(day,bs="cc",k=10) +
    ti(long,lat,bs="tp",d=2,k=50) +
    ti(day,long,lat,bs=c("cc","tp"),d=c(1,2),k=c(8,30))
)
@
using the \pkg{mgcv} smooth term constructor \fct{ti} for decomposition of the day of year and
the spatial effect into main effects and its deviations. Note that \code{bs = "cc"} specifies a
...
rainmodel <- bamlss(f, data = homstart, family = "cnorm",
  binning = TRUE, before = FALSE, results = FALSE,
  n.iter = 6000, burnin = 4000, thin = 10, cores = 7)
@
