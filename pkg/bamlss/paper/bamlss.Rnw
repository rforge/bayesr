...
\ell(\boldsymbol{\beta} | \mathbf{y}) =
  \sum_{i = 1}^n ln \, f(\mathbf{y}_i | \theta_{i1} = h_1^{-1}(\eta_{i1}(\boldsymbol{\beta}_1)), \ldots,
  \theta_{iK} = h_K^{-1}(\eta_{iK}(\boldsymbol{\beta}_K)))
\end{equation}
...
\item \emph{Derivative based Metropolis-Hastings}: \label{sec:dmh} \\
  ...
  information
  $$
  \boldsymbol{\Sigma}_{jk} = -\left( \frac{\partial^2 p(\boldsymbol{\beta} | \mathbf{y})}{
  ...
  \qquad \boldsymbol{\mu}_{jk}^{(t)} = \boldsymbol{\Sigma}_{jk}^{(t)}\left\{ s\left(\boldsymbol{\beta}_{jk}^{(t)}\right) -
