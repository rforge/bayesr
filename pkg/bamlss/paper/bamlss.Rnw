...
Finding good starting values is especially important for complex model terms that, e.g., include
multiple smoothing variances. Therefore, we propose to estimate parameters $\boldsymbol{\tau}_{jk}$
...
updating systems like (\ref{eqn:blockupdate}) and (\ref{eqn:blockblockupdate}), we show within this
section that the following ``Lego bricks'' are repeatedly used in Algorithm~\ref{fig:algodesign}:
\begin{enumerate}
...
\item \label{item:3} prior densities $p_{jk}(\boldsymbol{\beta}_{jk}; \boldsymbol{\tau}_{jk}, \boldsymbol{\alpha}_{jk})$.
\end{enumerate}
Moreover, model term updating Algorithms~\ref{fig:umode} and \ref{fig:mcmc} usually require
\begin{enumerate}
\setcounter{enumi}{3}
...
be simplified when fragmenting with the chain rule.
...
This sections describes the quantities needed for applying the general
Algorithm~\ref{fig:algodesign}. More precisely we first describe posterior mode updating
as used within Algorithm~\ref{fig:umode}, before we introduce several MCMC sampling schemes that
can be applied in updating Algorithm~\ref{fig:mcmc}.
