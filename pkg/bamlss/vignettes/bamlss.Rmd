...
of `10`, i.e., only every 10th sample is saved. Note that per defaul `bamlss()` uses a bachfitting
...
```
```{r, fig.width = 9, fig.height = 5, fig.align = "center", echo = FALSE, dev = "png", echo=FALSE}
par(mfrow = c(1, 2))
...
```
```{r, fig.align = "center", dev = "png", results='hide', message=FALSE, echo=FALSE, out.width="60%"}
...
```{r}
nd$p <- predict(b, newdata = nd, model = "mu", FUN = quantile)
print(head(nd))
```
