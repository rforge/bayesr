...
```
where `s()` is the smooth term constructor from the _mgcv_ [@bamlss:Wood:2018]. Note that all
smooth terms, i.e., `te()`, `ti()`, etc., are supported by _bamlss_. This way, it is also possible
to incorporate user defined model terms. A full Bayesian model is the estimated with
```{r, eval=FALSE}
library("bamlss")
