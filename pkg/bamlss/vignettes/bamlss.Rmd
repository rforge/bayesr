...
```
```{r, fig.width = 8, fig.height = 4, fig.align = "center", dev = "png", results='hide', message=FALSE, echo=FALSE, out.width="80%"}
...
the estimated densities for `times = 10` and `times = 40`:
```{r, fig.width = 5, fig.height = 4, fig.align = "center", dev = "png", out.width="40%"}
...
for(i in 1:ncol(ptimes$mu)) {
  ## Density for times = 10.
  par <- list("mu" = ptimes$mu[1, i, drop = TRUE], "sigma" = ptimes$sigma[1, i, drop = TRUE])
  dens$t10 <- cbind(dens$t10, fam$d(mcycle$accel, par))

  ## Density for times = 40.
  par <- list("mu" = ptimes$mu[2, i, drop = TRUE], "sigma" = ptimes$sigma[2, i, drop = TRUE])
  dens$t40 <- cbind(dens$t40, fam$d(mcycle$accel, par))
}

plot2d(dens$t10 ~ accel, data = mcycle, col.lines = rgb(1, 0, 0, alpha = 0.05))
plot2d(dens$t40 ~ accel, data = mcycle, col.lines = rgb(0, 0, 1, alpha = 0.05), add = TRUE)
```
