...
infrastructure, which allows the user to exchange both, optimization functions for
posterior mode estimation (penalized likelihood) and sampling functions for full
Bayesian Inference. Within this unified modeling approach it is relatively straightforward
to develop new estimation functions, e.g., such a function could also be an interface 
function to  other software (see, e.g., the `JAGS()` sampling function). In the following,
the basic requirements for optimizer and sampling functions are described.
...
The default optimizer using the `bamlss()` wrapper function is `bfit()`, which is
...
a vector of estimated regression coefficients, while sampling functions return a matrix
of samples of class `"mcmc"` or `"mcmc.list"` (see the documentation of the _coda_ package).

Internally, what the optimizer or sampling function is actually running is not important
for the `bamlss()` wrapper as long as a vector or matrix of parameters is returned. For
optimizer functions the return value needs to be named list with an element `"parameters"`,
the vector (also a matrix, e.g., for `lasso()` and `boost()` optimizers) of estimated 
parameters. The most important requirement to make use of all extractor functions like
`summary.bamlss()`, `predict.bamlss()`, `plot.bamlss()`, `residuals.bamlss()`, etc., is
to keep the naming convention of the returned estimates. The parameter names are based
on the names of the distribution parameters as specified in the family object. For example,
the family object `gaussian_bamlss()` has names `"mu"` and `"sigma"`
```{r}
...
lm_bamlss <- function(...) {
  f <- list(
    ...
    "d" = function(y, par, log = FALSE) {
      dnorm(y, mean = par$mu, sd = 1, log = log)
    },
    "p" = function(y, par, ...) {
