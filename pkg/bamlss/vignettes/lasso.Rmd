...
```{r, eval=FALSE}
lasso_plot(rentmodel, which = "criterion")
```
```{r, fig.width = 8, fig.height = 4, fig.align = "center", echo = FALSE, dev = "png", echo=FALSE}
par(mfrow = c(1, 2))
lasso_plot(rentmodel, which = "criterion", spar = FALSE)
```
...
```{r, eval=FALSE}
lasso_plot(rentmodel, which = "parameters", model = "mu")
```
...
to select specific paths for plotting is to set the `name` argument in `lasso_plot()`, e.g.,
...
```{r, fig.width = 5, fig.height = 5, fig.align = "center", dev = "png", results = 'hide'}
lasso_plot(rentmodel, which = "parameters", model = "sigma",
