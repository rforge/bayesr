...
if(!file.exists("figures/lasso.rda")) {
  ...
  rent99$area <- cut(rent99$area, breaks = c(0, seq(30, 140, by = 10), Inf),
  ...
  rentmodel <- bamlss(f, data = rent99, optimizer = lasso, sampler = FALSE,
    criterion = "BIC", nlambda = 30, multiple = TRUE)

  rentmodel_s <- bamlss(f, data = rent99, optimizer = bfit, sampler = FALSE,
    criterion = "BIC", start = coef(rentmodel))
...
we use the year of construction (`yearc`), area (`area`) and the the location classification
(`location', derived from expert knowledge) of the apartment as covariates. Following
...
rentmodel <- bamlss(f, data = rent99, optimizer = lasso, sampler = FALSE,
  upper = exp(30), lower = 0.223102, criterion = "BIC", nlambda = 30, multiple = TRUE)
```
...
The optimal tuning parameters are selected by BIC on a 2-dimensional grid. The marginal BIC curves
can be visualized with
```{r, eval=FALSE}
lasso.plot(rentmodel, which = "criterion")
```
```{r, fig.width = 8, fig.height = 4, fig.align = "center", echo = FALSE, dev = "png", echo=FALSE}
par(mfrow = c(1, 2))
lasso.plot(rentmodel, which = "criterion", spar = FALSE)
```
In this example both tuning parameters have the same value, also indicated by the gray dashed
vertical lines. The coefficient paths w.r.t.\ the
tuning parameters can be plotted, e.g. for parameter $\mu$, with
```{r, eval=FALSE}
lasso.plot(rentmodel, which = "parameters", model = "mu")
```
```{r, fig.width = 5, fig.height = 5, fig.align = "center", echo = FALSE, dev = "png", results = 'hide', message=FALSE}
par(mfrow = c(1, 2))
lasso.plot(rentmodel, which = "parameters", model = "mu")
```
The legend on the right hand side of the plot is not necessarily very nice. One option to select
specific path for plotting is the `name` argument, e.g., plotting the coefficient paths for
the year of construction for parameter $\mu$ can be done by
...
Similarly, the effect on parameter $\sigma$ for the area of
the apartment can be plotted by
```{r, fig.width = 5, fig.height = 5, fig.align = "center", dev = "png", results = 'hide'}
lasso.plot(rentmodel, which = "parameters", model = "sigma",
  name = "sigma.s.la(area).area")
```
The plot shows that quite a few categories of covariate `area` are fused using the BIC for tuning
parameter selection.
...
LASSO and fused LASSO terms can be mixed, which is not the case if only single tuning parameter
for each distributional parameter (e.g., $\mu$ and $\sigma$ using the Gaussian distribution) is used.
...
rentmodel_s <- bamlss(f, data = rent99, optimizer = bfit, sampler = FALSE,
  criterion = "BIC", start = coef(rentmodel))
```
using the estimated parameters of the first model as starting values. To compare both models
by BIC, the `lasso.stop()` functions returns the optimum iteration of the `lasso()` optimizer
```{r}
lasso.stop(rentmodel)
```
...
Using the `lasso.stop()` function, we can also extract the corresponding regression coefficients
or compute predictions.
```{r, eval=FALSE}
coef(rentmodel, mstop = lasso.stop(rentmodel))
predict(rentmodel, mstop = lasso.stop(rentmodel))
```
