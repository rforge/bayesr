...
f <- list(num~s(week,bs="cc"),~s(week,bs="cc"),~s(week,bs="cc"))

b <- bamlss(f, data = fatalities, family = BCCG(mu.link = "log"))
...
with(nd, matplot(week, cbind(q95, q50, q05),
  ...
  col = 4, add = TRUE))
d2020 <- subset(fatalities, year == 2020)
with(d2020, points(week, num, pch = 16, col = 2))
