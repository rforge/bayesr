...
/*  if(ntau2 > 1) {*/
/*    for(j = 0; j < nc; j++) {*/
/*      tau2ptr[j] = thetaptr[nc + j];*/
/*      XWXptr[j + nc * j] += 1.0 / (thetaptr[nc + j] * pow(pow(thetaptr[j], 2.0) + 0.00001, 0.5));*/
/*      for(i = 0; i < nr; i++) {*/
/*        mu0ptr[j] += Xptr[i + nr * j] * xrresptr[i];*/
/*      }*/
/*      XWXptr[j + nc * j] = 1.0 / XWXptr[j + nc * j];*/
/*      mu1ptr[j] = XWXptr[j + nc * j] * mu0ptr[j];*/
/*    }*/
/*  } else {*/
/*    tau2ptr[0] = thetaptr[nc];*/
/*    for(j = 0; j < nc; j++) {*/
/*      XWXptr[j + nc * j] += 1.0 / (tau2ptr[0] * pow(pow(thetaptr[j], 2.0) + 0.00001, 0.5));*/
/*      for(i = 0; i < nr; i++) {*/
/*        mu0ptr[j] += Xptr[i + nr * j] * xrresptr[i];*/
/*      }*/
/*      XWXptr[j + nc * j] = 1.0 / XWXptr[j + nc * j];*/
...
/*  GetRNGstate();*/
/*  for(j = 0; j < nc; j++) {*/
/*    gamma1ptr[j] = mu1ptr[j] + XWXptr[j + j * nc] * rnorm(0, 1);*/
/*    edf0 = edf0 + XWX0ptr[j + j * nc] * XWXptr[j + j * nc];*/
/*  }*/
/*  PutRNGstate();*/
/* */
/*  SEXP edf;*/
...
/*  SEXP QBETAPROP;*/
/*  PROTECT(QBETAPROP = eval_dmvnorm_log(getListElement(x, "dmvnorm_log"), gamma1, mu1, getListElement(x, "XWX"), rho));*/
/*  ++nProtected;*/
/*  double qbetaprop = REAL(QBETAPROP)[0];*/
