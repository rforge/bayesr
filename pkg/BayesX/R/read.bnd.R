read.bnd <- function(file, sorted = FALSE) {
  ## Liste wird erstellt: 1. Element enthaelt die 1. Spalte aus BND-Datei, 2.
  ## Element die 2. Spalte
  ...
  data.numeric <- lapply(data.raw, as.numeric)
  ...
  unquote <- function(string) {
    return(gsub(pattern = "\"", replacement = "", x = string))
  }
  ...
  whereRegionNames <- setdiff(which(is.na(data.numeric[[1]])), whereIsIn)
  ...
  enclosedPolygonsInds <- findInterval(x = whereIsIn, vec = whereRegionNames)
  
  ## so we can already setup the surrounding list
  surrounding <- replicate(n = nPolygons, expr = character())
  ...
  data.numeric <- cbind(data.numeric[[1]], data.numeric[[2]])
  ...
  ## these are the start indices for the polygons (plus one index past the last
  ## polygon)
  ...
  for (k in seq_along(map)) {
    ...
    if (sum(map[[k]][1, ] == map[[k]][polyLengths[k], ]) != 2) 
      warning(paste("Note: First and last point of polygon ", k, " (region ", 
        names(map)[k], ") are not identical", sep = ""), call. = FALSE)
  }
  ...
  if (sorted) {
    ...
    ## decide which ordering to apply: are there some characters?
    newOrder <- if (any(is.na(numericNames))) {
  ...
  return(structure(map, class = "bnd", height2width = height2width, surrounding = surrounding, 
    regions = regions))
}
