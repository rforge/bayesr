createxymap <- function(x, y, districts = NULL, p = 2, max.dist) {
  ...
  bnd <- list()
  for (i in 1:S) {
    bnd[[i]] <- matrix(data = c(x[i] - step, y[i] - step, x[i] - step, y[i] + 
      step, x[i] + step, y[i] + step, x[i] + step, y[i] - step, x[i] - step, 
      y[i] - step), byrow = T, nrow = 5, ncol = 2)
  }
  
  names(bnd) <- districts
  
  attr(bnd, "surrounding") <- replicate(n = length(bnd), expr = character())
