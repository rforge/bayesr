...
betazoi.BayesR <- function(links = c(mu = "logit", sigma2 = "logit", nu = "log", tau = "log"), ...)
{
  rval <- list(
    ...
    "mu" = function(eta, ...) {
      linkinv$mu(eta) * (1 - (linkinv$nu(eta) + linkinv$tau(eta)) / (1 + linkinv$nu(eta) + linkinv$tau(eta))) +
        linkinv$tau(eta) / (1 + linkinv$nu(eta) + linkinv$tau(eta))
    }
  )
  class(rval) <- "family.BayesR"
  rval
...
betazi.BayesR <- function(links = c(mu = "logit", sigma2 = "logit", nu = "log"), ...)
{
  rval <- list(
    ...
    "mu" = function(eta, ...) {
      linkinv$mu(eta) * (1 - (linkinv$nu(eta)) / (1 + linkinv$nu(eta)))
    }
  )
  class(rval) <- "family.BayesR"
  rval
...
betaoi.BayesR <- function(links = c(mu = "logit", sigma2 = "logit", tau = "log"), ...)
{
  rval <- list(
    ...
    "mu" = function(eta, ...) {
      linkinv$mu(eta) * (1 - linkinv$tau(eta) / (1 + linkinv$tau(eta))) +
        linkinv$tau(eta) / (1 + linkinv$tau(eta))
    }
  )
  class(rval) <- "family.BayesR"
  rval
...
lognormal.BayesR <- function(links = c(mu = "log", sigma = "log"), ...)
{
  rval <- list(
	...
	"mu" = function(eta, ...) {
      exp(linkinv$mu(eta) + 0.5 * (linkinv$sigma(eta))^2)
    }
  )
  class(rval) <- "family.BayesR"
  rval
...
dagum.BayesR <- function(links = c(a = "log", b = "log", p = "log"), ...)
{
  rval <- list(
	...
	"mu" = function(eta, ...) {
      - (linkinv$b(eta)/linkinv$a(eta)) * (gamma(- 1 / linkinv$a(eta)) * gamma(linkinv$p(eta) + 1 / linkinv$a(eta))) / (gamma(linkinv$p(eta)))
    }
  )
  class(rval) <- "family.BayesR"
  rval
