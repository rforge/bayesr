...
A general setup is obtained by using normal priors for $\boldsymbol{\beta}_j$ of the form
...
weakly informative inverse Gamma hyperprior $\tau_j^2 \sim IG(a_j, b_j)$, with $a_j = b_j = 0.001$
as a standard option. Small values for $a_j$ and $b_j$ correspond to an approximate uniform
distribution for $\log \tau_j^2$.

A detailed discussion of the GAMLSS methodological framework is provided in
\citet{bamlss:Rigby+Stasinopoulos:2005}, on model with STAR predictor in
...
distributions. In practice, only few implementations support an entirely modular setup that can be
extended by the user. Examples that do support this flexibility are the \proglang{R}
...
To meet the requirements of the BAMLSS concept a setup with similar modularity is necessary.
For the estimation of a model the exchangeable family objects need the probability density function
and the link functions as a minimum. However, to fully understand a distribution the distribution
function, the quantile function and a function for random number generation are additionally
required.

Moreover, some estimation engines may need information about the score and hessian of the
likelihood function, or even more specific information, e.g., provided as character strings.

Therefore, the family specification setup of a BAMLSS implementation should be easily extendable and
flexible enough to allow for interactions with other systems and complete information on
...
smooth and random effect terms with the \code{s()} descriptor. A typical linear regression with
covariates \code{x1} and \code{x2} is then represented by
\begin{center}
\code{response} $\sim$ \code{x1 + x2}
\end{center}
A model with two additional nonlinear modeled terms of covariates \code{z1}, \code{z2} and \code{z3}
is set up with
\begin{center}
\begin{tabular}{l}
\code{response} $\sim$ \code{x1 + x2 + s(z1) + s(z2, z3)}
\end{tabular}
\end{center}
...
\begin{tabular}{l}
\code{list(} \\
$\quad$ \code{response} $\sim$ \code{x1 + x2 + s(z1) + s(z2),} \\
...
\begin{tabular}{l}
\code{list(} \\
$\quad$ \code{response} $\sim$ \code{x1 + x2 + s(z1) + s({\color{heat1}{id1}}),} \\
...
with three categories within the \code{response} variable, e.g., a multinomial model with some
reference category can be defined by
\begin{center}
{ \renewcommand{\arraystretch}{1}
\begin{tabular}{l}
\code{list(} \\
$\quad$ \code{response} $\sim$ \code{x1 + s(z1) + s(z2),} \\
