...
i.e., from the frequentist perspective (\ref{eqn:posterior}) can be viewed as a penalized
...
\end{equation}
The first partial derivatives of $\boldsymbol{\vartheta}$ of the log-likelihood can be further
fragmented by
\begin{equation}
\frac{\partial l(\boldsymbol{\vartheta} | \mathbf{y})}{\partial \boldsymbol{\vartheta}} =
  \sum_{i = 1}^n \sum_{k = 1}^K \frac{\partial ln \, l(\boldsymbol{\vartheta} | y_i)}{\partial \theta_{ik}}
  \frac{\partial \theta_{ik}}{\partial \eta_{ik}}\frac{\partial \eta_{ik}}{\partial \boldsymbol{\vartheta}}.
\end{equation}
Hence, for gradient based algorithms the derivates of the log-likelihood with respect to the 
parameters $\boldsymbol{\theta}_1, \ldots, \boldsymbol{\theta}_1$, the derivate of the link functions
and of the linear predictor with respect to the regression coefficients is needed.
