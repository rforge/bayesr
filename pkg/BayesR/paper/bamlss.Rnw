...
equation (\ref{eqn:structadd}). Estimation usually requires to evaluate
the log-likelihood function
\begin{equation}
l(\boldsymbol{\vartheta} | \mathbf{y}) = ln \, p(\boldsymbol{\vartheta} | \mathbf{y}) =
  \sum_{i = 1}^n ln \, f(y_i | \theta_{i1}(\eta_{i1}(\boldsymbol{\beta}_1, \boldsymbol{\gamma}_1)), \ldots, \theta_{iK}(\eta_{iK}(\boldsymbol{\beta}_K, \boldsymbol{\gamma}_K)))
\end{equation}
...
regression coefficients of the corresponding parameters
$\boldsymbol{\theta}_1, \ldots, \boldsymbol{\theta}_K$ of the distribution used in (\ref{eqn:dreg}). 
Furthermore, Bayesian models assign prior distributions to the parameters, e.g., normal priors given 
by (\ref{eqn:prior}) and inverse gamma priors (\ref{eqn:ig}) for the variance parameters, resulting in the log-posterior
\begin{equation}
ln \, p(\boldsymbol{\vartheta}, \boldsymbol{\tau}^2 | \mathbf{y}) =
  l(\boldsymbol{\vartheta} | \mathbf{y}) +
  \sum_{k = 1}^K\sum_{j = 1}^{p_k} \left\{ ln \, p(\boldsymbol{\beta}_{jk} | \tau_{jk}^2) +
  ln \, p(\tau_{jk}^2) \right\}.
\end{equation}

Maximum likelihood and other gradient based algorithms require the evaluation of the first
derivative of the log-likelihood or score vector. Within the Bayesian formulation the resulting
score vectors are
\begin{equation}
s(\boldsymbol{\vartheta}, \boldsymbol{\tau}^2) = \begin{pmatrix}
  \frac{\partial ln \, p(\boldsymbol{\vartheta}, \boldsymbol{\tau}^2 |
    \mathbf{y})}{\partial \boldsymbol{\vartheta}} \\[1ex]
  \frac{\partial ln \, p(\boldsymbol{\vartheta}, \boldsymbol{\tau}^2 |
    \mathbf{y})}{\partial \boldsymbol{\tau}^2} \\
\end{pmatrix} = \begin{pmatrix}
  \frac{\partial l(\boldsymbol{\vartheta} | \mathbf{y})}{\partial \boldsymbol{\vartheta}} +
    \frac{\partial \sum_{k = 1}^K\sum_{j = 1}^{p_k} ln \, p(\boldsymbol{\beta}_{jk} |
    \tau_{jk}^2)}{\partial \boldsymbol{\vartheta}} \\
  \frac{\partial ln \, p(\tau_{jk}^2)}{\partial \boldsymbol{\tau}^2},
\end{pmatrix}
\end{equation}
that is, for frequentist maximum likelihood estimation only
$\frac{\partial l(\boldsymbol{\vartheta} | \mathbf{y})}{\partial \boldsymbol{\vartheta}}$ needs
to be computed. The first partial derivatives of $\boldsymbol{\vartheta}$ of the log-likelihood
are given by
\begin{equation}
\frac{\partial l(\boldsymbol{\vartheta} | \mathbf{y})}{\partial \boldsymbol{\vartheta}} =
  \sum_{k = 1}^K 1
