...
algorithm (see \citealp{bamlss:Rigby+Stasinopoulos:2005}, Appendix~C), where in each iteration $i$
an adjusted dependent variable
...
$\boldsymbol{\vartheta}_k$ in each iteration. Depending on the type of algorithm different 
weights are used, e.g.,
$\mathbf{W}_{kk} = - E\left(\partial^2 l(\boldsymbol{\vartheta} | \mathbf{y}) / \partial \boldsymbol{\eta}_k \partial \boldsymbol{\eta}_k^\top\right)$. In the corresponding Bayesian approach
property (\ref{eqn:z}) can be utilized to construct multivariate normal proposals for
$\boldsymbol{\vartheta}_k$, see \citet{bamlss:Klein+Kneib+Lang:2013}.

In summary, candidate estimation algorithms all access the following quantities:
\begin{itemize}
\item $f()$.
\item 
\end{itemize} 
