...
\dots \citet{BayesR:Fahrmeir+Lang:2001} \dots
...
Structured additive distributional regression available through \pkg{BayesR} is based on generalised additive models for location, scale, and shape~\citep[GAMLSS, ][]{BayesR:RigSta2005} and has recently been developed for univariate responses in a Bayesian framework in~\citet{BayesR:KleKneLan2013b} and generalised for multivariate responses in~\citet{BayesR:KleKneKlaLan2013}. The basic idea is to extend the structured additive regression model class~\citep{BayesR:Fahrmeir+Kneib+Lang:2004} by relaxing the usual exponential family assumption for the distribution of the response variable and to estimate the whole distribution in one model by allowing further moments like variance or skewness to depend on covariates. Similar to STAR models structured additive distributional regression covers well known model classes such as generalised
linear models \citep[GLMs, see e.g., ][]{BayesR:McCullagh+Nelder:1989} generalised
additive models \citep[GAM, ][]{BayesR:Hastie+Tibshirani:1990}, generalised additive mixed models
\citep[GAMM, ][]{BayesR:Lin+Zhang:1999}, geoadditive models \citep{BayesR:Kamman+Wand:2003} or
varying coefficient models \citep{BayesR:Hastie+Tibshirani:1993}. We assume that the conditional distribution of a univariate response variable $y$ or a multivariate response vector $\mathbf{y}$ with known covariate information $\mathbf{x}$ is given by a prespecified class of $K$-parametric distributions $f(y|\vartheta_{1},\ldots,\vartheta_{K})$ / $f(\mathbf{y}|\vartheta_{1},\ldots,\vartheta_{K})$ indexed by the parameters $\vartheta_{1},\ldots,\vartheta_{K}$. Note that $f$ is considered a general density, i.e. we use the same notation for continuous responses, discrete responses and also mixed discrete-continuous responses. Each parameter $\vartheta_{k}$ is linked to a semiparametric regression predictor 
