...
## fit model
b <- bayesx2(y ~ sx(x1) + sx(fac, bs = "re") & fac ~ sx(x2) | y ~ sx(fac, bs = "re") & fac ~ sx(x2), family = normal.BayesX, data = dat, cores = 2, combine = TRUE)
...
a <- bayesr(y ~ s(x1) + s(x2), ~ s(x1), data = dat, family = beta.BayesR(sigma = "pow"))
