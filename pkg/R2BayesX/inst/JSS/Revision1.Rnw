...
\item \emph{On page 2, Section 5 does not appear in the last paragraph.}
...
\item \emph{State in the caption of Figure 1 that data (both outcomes
   and covariates are missing?) of the areas shaded with
   vertical lines are missing.}

\item \emph{Page 17, the third line underneath subsection 6.1,
...
\item \emph{Last line on page 22, ore \code{->} or?}
...
\item \emph{It looks like the authors misunderstood the way that R installs on a mac.
...
\item \emph{It would also be nice if a function was provided that checked the
...
\item \emph{Section 2: The option "full = TRUE" in the formula is not
...
\item \emph{Bottom of page 15: "several informations" \code{->} "several pieces of
   information" or just "information"}

\item \emph{Same place: "further arguments may be passed to the function system()"
  <-- I would probably use 'can' instead of 'may' - I misread the sentence
  the first time.}

\item \emph{Top of page 16: " model.name the name of the model" \code{<-} missing is}

\item \emph{Page 16 paragraph 2: What is a "very large dataset" for BayesX? Are
   there benchmarks?}

\item \emph{Page 20: I'm not a fan of an enormous piece of R output breaking
   a sentence.}
...
\item \emph{Something weird has happened in the second paragraph of page 34.}

\item \emph{Table 7: This should include the "acf=TRUE" option used in Figure 7.}
...
\item \emph{The discussion of the relationship between the two packages is probably useful to 
