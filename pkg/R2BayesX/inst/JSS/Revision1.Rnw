...
\item \textbf{Sections 2 and 3 can be merged into one section, which first
  ...
  \fixme{Achim, is that really useful? Also, I cannot see where we can skip code in the example?
    We could write the formula within the \code{bayesx()} call, but that is not really less code!}
...
\item \textbf{It would also be nice if a function was provided that checked the
  ...
  \fixme{Achim, not sure if we should just drop this point and refer to the point above?}
...
\item \textbf{Although it would involve substantial rewriting, I think the paper would be much more 
  ...
  of the manuscript. To improve the reading flow, we have also moved Section~5.3 to the appendix,
  since this section is probably less frequently used in practice.
  
  \fixme{Achim, moved Section~5.3 to the appendix, is the note above enough?}
