...
\item \textbf{Is there any option to run multiple chains within
  ...
  Unfortunately multiple chain/core processing is not implemented yet, but it is on our
  to do list.
...
\item \textbf{Not being able to store large objects is an issue
  ...
  \readme{Achim, probably we need to describe better that BayesX only provides
  predictions for unique covariate values etc.? However, this is already explained in
  Section~5.3. Additional options?}
...
\item \textbf{Although missing data is implicitly discussed in the
  ...
  \fixme{Achim, I suppose that the \code{na.action} argument is sufficient?}
  Missing data is processed by the \code{na.action} argument in \fct{bayesx}/\fct{parse.bayesx.input}.
  The default in \fct{bayesx} is \code{na.action = NULL}, which means that the function that is
  provided within the \fct{options} setting will be used, the default here is \fct{na.omit}.
  \pkg{BayesX} will then process on the data that is provided from \proglang{R}, in general,
  the \pkg{BayesX} binary will remove all \code{NA}s. \fixme{Stefan/Thomas, is that true?}
...
\item \textbf{How to compute posterior summary of transformed variables
  ...
  Prediction using new covariate values is not implemented yet, since the \pkg{BayesX}
  binary does not support prediction at the moment. The plan is to do a work around, i.e.\ construct
  design matrices for new covariate values within \proglang{R} directly and combinate them with the
  samples of the parameters for prediction.
  \fixme{Achim, should we note that in the paper?}
...
\item \textbf{It would be useful to provide some guidance on specifying
  ...
  explanation on this topic in Section~5.2 Available additive terms. The user may look up all
  default settings for all possible terms using function \code{bayesx.term.options()}.
  \fixme{Stefan/Thomas, any suggestions here?}
...
\item \textbf{Could the authors briefly compare BayesX to WinBUGS and INLA?
  ...
  and also a section with a \pkg{WinBUGS} comparison. \fixme{Achim, one or two words on \pkg{INLA}?}
...
\item \textbf{On page 21, please rewrite the sentence "The range of the
  ...
  \fixme{Stefan, Thomas, computation of the VPC for non Gaussian responses and multilevel models?}
...
\item \textbf{I tried to run the munich rent example that comes with INLA in R2BayesX
  ...
  Munich rent example code runs smoothly now
<<eval=FALSE>>=
...
\item \textbf{It would also be nice if a function was provided that checked the
  ...
  \fixme{Thomas, is there a official format for graph files?}
...
\item \textbf{Would it be better in the STEP algorithm to set the quantiles to NA
  ...
  \fixme{Stefan, Thomas, the output matrices could set \code{NA}s instead of \code{0}?
    The problem is that I cannot account for that in the interface, because I do not know 
    if all \code{0}s are really \code{NA}s!}

\item \textbf{In the bayesx() call, the family parameter is case sensitive (I
  accidentally wrote "Gaussian" instead of "gaussian"). This could be
  fixed with tolower(). A similar thing happened with CI = "MCMCselect".}

  \medskip

  The case sensitivity for the \code{family} and \code{CI} parameter has been removed.

\item \textbf{Section 2: The option "full = TRUE" in the formula is not
...
\item \textbf{Section 4.2: I note in passing that the INLA package can solve a
  ...
  \fixme{Achim? I do not know why he mentions Section 4.2 here?}
...
\item \textbf{Table 4:  It would be useful to specify which GMRF model is being used.
  ...
  \fixme{Stefan, Thomas? Can you add the references?}
...
\item \textbf{Table 7: This should include the "acf=TRUE" option used in Figure 7.}
...
\item \textbf{There seems to be an ordering problem with fitted.bayesx. I noticed this
  ...
  cor(fitted(b1)$mu,dat$y)  ## about 7% - should be about 80%, I think
  plot(fitted(b1)$mu,dat$y)  ## shows almost no relationship !
  \end{Sinput}
  \end{Schunk}
...
\item \textbf{The discussion of the relationship between the two packages is probably useful to 
