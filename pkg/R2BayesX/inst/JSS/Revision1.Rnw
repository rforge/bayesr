...
\item \textbf{Is there any option to run multiple chains within
  ...
  using the functionality of the \pkg{parallel} package. The user can now start models on e.g.\ 3
  cores with 2 chains each by typing
<<eval=FALSE>>=
  library("R2BayesX")

  ## load artificial data set
  data("GAMart")
...
@
  ...
  convenience function that directly computes the Gelman and Rubin's convergence diagnostic using
  function \code{gelman.diag()} of the \pkg{coda} package. E.g., the user may type
<<eval=FALSE>>=
...
\item \textbf{The STEP method for model/variable selection is
  ...
  heuristics as described in \medskip \\
  ...
  frequency each model term has been selected during the bootstrap iterations. We have now
...
\item \textbf{Not being able to store large objects is an issue
  ...
  optimized (see Section~5.3) to alleviate the memory burden to some degree. For example,
...
\item \textbf{Although missing data is implicitly discussed in the
...
\item \textbf{How to compute posterior summary of transformed variables
  ...
  function \fct{samples}, which can then e.g.\ be used in combination with function \fct{quantile}
...
\item \textbf{It would be useful to provide some guidance on specifying
  ...
  explanation on this topic in Section~5.2 Available additive terms. Please find more information
  on Bayesian P-Splines In: \medskip \\
  Lang S, Brezger A (2004). \emph{"Bayesian P-Splines."} Journal of Computational and Graphical
  Statistics, \textbf{13}, 183--212. \medskip \\
  Brezger A, Lang S (2006). \emph{"Generalized Structured Additive Regression Based on Bayesian
  P-Splines."} Computational Statistics \& Data Analysis, \textbf{50}, 947--991. \medskip\\
  The user may look up all default settings for all possible terms using function \\
...
\item \textbf{Could the authors briefly compare BayesX to WinBUGS and INLA?
  ...
  In the paper \medskip \\
  Brezger A, Kneib T, Lang S (2005). \emph{"\pkg{BayesX}: Analyzing Bayesian Structured Additive
  Regression Models."} Journal of Statistical Software, \textbf{14}(11), 1--22.
  URL~\url{http://www.jstatsoft.org/v14/i11/.} \medskip \\
  Section~3, a thorough comparison of software with built in functions for GAM modeling is presented
  and also a section with a \pkg{WinBUGS} comparison. We have added a small paragraph at the end of
  Section 3 that points out software packages which have a similar scope.


\item \textbf{On page 2, Section 5 does not appear in the last paragraph.}
...
\item \textbf{Sections 2 and 3 can be merged into one section, which first
...
\item \textbf{State in the caption of Figure 1 that data (both outcomes
  and covariates are missing?) of the areas shaded with
  vertical lines are missing.}
...
\item \textbf{Page 17, the third line underneath subsection 6.1,
 ``development countries" \code{->} ``developing countries"?}
...
\item \textbf{On page 21, please rewrite the sentence ``The range of the
  ...
  one of a Markov random field and the other of an $i.i.d.$ random effect. As has been shown in \medskip \\
  ...
  be said about the relative importance of both effects. Exception are cases where one of both
  effects (either the unstructured or the structured effect) is estimated practically zero and the
...
\item \textbf{On page 22, autocorrelation is one measure for assessing
  ...
  We have now added an example using 2 chains where we compute the Gelman and Rubin’s convergence
  diagnostic for certain parameters.


\item \textbf{Last line on page 22, ore \code{->} or?}
...
\item \textbf{The installation of R2BayesX on OSX was problematic. It didn't
  ...
  \fixme{Daniel, please check.}


\item \textbf{It looks like the authors misunderstood the way that R installs on a mac.
  This is fairly common when porting from unix.  It can be fixed with
  using a symbolic link, but that's not very satisfactory!  It would be
  better if they used the correct directory to begin with. To be honest,
  were I not reviewing this package, I probably wouldn't have spent the
  time tracking down a fix.}

  \medskip

  \fixme{Daniel, is there anything else to do here? I guess not and we can probably
  just delete the second part if this question\dots}


\item \textbf{I tried to run the munich rent example that comes with INLA in R2BayesX
...
\item \textbf{It would also be nice if a function was provided that checked the
  validity of graph files.}

  \medskip

  \fixme{Achim, not sure if we should just drop this point and refer to the above point?}


\item \textbf{Would it be better in the STEP algorithm to set the quantiles to NA
  (not available) rather than 0.00?}

  \medskip

  We have set these to \code{NA} now.


\item \textbf{In the bayesx() call, the family parameter is case sensitive (I
...
\item \textbf{Section 2: The option \code{full = TRUE} in the formula is not
...
\item \textbf{Section 3: Is the list on page 5 an exhaustive list of models
...
\item \textbf{Section 4.2: I note in passing that the INLA package can solve a
  ...
  We have now added a small paragraph at the end of Section 3 that points out software packages that
  have a similar scope.
  \fixme{Achim, is that sufficient?}


\item \textbf{Table 4:  It would be useful to specify which GMRF model is being used.
  ...
  The paper \medskip \\
  Fahrmeir L, Kneib T, Lang S (2004). \emph{"Penalized Structured Additive Regression for Space
  Time Data: A Bayesian Perspective."} Statistica Sinica, \textbf{14}, 731--761. \medskip \\
  provides a detailed discussion of the methodology used, we have added the reference in Table 4,
  too.
  \fixme{Achim, is that ok?}


\item \textbf{Bottom of page 15: ``several informations" \code{->} ``several pieces of
...
\item \textbf{Same place: ``further arguments may be passed to the function system()"
  <-- I would probably use 'can' instead of 'may' - I misread the sentence
  the first time.}
...
\item \textbf{Top of page 16: ``model.name the name of the model" \code{<-} missing is}
...
\item \textbf{Page 16 paragraph 2: What is a ``very large dataset" for BayesX? Are
  there benchmarks?}

  \medskip

  \pkg{BayesX} is able to handle rather complex models with several hundred thousand of
  observations. E.g., we have successfully fitted a large model using precipitation data with more
  than a million observations. See also \medskip \\
  Nikolaus Umlauf, Georg Mayr, Jakob Messner, and Achim Zeileis. Why does it always rain on me? A
  spatio-temporal analysis of precipitation in Austria. \emph{Austrian Journal of Statistics},
  \textbf{41}(1):81--92, 2012.


\item \textbf{Page 20: I'm not a fan of an enormous piece of R output breaking
  a sentence.}
...
\item \textbf{Figure 4: I am unsure what the x axis on these images stand for.  And
  ...
  The kernel density estimates are based on the mean of the sampled coefficients, i.e., the
  estimated mean coefficients for all regions are used. We have added some more explanation in 
  the caption of Figure~4.


\item \textbf{It would be nice to mention that all of these plotting options can be
  found in Appendix B.}
...
\item \textbf{Bottom of page 22: probably didn't mean ``ore"...}
...
\item \textbf{Same place: As this is a manual, it would be nice to give some
  indication of the thought process behind increasing the number of
  iterations to 20k or 30k.}

  \medskip

  \fixme{Stefan, can you add a sentence here?}


\item \textbf{Bottom of page 23: Most users would probably attack the 'verbose' option
...
\item \textbf{Example in 6.2: Is there an easy way to perform spatial prediction
...
\item \textbf{Something weird has happened in the second paragraph of page 34.}

  \medskip

  \fixme{Niki, some half-sentence seems to have been forgotten/lost here. The second
  sentence starts with ``In the userdefined case'' but this is not concluded before
  the third sentence ``Besides the default\dots'' starts.}

  \fixme{Stefan, how to define userdefined models? We need to add some more explanation in
  Section 6.3.}


\item \textbf{Table 7: This should include the \code{acf=TRUE} option used in Figure 7.}
...
\item \textbf{Would it be possible to store all the masses of output information from
...
\item \textbf{There seems to be an ordering problem with fitted.bayesx. I noticed this
...
@
  ...
  Thanks, the ordering problem has been fixed.
...
\item \textbf{It is not entirely clear who the authors have in mind as the audience for the paper.
  ...
  with \proglang{R} (and may have more or less experience with other \proglang{R}~packages like \pkg{mgcv}),
  (3)~\proglang{R}~programmers interested in designing interfaces to external programs for
...
\item \textbf{Although it would involve substantial rewriting, I think the paper would be much more 
  ...
  \fixme{Everyone: Should we do more? I would suggest to move Section~5.3 to an appendix as this is
  probably less frequently used in practice.}
  
\item \textbf{In its current form, the reader is simply provided with a list of options in Table 
  ...
  \fixme{Everyone: This would indeed require a lot of rewriting and would teach readers the
  STAR framework while providing \proglang{R} examples for the different types of models.
  My feeling is that this would be beyond the scope of the paper and we reference other resources
  with thorough STAR introductions. But maybe someone has an idea how to improve the manuscript
  in this direction without a major rewrite?}

\item \textbf{It might also be useful if the authors added a brief discussion of some of the
  ...
  \fixme{Niki, Stefan, Thomas: My feeling is that this is mainly terminology. Is there a really
  sharp definition where GAMs end and where more general STARs start? For example, the \pkg{mgcv}
  and \pkg{gamlss} packages employ the term GAM in a broader sense (not unsimilar to STAR) than
  other resources. If this is correct, then we could maybe just add some more explanations for
  a unifying view in Section~3.}
