...
\code{"ps"}, \code{"psplinerw1"}, \code{"psplinerw2"} & P-spline with first or second order
                                                        difference penalty. \\
\code{"te"}, \code{"tensor"}, \code{"pspline2dimrw1"} & Defines a two-dimensional P-spline based on the tensor
...
As mentioned in Section~\ref{sec:modelspecs}, the model term constructor function \fct{sx} basically
calls \pkg{mgcv}'s constructor function \fct{s} in the end. The output returned from \fct{s} is then
further processed by the generic function \fct{bayesx.construct} to create \pkg{BayesX}
interpretable commands. Hence, users may optionally call the constructor function \fct{s} directly.
...
@
produces a P-spline term of basis dimension 10 constructed of 8 inner knots and degree 3 B-spline
basis functions. To generate a \pkg{BayesX} term with the same base setting as provided from
function \fct{sx}, the user would need to specify argument \code{k} additionally with
<<appendix-s, echo=TRUE, eval=TRUE>>=
...
\code{ZambiaBnd} is supplied with \code{s(district, xt = list(map = ZambiaBnd))}.
