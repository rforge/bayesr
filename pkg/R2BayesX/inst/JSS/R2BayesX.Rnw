...
regression models before Section~\ref{sec:implementation} describes the implementation details.
Readers not interested in the implementational details of \pkg{R2BayesX} may directly jump to
the description of the user interface in Section~\ref{sec:userinterface}. In
Section~\ref{sec:illustrations}, the versatility of \pkg{BayesX}
...
usually fit for common regression problems, however, in some situations it might be useful to
evaluate the sensitivity of the results when changing certain parameters (number of knots, degree of
...
plot(fm2, term = "sx(id)", map = BeechBnd,
  interp = TRUE, outside = TRUE)
@
Plotting interpolated values now leads to a better representation of the effect. The argument
\code{interp} specifies if interpolation of estimated effects should be applied. Argument
\code{outside} specifies if values outside the polygon areas should be shown, which is necessary
in this example, since the polygon areas are relatively small. ()
...
\code{symmetric} & If set to \code{TRUE}, a symmetric legend will be drawn corresponding to the
          $\pm \max(|x|)$ of values $x$ that are used for plotting. \\
\code{interp} & If set to \code{TRUE}, values will be smoothly interpolated. \\
\code{outside} & If set to \code{TRUE}, interpolated values outside the polygon areas will be plotted. \\ \hline
\end{tabular}
