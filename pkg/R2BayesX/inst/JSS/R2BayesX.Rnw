...
computational performance of the supported models, the algorithms implemented utilize efficient 
(sparse) matrix architectures and are written in a \proglang{C++} environment. 
...
may now 
\begin{itemize}
\item specify and estimate STAR models directly from the \proglang{R} console, 
\item apply a set of extractor functions and methods on fitted-model objects, e.g. producing high
  level graphics of estimated effects and model diagnostic plots,
...
To give a first example of the various features of the interface, we demonstrate estimation of a 
Bayesian normal geoadditive regression model with the childhood malnutrition dataset in Zambia 
(see Section~\ref{subsec:zambia}) using Markov chain Monte Carlo (MCMC) simulation. Therefore, the 
...
@
where the possibly nonlinear effects of covariates \code{agechild} and \code{bmi} are modeled with
P-splines and the spatial effect by a geospline term using the centroids of the district polygons in 
object \code{ZambiaBnd}. The model is then fitted with the function call to 
...
@
Afterwards the estimated effects may be visualized e.g. typing
<<plot-illustration, echo=TRUE, eval=FALSE, fig=FALSE>>=
plot(b, map = ZambiaBnd)
@
which basically produces Figure~\ref{fig:illustration}.
\begin{figure}[t!]
...
par(mar = c(4.1, 4.1, 0.1, 1.1))
plot(b, term = "s(agechild)", resid = TRUE, cex = 0.1)
@ 
\\[2ex]
...
par(mar = c(0, 1.1, 0, 0))
plot(b, term = "s(district)", range = c(-0.5, 0.5), zlim = c(-1, 1), 
  swap = TRUE, theta = 40, extrap = TRUE, linear = FALSE)
@
\end{center}
...
effect for covariate \code{district} using an image plot with a color legend, the lower right panel 
draws the same effect with a perspective plot in a slightly rotated angle.}
\end{figure}

Besides models for responses from univariate exponential families, BayesX also supports models from 
...
Section~\ref{sec:illustrations}, the interface usability is further demonstrated on the complex 
...
A prior for a function $f_j$ is now defined by specifying a suitable design matrix $Z_j$ and a prior 
distribution for the vector $\boldsymbol{\beta}_j$ of unknown parameters. The general form of the 
prior for $\boldsymbol{\beta}_j$ is
