...
Subsection~\ref{subsec:zambia}. The structured additive predictor~(\ref{eqn:zambia-eta}) contains
two continuous covariates \code{bmi} and \code{agechild} that are assumed to have a possibly 
nonlinear effect on the response \code{stunting} and are modeled with P-splines. However, a linear 
effect could be more appropriate and, hence, the linear effect is also considered for model 
selection. Additionally, for each variable and function, the selection procedure decides if a term 
is included or removed from the model. 
<<fit-zambia-model-step, echo=TRUE, eval=FALSE>>=
...
data("ZambiaNutrition", "ZambiaBnd", package = "R2BayesX")
<<fit-zambia-model-step>>
save(zms, file = "zambia-model-step.rda")
}
@
