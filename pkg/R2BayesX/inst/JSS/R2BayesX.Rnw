...
supporting local regression and smoothing splines in combination of a backfitting algorithm and is
...
finally merged into a complete program which may be send to \pkg{BayesX}. Next to the integration of
function \fct{s}, a more \pkg{BayesX} alike model term constructor function, that accounts for
specific defaults and additional arguments, and is therefore seen as the main constructor function
of the package, has been implemented. To avoid package conflicts the new function simply interfaces
...
also assigned with suitable classes and have plotting methods. Particular effort has been raised on
