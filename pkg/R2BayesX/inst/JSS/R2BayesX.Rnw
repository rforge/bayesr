...
interface (API), amongst others, embedding the sources in an \proglang{R} package seemed to be
difficult since the development status of the project is very advanced. The effort of writing and
reading data is comparatively small to the processing time needed for estimation of STAR models
with \pkg{BayesX}. Therefore, we decided to build an interface that primarily sends input program
files to the command-line binary version of \pkg{BayesX} and reads the returned output files back
into \proglang{R}.  
...
\section[Appendix]{Appendix} \label{appendix:minstall}

\subsection[Installing the BayesX Windows binary]{Installing the \pkg{BayesX} Windows binary}
\label{appendix:wininstall}
...
\subsection[Installing the BayesX sources]{Installing the \pkg{BayesX} sources}
\label{appendix:otherinstall}
