...
The \proglang{R} package \pkg{R2BayesX} provides a semi-interactive interface to the standalone
software package \pkg{BayesX}. The implemented tools allow the user to specify and estimate STAR
models in a convenient \proglang{R} style oriented on commonly used regression model fitting
functions. 
