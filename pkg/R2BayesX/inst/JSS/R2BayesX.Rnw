...
\code{sx(mbmi, degree = 2, knots = 10)}. For supplying additional boundary or graph files, that are
...
\proglang{R} package \pkg{sp} \citep{R2BayesX:Pebesma+Bivand:2005} and are basically transformed
internally if passed to function \fct{sx} or \fct{plotmap} to the \code{"bnd"} format needed using
\pkg{BayesX}. However, the interplay of popular spatial classes will be extended in future work. Now
for instance, the necessary boundary file \code{ZambiaBnd} for the geokriging term in
...
\readme{Achim, the function is pretty static and returns all options for a given \code{bs} that are implemented in \pkg{BayesX}. Here we only show the first two since the list is quite long...}
...
spatial effect, both indicating a relatively good model fit. 
...
\fixme{Niki, what is pmean? Clarify.}
...
\fixme{Niki, again, some interpretation.}
...
hardly possible in this example. Moreover, the estimated effects of the first model seem to almost
completely vanish if a spatially correlated effect is included in the model.
...
also \fixme{Niki, clarify: Do you mean in general? In this example? Or\dots?}
...
and what specifies the maximal possible model. Maybe this could be clarified?}. Therefore, the starting
