...
paper presents the full interactive \proglang{R} interface \pkg{R2BayesX} to BayesX. With the new 
package, STAR models can be conveniently specified using \proglang{R}'s formula language (with some 
extended terms), fitted using the BayesX binary, represented in \proglang{R} with objects of 
suitable classes, and finally printed/summarized/plotted. As a result of the superior graphics 
capabilities of \proglang{R} package \pkg{R2BayesX} greatly enhances the usability of BayesX. On the 
other hand some of the more complex models from the STAR class, especially models for multivariate 
responses, extend the already impressive capabilities  for semiparametric regression in  
...
\item apply a set of extractor functions and methods on BayesX fitted model objects, e.g. producing
  ...
  (see Table~\ref{tab:funmethods}),
\item run already existing BayesX program files from \proglang{R}
...
As a third alternative BayesX provides a penalized least squares (or penalized likelihood)
...
with a path to a dataset stored on disc, which may be reasonable when using large datasets. An example is
...
are arranged by function \fct{bayesx.control}. Note that a call to \fct{bayesx.control} to set the
additional options is not necessary in function \fct{bayesx}, i.e. all controlling parameters may be
directly declared in \fct{bayesx}, since any argument set within "\code{...}" is automatically send
to \fct{bayesx.control}. The most important parameters for the different methods are listed in
...
may be useful to inspect the log-file generated by the BayesX binary. The file can either be viewed
...
\code{"gs"} & geosplines based on two-dimensional P-splines with first order random walk penalty:
              ...
              map object in boundary format, see function \fct{read.bnd}, within the \code{xt}
...
\code{"mrf"} & Markov random fields: defines a Markov random field prior for a spatial covariate,
               where geographical information is provided by a map object in boundary or graph
...
\code{"te"} & defines a two-dimensional P-spline based on the tensor product of one-dimensional
              ...
              the spline. \\ \hline
\code{"factor"} & special BayesX specifier for factors, especially meaningful if
...
\code{bin} specifies the location of the BayesX \code{bin}ary the program file is sent to, see also
...
typically reserve much less disc space, importing the fitted model objects into \proglang{R} using 
\fct{read.bayesx.output} is straightforward in most cases, whereas handling the complete dataset 
within \proglang{R} may be problematic. As mentioned in Subsection~\ref{subsec:processing}, users 
can exploit this by providing a character string to argument \code{data} in function \fct{bayesx}, 
which includes the path to a dataset instead of an \proglang{R} data object. As a consequence, this 
dataset will not be loaded within \proglang{R} and is only used internally by the BayesX binary. To 
give an example, we generate a large dataset that might produce problems with \proglang{R}'s memory 
allocation using a model fitting function, especially if the model contains a large number of 
parameters. Therefore, we store the data on disc in the temporary folder of the running session with
...
@
Since function $f_{str}$ is modeled by a Markov random fields term, BayesX needs  information
...
be used. With the information of the boundary file BayesX may compute an appropriate adjacency matrix,
...
the random number generator in BayesX, which is meaningful for exact replication of the examples.
...
plot(zm, term = c("s(bmi)", "s(agechild)"))
@
\begin{figure}[!ht]
\setkeys{Gin}{width=0.46\textwidth}
\centering
...
plot(zm, term = "s(district)", map = FALSE, main = "")
@
...
par(mar = c(4.1, 4.1, 0.4, 1.1))
plot(fm2, term = "s(id)", map = FALSE, main = "")
@ 
\\[2ex]
...
par(mar = c(0.1, 0.1, 0.1, 0.1))
range <- R2BayesX:::kde.quantiles(fitted(fm2, term = "s(id)")$Estimate)
range <- c(-1 * max(abs(range)), max(abs(range)))
plot(fm2, term = "s(id)", map = BeechBnd, pos = "topleft",
  height = 0.24, width = 0.41,
  range = range)
...
mf <- stunting ~ memployment + urban + gender +
  s(education, bs = "factor") + s(bmi, bs = "ps") + s(agechild, bs = "ps") +
  s(district, bs = "mrf", xt = list(map = ZambiaBnd)) + r(district)
