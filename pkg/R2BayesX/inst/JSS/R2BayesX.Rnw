...
that ships the \pkg{BayesX} \proglang{C++} sources along with a flexible \code{Makefile} so that upon
installation of the \proglang{R} package a suitable \pkg{BayesX} binary is produced on all platforms.
\readme{Daniel, is this technically sound? If not, please adapt.}
...
children (\code{agechild}), the body mass index of the mother (\code{mbmi}) and the district the children live
...
@
.
\fixme{Niki, my feeling is that the boundary file explanations are too lengthy and
interrupt the reading flow in this introductory example. Maybe you can shorten it and present
the details elsewhere? The details should then also make some reference to the sp tools.}
\readme{Achim, I explained all this now in Section~\ref{subsec:addterms}, should we do a ref to this
section here, too?}
...
b <- bayesx(f, family = "gaussian", method = "MCMC", data = ZambiaNutrition)
@
...
@
The plots are shown in Figure~\ref{fig:illustration}. Here, the estimated effect of the
mother's bmi is apparently negative for values up to 25. The resulting effect of covariate
\code{agechild} is decreasing from 0 to 20 month, for older children the effect remains almost 
constant. The map effect plot indicates that stunting is a larger problem in the northern parts of
Zambia, whereas the effect in the southern parts is mostly negative or relatively small. In
comparison, the effects of \code{mbmi} and the spatial effect seem to have a quite similar influence
in absolute magnitude, while the strongest driver of stunting appears to be covariate
\code{agechild}.

\readme{Achim, added some explanation, not sure what to say about the model summary since the info
is not very important since we do not compare other models?}
...
\readme{I rephrased the following paragraphs in order to get away from the narrative style in
past tense: We considered \dots and looked for \dots and developed \dots}
...
\pkg{BayesXsrc} along with a suitable \code{Makefile} so that upon installation of the
...
\fixme{Daniel, please check whether these paragraphs are technically sound or need
modification.}
...
the development of easy-to-use map effect plots using color legends (using HCL-based palettes,
\citealp{R2BayesX:Zeileis+Hornik+Murrell:2009}, from the \pkg{colorspace} package, \citealp{R2BayesX:Ihaka+Murrell+Hornik:2012}).
See also Section~\ref{subsec:processing} and Section~\ref{sec:illustrations}.

\fixme{Niki, somewhere we need to briefly discuss the type of support for spatial objects.
We could do this here, or probably better in the next section. This needs to say something
about how to connect to the standard sp classes.}
\readme{Achim, info is now in Section~\ref{subsec:addterms}.}
...
Note that some terms have equivalent short and long specifications, e.g.,
...
term. The ``\code{...}'' argument is used to set term-specific control parameters
or additional geographical information. For example to modify the degree and the inner knots for the
P-spline term \code{sx(mbmi)} from Section~\ref{sec:motivation}, the user could use
\code{sx(mbmi, degree = 2, knots = 10)}.
\readme{Achim, added some info on \code{"bnd"} and \pkg{sp} in the following \dots}
For supplying additional boundary or graph files, that are
used to compute suitable neighborhood penalty matrices for terms using Markov random field priors,
or to calculate the centroids of particular regions for geosplines and geokriging terms, an argument
named \code{map} needs to be provided. Boundary files are basically shapefiles (see package
\pkg{shapefiles}, \citealp{R2BayesX:Stabler:2006}) that only include information on the polygons
that form the map and may be constructed using function \fct{shp2bnd} or imported with
\fct{read.bnd} (see also function \fct{read.gra} and \fct{bnd2gra} for importing and creation of
spatial neighborhood information objects in graph format that may also be sent to \pkg{BayesX}).
Currently, package \pkg{R2BayesX} has only limited support for map objects of class
\code{"SpatialPolygonsDataFrame"}, that are created within the functionality of the
\proglang{R} package \pkg{sp} (\citealp{R2BayesX:Pebesma+Bivand:2005}, e.g.\ by reading shapefiles
with function \fct{readShapePoly}) and are basically transformed internally to the \code{"bnd"}
format needed using \pkg{BayesX} if passed to function \fct{sx} or \fct{plotmap}. However, the 
interplay of popular spatial classes will be extended in future work. Now for instance, the 
necessary boundary file \code{ZambiaBnd} for the geokriging term in Section~\ref{sec:motivation} is 
included with \code{sx(district, bs = "gk", map = ZambiaBnd)}. Information about all possible extra 
arguments for a particular term basis/type can be looked up using function
\fct{bayesx.term.options}, e.g., possible options for P-splines using \code{"MCMC"} are printed to 
the console by 
\begin{Sinput}
...
\fixme{Niki, is it possible to extend/modify this? Could I request more than two options?}
\readme{Achim, the function is pretty static and returns all options for a given \code{bs} that are implemented in \pkg{BayesX}. Here we only show the first two since the list is quite long \dots}
...
dataset within \proglang{R} may be more burdensome. As mentioned in Section~\ref{subsec:processing}, 
...
respectively. With the information of the boundary file \pkg{BayesX} may compute an appropriate adjacency 
matrix, allowing for a smoothly varying effect of the neighboring regions. There is a generic 
plotting method implemented for objects of class \class{bnd}, which essentially calls function
...
credible intervals.}
\end{figure}
and are shown in Figure~\ref{fig:zambia-agechild}. The resulting plots indicate that the effect
of the age of the child exceeds the influence on \code{stunting} from the mother's bmi. The mother's
bmi has a negative effect on \code{stunting} for values up to 25, where the effect of the age of the
child is decreasing for 0 to 20 month of age and remains almost constant and slightly negative for
older children. Moreover, the plot of the effect for the mother's bmi suggests that the effect
could also be modeled approprietly by a linear term.

\fixme{Niki, some interpretation should be given for the results so far.}
\readme{Achim, added some text...}
...
\end{figure}
The kernel densities reveal the general form of the random effects distributions, while the
range of the estimated random spatial effect is much smaller than the range of the structured
spatial effect, both indicating that the model specification is relatively accurate in this
situation. 

\fixme{Niki, tne sentence explaining the findings from this figure should be provided.}
...
\fixme{Niki, what is pmean? Clarify.} \readme{Achim, hope its clear now?}
...
structured spatially correlated effect clearly exceeds the unstructured effect, as mentioned
above, also indicating that for future analysis and detecting the drivers of stunting additional
information needs to be gathered since the spatial effect could probably be explained through other
covariates.

\fixme{Niki, again, some interpretation.} \readme{Achim, added some text here \dots}
...
We can update the  model by adding a \code{"gs"} effect \readme{updating the model object directly rather than the formula}:
<<fit-forest-model-02-show, echo=TRUE, eval=FALSE>>=
<<fit-forest-model-02>>
@
Taking a look at model information criteria \fixme{Niki, the implementation of BIC with df is non-standard in R.
\code{logLik} returns a df attribute but \code{AIC} and \code{BIC} just return the information criteria}
...
\caption{\label{fig:forest-spatial-kde} Forest damage: Kernel density estimate of the spatial 
  effect (upper panel) together with a map effect plot (lower panel).}
\end{figure}
...
plot(fm2, term = "sx(id)", map = BeechBnd)
@
Both the kernel density and map effect plot are shown in Figure~\ref{fig:forest-spatial-kde}.
In this example the coloring of the plot is strongly influenced by a few very high and low values. 
Therefore, it might be useful to restrict the color range e.g., according to the 10\% and 90\% 
quantiles of the kernel density estimate of the effect,  by typing
...
\fixme{Niki, check the computations of \texttt{range} in these figures, shouldn't they be in sync?}
\readme{Achim, fixed...}
\begin{figure}[!ht]
\setkeys{Gin}{width=0.65\textwidth}
\centering
...
color scaling based on the 10\% and 90\% quantiles of the kernel density estimate of the effect.}
\end{figure}
...
\fixme{Niki, the final conclusion of the results could maybe be somewhat clearer: What are
the drivers of defoliation here?}
\readme{Achim, I don't know, I think here it is relatively clear, effects seem to vanish if we
add the spatial effect, we simply don't have covariates, except age, that can explain defoliation
well!?}
...
with P-splines. However, a linear effect could be more appropriate in general and, hence, the
linear effect is also \fixme{Niki, clarify: Do you mean in general? In this example? Or\dots?}
...
Another important feature of the stepwise procedure is the definition of a \code{startmodel}, the
options are listed in Table~\ref{tab:control}. Besides the default of an \code{"empty"}
\code{startmodel}, it may be reasonable to start with a user-defined model.
\fixme{Niki, it is somewhat unclear to me which part of the formula specifies the startmodel
and what specifies the maximal possible model. Maybe this could be clarified?}
\readme{Achim, using the user-defined option means that the algorithm starts with user-defined
degrees of freedom (complexity) for the nonlinear modeled terms, the formula for the start model
is then the same as the user has specified, in Table~\ref{tab:control} the options are described.}
Therefore, the starting values for the degrees of freedom of the P-spline, spatial and random effect
terms need to be specified, by typing
...
zmsud <- bayesx(f, family = "gaussian", method = "STEP",
  ...
  iterations = 10000, step = 10, seed = 123, data = ZambiaNutrition)
@
which actually produces the model output of the first model again. Summarizing these results,
in \pkg{BayesX}, the stepwise procedure is a fast and stable option for selecting smooth terms
and smoothing parameters in cases where it is not clear if terms should be modeled nonlinearly or
parametrically or even dropped from the final model specification.

\fixme{Niki, again I would like to see somewhat more interpretation. There is a little bit here but
the advantages/differences to the first analysis could be pointed out more clearly.}
...
\fixme{Daniel, bitte Beschreibung von \pkg{BayesXsrc} ergaenzen.}
...
\fixme{A short paragraph given some overview of the general approach implemented would be useful
