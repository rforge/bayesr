...
\citep[current version~2.0.1,][]{R2BayesX:Belitz+Brezger+Kneib+Lang:2011} \fixme{update version prior to resubmission to JSS}
...
\readme{The following paragraph has been rewritten in response to Reviewer~4, please check or further improve.}
...
Software packages which have a similar scope are the \proglang{R} package \pkg{INLA}
...
sampling but on integrated nested Laplace approximation (INLA).
\fixme{Thomas, can you add some words about \pkg{INLA} here?}
The latter packages apply Bayesian inference using Gibbs sampling, where the user can in principle
program a number of very complex models, also the ones covered by \pkg{R2BayesX}, but due to their
flexible MCMC sampler and the data management/efficiency, STAR models usually run very long and 
have inferior sampling properties or even do not converge. For a comparison of \pkg{BayesX} with
other software packages including \pkg{WinBUGS} see also
\citet[Section 3]{R2BayesX:Brezger+Kneib+Lang:2005}.
...
usually be well-suited for common regression problems. However, in some situations it might be useful to
evaluate the sensitivity of the results when changing certain parameters (such as number of knots, degree of
the spline, hyperpriors, etc.), e.g., when modeling highly oscillating functions 20~knots may not
be sufficient to capture the overall curvature.
\readme{Achim, added this line due to comment of reviewer 1. Do you think this needs further explanations?
Niki, I think the paragraph is intelligible. But it would be even better if we could reference some
resource where more details or a worked example are provided. Is there some section in the BayesX
manual for this?}
...
Kernel density estimates of the mean coefficients for all regions of the structured, left panel, and
the unstructured spatial effect, right panel respectively.}
\end{figure}
...
run (e.g., 22000 or even 32000 iterations).
...
geospline that replicates the analysis of \citet{R2BayesX:Fahrmeir+Kneib+Lang:2009}. The associated
...
\code{"userdefined"} case \fixme{Stefan/Thomas, how to define userdefined models?} 
