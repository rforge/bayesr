...
@
\end{center}
\caption{\label{fig:illustration} Illustration example: comparison of estimated effect with the 
package \pkg{mgcv} model fitting function \fct{gam} using GCV, left panel, and, \pkg{BayesX} model 
fitting function \fct{bayesx} using MCMC, right panel respectively.}
\end{figure}
