...
effects, e.g., for geographical or spatio-temporal data. This allows to specify
complex and realistic models that can be estimated using Bayesian inference based on modern Markov
chain Monte Carlo (MCMC) simulation techniques, based on a mixed model representation of STAR models, or with stepwise regression techniques combining penalized least squares estimation with model selection.
...
the already impressive capabilities  for semiparametric regression in  \proglang{R}. Moreover,
...
using function \fct{getscript}. The produced \proglang{R} may be useful for less experienced users
of the package to get a quick overview of post estimation commands. Moreover, the script facilitates
the final preparation of plots and diagnostics to be included in publications.
A list of all available functions and methods of package \pkg{R2BayesX} can be found in
...
plot(zm, term = "r(district)", map = ZambiaBnd,
  ...
  range = range, lrange = lrange)
@ %$
The resulting map is also shown in Figure~\ref{fig:zambia-district-structured-unstructured}. Using
for both the structured and the unstructered effect the same scale is useful for comparison. In most
cases one of the two effects clearly dominates the other. In our case the structured spatially
correlated effect clearly exceeds the unstructured effect.
...
\end{figure}
as shown in Figure~\ref{fig:forest-spatial}. In this example the coloring of the plot is strongly
influenced by a few very high values. Therefore, it might be useful to restrict the color range e.g.
according to the 5\% and 95\% quantiles of the kernel density estimate of the effect, e.g. by typing
<<forest-spatial-id-restrict, echo=TRUE, eval=FALSE>>=
plot(fm2, term = "s(id)", map = BeechBnd,
  pos = "topleft", range = c(-4, 4))
@
the resulting map and kernel density plot of the effect are shown in
Figure~\ref{fig:forest-spatial-restrict}.
\begin{figure}[!ht]
\setkeys{Gin}{width=0.65\textwidth}
\centering
...
plot(fm2, term = "s(id)", map = FALSE, main = "",
  kde.quantiles = TRUE, probs = c(0.05, 0.95))
@ %$
\caption{\label{fig:forest-spatial-restrict} Forest damage: Estimate of the spatial effect with
color scaling based on the 5\% and 95\% quantiles of the kernel density estimate of the effect. The
kernel density estimate together with the 5\% and 95\%, represented by the dashed lines, are shown
in the lower panel.}
\end{figure}
