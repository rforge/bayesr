...
\fixme{Niki, point out more clearly that these do the same as sx() and are limited by what BayesX can do}
\readme{Achim, included some extra info here \dots}
...
arguments are limited to arguments corresponding to \pkg{BayesX} available arguments given by 
\begin{Code}
  s(..., k = -1, bs = "ps", m = NA, by = NA, xt = NULL)
\end{Code}
Note that within \fct{sx}, the default values for the arguments are the same as the defaults in
\pkg{BayesX}, while the defaults using \fct{s} with package \pkg{R2BayesX} remain unchanged as
specified in package \pkg{mgcv}.
