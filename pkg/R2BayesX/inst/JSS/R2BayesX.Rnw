...
\item run already existing \pkg{BayesX} input program files from \proglang{R} \\
  (function \fct{run.bayesx}, Section~\ref{sec:implementation}),
\item automatically import \pkg{BayesX} output files into \proglang{R} \\
  (function \fct{read.bayesx.output}, Section~\ref{sec:implementation}).
\end{itemize}
...
implementing new terms for the model fitting function \fct{gam} but also greatly supports the syntax
translation methods implemented in \pkg{R2BayesX}. The major reason for interfacing function \fct{s}
is to provide a similar adjustable, but more adaptive set up to the \pkg{BayesX} specific model
terms, specifications and defaults. Furthermore, using MCMC estimation, the number of options that
may be set for particular model terms is large, wherefore the representation of a function call to
\fct{sx} is slightly more parsimonious. 
...
\eta &=& \gamma_0 + \gamma_1\texttt{memployment1} + \gamma_2\texttt{urban1} +
         \gamma_3\texttt{gender1} + \nonumber \\
     & & \gamma_4\texttt{educationno/incomplete} + \gamma_5\texttt{educationminimum primary} + 
         \nonumber \\
...
respectively, likewise for covariates \code{gender1}, \code{urban1}, \code{educationno/incomplete}
and \code{educationminimum primary}. As mentioned in the introduction, functions $f_1$ and $f_2$ of
the continuous covariates \code{agechild} and \code{bmi} are assumed to have a possibly nonlinear
...
f <- stunting ~ memployment + urban + gender + education +
...
The data set on forest health comprises information on the defoliation of beech trees, which serves
