...
improved computational performance of the supported models, the algorithms implemented utilize 
...
BayesX, which has recently been added to the existing Comprehensive \proglang{R} Archive Network 
(CRAN) package \pkg{BayesX} \citep{R2BayesX:Kneib+Heinzl+Brezger:2011}. Within the new version of 
the package, users may now 
\begin{itemize}
...
linear models (GLM) (e.g. see \citet{R2BayesX:Nelder+Wedderburn:1972} and 
...
STAR models \citep{R2BayesX:Fahrmeir+Kneib+Lang:2004, Brezger+Lang:2006}, the linear predictor is 
replaced by a more general and flexible, structured additive predictor
...
\proglang{R}'s memory allocation using a model fitting function, especially if the model contains of
...
presented datasets have been added to package \pkg{BayesX}, ensuring straightforward traceability of 
the following code. In the first example, a Gaussian regression model is estimated using Markov 
...
\citet{R2BayesX:Fahrmeir+Tutz:2001} and \citet{R2BayesX:Kneib+Fahrmeir:2004}). The last example 
...
> b.regress stunting = bmi(psplinerw2,nrknots=8,degree=3) + 
    ...
    family=gaussian iterations=12000 burnin=2000 maxint=150 step=10 setseed=123 
    aresp=1 bresp=0.005 predict using d
...
defoliation, with similar findings for covariates \code{inclination} and \code{elevation}.
However, the estimate of the \code{age} effect seems to be improved in terms of monotonicity, see 
...
@
\end{center}
\caption{\label{fig:forest-spatial-nonpara} Forest damage: Estimated effect of covariate \code{age} 
including 80\% and 95\% point-wise confidence intervals of the model including the spatial effect.}
\end{figure}
...
Summarizing these results identifies a strong influence of the spatial effect of the overall model 
...
function, the procedure decides if a term is included or removed from the model. To estimate the
model using the stewise algorithm we use the same model formula of Subsection~\ref{subsec:zambia} 
and call
...
zmscb <- bayesx(mf, family = "gaussian", method = "STEP", 
  CI = "MCMCselect", iterations = 10000, setp = 10,
  data = ZambiaNutrition)
@
...
save(zms, zmscb, file = "zambia-model-step.rda")
}
@
The summary statistics of the final selected model are then provided with
<<zambia-model-step-summary, echo=TRUE, eval=TRUE>>=
summary(zms)
@
Note that variable \code{memployment} was removed from the model and variable \code{bmi} is modelled 
by a linear effect. Moreover, the colums \code{sd}, \code{2.5\%}, \code{50\%} and \code{97.5\%} 
contain no values. The posterior quantiles may be computed if argument \code{CI} in function 
\fct{bayesx.control} is specified. E.g. conditional confidence bands can be calculated conditional 
on the selected model, i.e. they are computed for selected variables and functions only. The 
computation of conditional confidence bands is based on an MCMC-algorithm subsequent to the 
selection procedure.  For the selection of a model with an adjacent computation of conditional 
confidence bands the user may type
<<fit-zambia-model-step-03, echo=TRUE, eval=FALSE>>=
zmscb <- bayesx(mf, family = "gaussian", method = "STEP", 
  CI = "MCMCselect", iterations = 10000, step = 10,
  data = ZambiaNutrition)
@
Using conditional confidence bands for estimation the summary statistics give
