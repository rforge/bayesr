...
influenced by a few very high values of the estimate. Therefore, it might be useful to restrict the 
color range, e.g. according to the 5\% and 95\% quantiles of a kernel density estimate 
of the effect by typing
<<forest-spatial-id-restrict, echo=TRUE, eval=FALSE>>=
plot(fm2, term = "s(id)", map = BeechBnd, 
  pos = "topleft", range = c(-3.8, 3.8))
@
The resulting map and kernel density plot are shown in Figure~\ref{fig:forest-spatial-restrict}. 
\begin{figure}[!ht]
\setkeys{Gin}{width=0.65\textwidth}
\centering
...
in the lower panel.}
\end{figure}
Trimming the color range of the plot now leads to a more informative representation of the image.
