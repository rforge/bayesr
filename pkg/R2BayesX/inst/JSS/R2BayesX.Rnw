...
compromise of creating an interface was seen advantageous, not only because of a faster development 
...
suitable classes, which may then be printed, summarized and visualized.

The interface described in this paper has been added to the CRAN package \pkg{BayesX} 
\citep{Kneib+Heinzl+Brezger:2011}, which previously provided mainly functions for exploring 
estimation results.
...
belongs to an exponential family with mean $\mu = E(y | u, \gamma)$ linked to a linear predictor 
$\eta$ by
\begin{equation} \label{eqn:glm}
\mu = h(\eta) \qquad \eta = u^{\prime}\gamma,
\end{equation}
...
\eta = f_1(z_1) + \ldots + f_p(z_p) + u^{\prime}\gamma,
\end{equation}
...
The unified representation of a STAR predictor arises from the fact that all functions $f_j$ may be 
specified by a basis function approach, where the vector of function  evaluations 
$f_j = (f_j(x_{1j}),\ldots,f_j(x_{nj}))$, with $i = 1,\ldots,n$ observations, can be written in 
matrix notation 
\begin{equation} \label{eqn:matnot}
f_j = X_j\beta_j,
\end{equation}
with $X_j$ as a design matrix, that only depends on the prior assumptions about smoothness of $f_j$, 
and $\beta_j$ are unknown regression coefficients which have to be estimated. Hence, the predictor 
of (\ref{eqn:structadd}) may be rewritten by
\begin{equation} \label{eqn:structaddmat}
f_j = X_1\beta_1 + \ldots X_p\beta_p + U\gamma,
\end{equation}
where $U$ corresponds to the usual design matrix for the fixed effects.
...
For all inferential concepts, the prior for the functions $f_j$ depends on the design matrix $X_j$, 
as e.g. determined by the functional types described above, and a prior for the vector $\beta_j$, 
which may also be written in a general form given by
\begin{equation} \label{eqn:prior}
p(\beta_j | \tau_j^2) \propto exp \left(- \frac{1}{2\tau_j^2} \beta_j^{\prime}K_j\beta_j\right),
\end{equation}
with $K_j$ as a quadratic penalty matrix that shrinks parameters towards zero or penalizes too 
abrupt jumps between neighboring parameters. In most cases $K_j$ will be rank deficient and the 
prior for $\beta_j$ is partially improper.
...
\code{defoliation} & percentage of tree defoliation in three ordinal
                     categories, 0 == `defoliation < 12.5\%', 1 == `12.5\% <=
                     defoliation < 50\%' and 3 == `defoliation >= 50\%'. \\ \hline
...
\begin{center}
\setkeys{Gin}{width=0.49\textwidth}
<<forest-age, echo=FALSE, fig=TRUE, width=5, height=4>>=
par(mar = c(4.1, 4.1, 0.1, 2.1))
plot(fm, term = "s(age)")
@
<<forest-year, echo=FALSE, fig=TRUE, width=5, height=4>>=
par(mar = c(4.1, 4.1, 0.1, 2.1))
plot(fm, term = "s(year)")
@
\\[2ex]

<<forest-inclination, echo=FALSE, fig=TRUE, width=5, height=4>>=
par(mar = c(4.1, 4.1, 0.1, 2.1))
plot(fm, term = "s(inclination)")
@
<<forest-canopy, echo=FALSE, fig=TRUE, width=5, height=4>>=
par(mar = c(4.1, 4.1, 0.1, 2.1))
plot(fm, term = "s(canopy)")
@
\end{center}
\caption{\label{fig:forest-spatial-nonpara} Forest damage: estimates of nonparametric 
effects including 80\% and 95\% point-wise confidence intervals.}
\end{figure}

\begin{figure}[t!]
\begin{center}
\setkeys{Gin}{width=0.49\textwidth}
<<forest-id-structured, echo=FALSE, fig=TRUE, width=7, height=4>>=
par(mar = c(0, 0, 0, 0))
plot(fm, term = "s(id)", map = BeechBnd, 
  pos = "topleft", width = 0.4, height = 0.2, 
  distance.labels = 2, swap = TRUE)
@
<<forest-id-unstructured, echo=FALSE, fig=TRUE, width=7, height=4>>=
par(mar = c(0, 0, 0, 0))
plot(fm, term = "s(id)", map = BeechBnd, 
  pos = "topleft", width = 0.4, height = 0.2, 
  distance.labels = 2, swap = TRUE)
@
\end{center}
\caption{\label{fig:forest-spatial} Forest damage: estimates of the structured, left panel, and 
unstructured spatial effects, right panel respectively.}
\end{figure}
