...
\author{Nikolaus Umlauf\\Universit\"at Innsbruck \And
        ...
        Daniel Adler\\Universit\"at G\"ottingen}
\Plainauthor{Nikolaus Umlauf, Thomas Kneib, Stefan Lang, Achim Zeileis, Daniel Adler}
...
The remainder of this paper is as follows. The next section gives a first example of a common 
\proglang{R} session applying \pkg{R2BayesX} on a dataset on childhood malnutrition in Zambia.
Section~\ref{sec:model} briefly discusses the methodological background of regression models with a
structured additive predictor, before a description on implementation details and the user interface
is given in Section~\ref{sec:implementation} and \ref{sec:userinterface} respectively. In
Section~\ref{sec:illustrations}, the interface usability is further illustrated with
the childhood malnutrition data and a dataset on forest health.
...
The functions $f_j$ are possibly
...
evaluations $\mathbf{f}_j = (f_j(\mathbf{z}_{1}),\ldots,f_j(\mathbf{z}_{n}))^{\prime}$ of the
...
The design of the interface  attempts to address the following major issues: First, the interfacing
function calls should be as practical as possible and oriented on \proglang{R}'s standards for
regression model fitting functions, second, to provide a set of functions and methods for a
convenient representation of fitted model objects, that would enhance the usability of \pkg{BayesX}.

In the developing process of package \pkg{R2BayesX} we first considered how to connect \pkg{BayesX}
and \proglang{R} most appropriately. Because \pkg{BayesX} does not have an application programming
interface (API), amongst others, adapting the sources in such a way seemed to be difficult since the
development status of the project is very advanced. The effort of writing and reading data is
comparatively small to the processing time needed for estimation of STAR models with \pkg{BayesX}.
Therefore, we decided to build an interface that primarily sends input program files to the
command-line binary version of \pkg{BayesX} and reads the returned output files back into
\proglang{R}. To provide an easy and \proglang{R} standard installation process of the \pkg{BayesX}
binary, the new package \pkg{BayesXsrc} containing the \pkg{BayesX} sources was developed
additionally (CRAN, \url{http://CRAN.R-project.org/package=BayesXsrc}, see also
Section~\ref{appendix:BayesXsrc} of the Appendix).

We also looked for existing \proglang{R} implementations to get an overview of best practices for
software designs including GAM's. The following gives a short summary of some of the larger
projects that have added to CRAN. One of the first implementations of GAM's in \proglang{R} is the
...
actually a version of the \proglang{S}-Plus routines for GAM's. The probably best-known and also
recommended package is \pkg{mgcv} \citep{R2BayesX:Wood:2006,
R2BayesX:Wood:2011b, R2BayesX:Wood:2011}, which provides fast an stable algorithms for estimating
GAM's based on GCV, REML and others. Vector generalized additive models (VGAM,
\citealp{R2BayesX:Yee:1996}) for categorical responses are covered by package \pkg{VGAM}
\citep{R2BayesX:Yee:2009}. Another comprehensive package for GAM's, accounting for responses that do
not necessarily follow the exponential family and may exhibit heterogeneity, is \pkg{gamlss}
\citep{R2BayesX:Rigby+Stasinopoulos:2005, R2BayesX:Stasinopoulos:Rigby:2007}. A package based on
mixed model technologies is \pkg{SemiPar} \citep{R2BayesX:Ruppert+Wand+Carrol:2003,
R2BayesX:Wand:2010} and its dependent version for adaptive splines \pkg{AdaptFit}
\citep{R2BayesX:Krivobokova}. The package \pkg{spikeSlabGAM} applies Bayesian variable selection,
model choice and regularization for GAMM's \citep{R2BayesX:Scheipl:2011}.

Within \proglang{R}, regression models are most conveniently specified using its formula language.
By reviewing the above mentioned packages for GAM's, we searched for existing structures that on the
hand could support a translation method for \proglang{R} commands to \pkg{BayesX} interpretable
syntax, and on the other hand, functionality that is already well established, such that applying
\pkg{R2BayesX} only requires little additional effort for the common \proglang{R} user.
Interestingly, most of the packages use different standards for incorporating additive
(smooth/special) terms in model formulas and the model frame building process, while a popular name
convention for a model term constructor function is ``\code{s}'', as it is used in package
\pkg{gam}, \pkg{mgcv} and \pkg{VGAM}. However, the implementations are again more or less different
from each other, i.e.\ loading packages simultaneously will result in conflicts. The most flexible
project seems to be package \pkg{mgcv}. Here, similar to package \pkg{gam} and \pkg{VGAM}, function
\fct{s} does not evaluate design or penalty matrices, but it returns a smooth term definition object
of class \code{"xx.smooth.spec"}, where \code{"xx"} may be specified by the user. To set up a model
with a user defined smooth, a method for the \proglang{S}3 generic function \fct{smooth.construct},
that returns a design matrix etc., needs to be supplied. Since implementation of additional model
terms is also a concern for \pkg{R2BayesX} and function \fct{s} is a quite parsimonious solution, we
adopted its functionality and developed methods for a new generic function \fct{bayesx.construct},
...
finally merged into a complete program which may be sent to \pkg{BayesX}. Next to the integration of
function \fct{s}, a more \pkg{BayesX} alike model term constructor function is provided. The
advantage is that a common \pkg{BayesX} user may specifiy models in a more familiar way, which will
make a change to \pkg{R2BayesX} more easy. The function is called \fct{sx} and is
described in Section~\ref{subsec:addterms}. To avoid package conflicts the new function simply
interfaces function \fct{s}. Moreover, a special constructor function for random effect terms is
provided.

The sequence of functions that are consecutively called using the interface is the following: A
final program file is generated by first applying function \fct{parse.bayesx.input} on the
...
described above, as well as setting up the necessary data files (folders) to be used with
\pkg{BayesX}. Afterwards, function \fct{run.bayesx} executes the program through a call to function
\fct{system}. The output files returned by the binary are imported into \proglang{R} using function
\fct{read.bayesx.output}. Hence, it is also possible to run and read already existing \pkg{BayesX}
...
\code{"bayesx"}, wherefore a set of base \proglang{R} functions and methods described in
Table~\ref{tab:funmethods}, amongst others, is available. The returned fitted model term objects are
also assigned with suitable classes which have plotting methods. Particular effort has been raised on
the development of easy to use map effect plots using color legends, see also
Section~\ref{subsec:processing} and Section~\ref{sec:illustrations}.

Functions \fct{parse.bayesx.input}, \fct{write.bayesx.input}, \fct{run.bayesx} and \\
\fct{read.bayesx.output} are combined in the front end main model fitting function \fct{bayesx}
presented in Section~\ref{subsec:processing}.
...
Therefore the \proglang{R} package \pkg{BayesXsrc} (CRAN,
\url{http://CRAN.R-project.org/package=BayesXsrc}) needs to be installed and loaded. The package
features the \pkg{BayesX} sources that have been succesfully compiled for the most popular operating
systems. For more details on this package please see Section~\ref{appendix:BayesXsrc} of the
Appendix.
...
typically improves  convergence of estimation algorithms used in \pkg{BayesX}. The distribution
...
\multirow{2}{*}[-2.9cm]{\code{"STEP"}} & \code{criterion} & character, specifies the goodness of fit
                   ...
                   estimate the mean squared prediction error (MSEP) which serves as the goodness of
                   fit criterion to compare different models),
                   \code{"GCV"} (Generalized Cross Validation based
                   on deviance residuals, see e.g. \citealp{R2BayesX:Wood:2006}), 
                   \code{"GCVrss"} (Generalized Cross Validation based on residual sum of squares,
                   see e.g. \citealp{R2BayesX:Wood:2006}),
                   \code{"AIC"} (Akaike Information Criterion, see e.g.
                   \citealp{R2BayesX:Burnham+Anderson:1998}), 
                   \code{"AIC\_imp"} (improved AIC with bias correction for regression models, see
                   e.g. \citealp{R2BayesX:Burnham+Anderson:1998}),
                   \code{"BIC"} (Bayesian information criterion, see e.g.
                   \citealp{R2BayesX:Hastie+Tibshirani+Friedmann:2009}),
                   \code{"MSEP"},
                   \code{"CV5"} (5--fold cross validation, see e.g
                   \citealp{R2BayesX:Hastie+Tibshirani+Friedmann:2009}),
                   \code{"CV10"} (5--fold cross validation, see e.g
                   \citealp{R2BayesX:Hastie+Tibshirani+Friedmann:2009}) and
 ...
 & \code{startmodel} & character, defines the start model for variable selection. Options are
                    \code{"linear"} (start model with degrees of freedom euqal to one for model
                    terms), \code{"empty"} (empty model containing only an intercept), \code{"full"}
                    (most complex possible model) and \code{"userdefined"} (start model is specified
                    by the user), default: \code{"linear"}. \\
\hline
\end{tabular}
...
of spatial objects that may be sent to \pkg{BayesX}), that are used to compute suitable neighborhood
...
dots \code{"..."} argument. The function returns a list including the log-file returned by
...
calculated by the centroid positions of tree polygons given by the boundary map file, which may be
loaded with
...
\code{startmodel}, it may be reasonable to start with an userdefined model. Therefore, the starting
values for the degrees of freedom of the P-spline, spatial and random effect terms need to be
specified, by typing
...
\subsection[Installing the BayesX binary - R package BayesXsrc]{Installing the \pkg{BayesX} binary - 
  \proglang{R} package \pkg{BayesXsrc}}
\label{appendix:BayesXsrc}

\fixme{Daniel: hier steht die Beschreibung von \pkg{BayesXsrc}}
