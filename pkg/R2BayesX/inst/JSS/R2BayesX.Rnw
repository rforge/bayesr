...
estimated models, the mean, standard deviation and quantiles of parameter samples are provided. See
Table~\ref{tab:funmethods} for a list of all available functions and methods.
\begin{table}[t!]
\begin{center}
...
The usage of \fct{s} in \pkg{BayesX} is in principle similar to package \pkg{mgcv}. However, only a 
reduced list of arguments and options is available using the interface. The implemented arguments 
are
\begin{Sinput}
...
\hypertarget{run.bayesx}{Function} \fct{run.bayesx} is used to run a BayesX program file. The 
arguments of \fct{run.bayesx} are
\begin{Sinput}
