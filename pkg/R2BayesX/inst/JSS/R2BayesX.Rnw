...
based on modern Markov chain Monte Carlo (MCMC) simulation techniques or with an equivalent mixed 
model representation. Although there is already a quite extensive existing toolset in \proglang{R} 
...
models for multi-categorical responses with either ordered or unordered categories, continuous time 
...
efficient (sparse) matrix architectures and are written in a \proglang{C++} environment. Besides 
exponential family regression, BayesX also supports non-standard regression situations such as 
regression for multi-categorical responses, hazard regression for continuous survival times, and 
...
\pkg{BayesX} \citep{Kneib+Heinzl+Brezger:2011}. Within the new version of the package, users may now 
\begin{itemize}
...
BayesX interpretable commands (see Section~\ref{sec:overview}). 
...
P-splines. Here, the spatially correlated effect $f_{spat}$ of locational covariate \code{district} 
is modeled by a stationary Gaussian random field based on centroid coordinates (geokriging) of the 
districts in Zambia. To estimate the model with BayesX from \proglang{R}, the data together with a 
...
\end{equation}
where $z$ represents a generic vector of all covariates, and $f_j$ are possibly smooth functions 
...
\begin{itemize}
  ...
  \item random intercepts with cluster index $c$: $f_j(z) = \beta_c$, 
  \item random slopes with cluster index $c$: $f_j(z) = x\beta_c$. 
\end{itemize}
STAR models cover a number of well known model classes as special cases, including generalized 
additive models (GAM) \citep{Hastie+Tibshirani:1990}, generalized additive mixed models (GAMM) 
\citep{Lin+Zhang:1999}, geoadditive models \citep{Kamman+Wand:2003}, varying coefficient models 
\citep{Hastie+Tibshirani:1993}, and geographically weighted regression 
...
A detailed description of the MCMC methodology is given in \citet{Brezger+Lang:2006}, inference 
based on a mixed model representation is covered by \citet{Fahrmeir+Kneib+Lang:2004}, estimation 
procedures including variable selection in \citet{Belitz+Lang:2008}. A thorough (and for most 
practical purposes sufficient) introduction into the regression models supported by the program is 
provided in the BayesX methodology manual \citep{Belitz+Brezger+Kneib+Lang:2011}. 
...
Before STAR models can be fitted with package \pkg{BayesX} from \proglang{R}, the binary command 
...
within \proglang{R} by calling the function \fct{install.bayesx}. Therefore package \pkg{BayesX} 
needs to be loaded.
...
command line binary \textbf{must} be declared to \proglang{R} at the beginning of every new session 
by setting
...
the code above to the \proglang{R} startup profile site, also see Appendix~\ref{appendix:linking}. 
...
\hypertarget{bayesx}{The} main model-fitting function in the package \pkg{BayesX} is called 
...
\citep[see][]{Chambers+Hastie:1992}. However, the object supplied to argument \code{data} is not 
necessarily an \proglang{R} data object, it is also possible to provide a character string with a 
path to a dataset stored on disc, which may be reasonable using large datasets. An example is given 
in Subsection~\ref{sec:implementation}. Note that the default specification for argument 
\code{contrasts} using factors is function \fct{contr.sum}. This allows for deviation (effect) 
coding of factor levels. As a consequence, the sampling and estimation performance of BayesX is 
improved. The distribution assigned to the response may be set with argument \code{family}. The 
...
\code{algorithm} & character, specifies the selection algorithm. Possible values are 
                   ...
                   \citet{Belitz+Brezger+Kneib+Lang:2011}), \code{"cdescent2"} (adaptive algorithms 
                   1 and 2 with backfitting, see remarks 1 and 2 of section 3 in 
                   \citet{Belitz+Lang:2008}), \code{"cdescent3"} (search according to cdescent1 
                   followed by cdescent2 using the selected model in the first step as the start 
                   model) and \code{"stepwise"} (stepwise algorithm implemented in the \code{gam} 
                   routine of S-plus, see \citet{Chambers+Hastie:1992}). & 
...
\fct{bayesx.control} of package \pkg{BayesX})}
\end{center}
\end{table}
...
plotting fitted term objects are quite flexible. It is possible to produce 2d, 3d, image, contour 
and map effects plots with or without colorlegends, amongst others. Function \fct{summary} generates 
"glm" like summary statistics for models estimated using method \code{"REML"}. For \code{"MCMC"} 
estimated models, the mean, standard deviation and quantiles of parameter samples are provided. In
some situations it may be useful to inspect the log-file generated by the BayesX binary. The file
can either be viewed directly during processing if argument \code{verbose} is set to \code{TRUE} in
function \fct{bayesx.control}, or extracted from the fitted model object using function 
\fct{bayesx\_logfile}. See Table~\ref{tab:funmethods} for a list of all available functions and 
methods.
\begin{table}[t!]
\begin{center}
...
detailed documentation is provided in the help sites of package \pkg{BayesX})}
\end{center}
\end{table}
...
The usage of \fct{s} in \pkg{BayesX} is in principle similar to package \pkg{mgcv}. The applicable 
arguments are
\begin{Sinput}
...
terms. Argument \code{bs} choses the basis/type of the term, possible options are shown in 
Table~\ref{tab:terms}. 
\begin{table}[htbp]
\begin{center}
...
In addition to \fct{s}, unit- or cluster specific unstructured effects may be incorporated into the 
model \code{formula} using the random effects term constructor function \fct{r}. The arguments of 
\fct{r} are 
\begin{Sinput}
r(id, by = NA, xt = NULL)
\end{Sinput}
...
term for the districts in Zambia with \code{r(district)}. Here, argument \code{by} takes covariates 
...
independently. This could be meaningful for two reasons: First, users may want to use already existing 
...
\code{bin} specifies the location of the BayesX binary the program file is send to, also see 
...
Another noteworthy feature of package \pkg{BayesX} is the internal handling of data. BayesX uses 
...
\proglang{R}'s memory allocation using a model fitting function. Therefore, we store the data on 
disc in the temporary folder of the running session with
...
and \citet{Fahrmeir+Kneib+Lang:2009}. The presented datasets have been added to package 
\pkg{BayesX}, ensuring straightforward traceability of the following code. In the first example, a 
Gaussian regression model is estimated using Markov chain Monte Carlo simulation. The second example 
covers restricted maximum likelihood estimation, where a cumulative multinomial distribution is 
assigned for the response variable. The last example illustrates the approach of the stepwise 
...
the stunting for child $i$ is represented by
\begin{equation*}
...
Following \citet{Kandala+Lang+Klasen+Fahrmeir:2001}, we estimate an additive regression model with 
predictor
\begin{eqnarray*}
...
in the introduction, functions $f_1$ and $f_2$ of continuous covariates \code{agechild} and 
...
respectively. With the information of the boundary file BayesX may compute an appropriate penalty, 
...
@
and the model is fitted using MCMC by calling
...
for which samples should be stored, i.e. if \code{step = 10}, only every 10th sampled parameter will
be saved.
...
\end{figure}
Note that for a map effect plot, the boundary file needs to be supplied to the plotting function, 
otherwise the effect will also be visualized using function \fct{blockplot}. 
...
@
see Figure~\ref{fig:zambia-bmi-coef-samples}.
\begin{figure}[!ht]
\setkeys{Gin}{width=0.75\textwidth}
\begin{center}
<<zambia-bmi-coef-samples-do, echo=FALSE, fig=TRUE, width=7, height=7>>=
plot(zm, term = "s(bmi)", which = "coef-samples", main = NA)
@
\end{center}
\caption{\label{fig:zambia-bmi-coef-samples} Example on childhood undernutrition: Sampling paths of 
the coefficients of term \code{s(bmi)}.}
\end{figure}
In addition, autocorrelation functions may be drawn, e.g. as shown in 
Figure~\ref{fig:zambia-autocorr} for the variance samples of term \code{s(bmi)}, by typing
...
\begin{center}
<<zambia-autocorr-02, echo=FALSE, fig=TRUE, width=6, height=5>>=
par(mar = c(4, 4, 0.01, 0.01))
plot(zm, term = "s(bmi)", which = "var-samples", 
  acf = TRUE, lag.max = 250, main = "")
@
\end{center}
...
\citet{Fahrmeir+Kneib+Lang:2009}. In this example, the percentage rate of defoliation of each tree 
is aggregated into three ordinal categories, which are modeled in terms of covariates characterizing 
the stand and site of a tree. In addition, temporal and spatial information is available, see also 
...
Similar to \citet{Fahrmeir+Kneib+Lang:2009}, we start with a three categorical ordered logit model, 
with $P(\texttt{defoliation}_{it} \leq r)$ of tree $i$ at time $t$, $r = 1,2$, and the 
additive predictor
\begin{eqnarray*}
...
\end{eqnarray*}
where functions $f_1, \dots, f_5$ of the continuous covariates are possibly nonlinear smooth 
functions and $\mathbf{x}_{it}^{\prime}\boldsymbol{\gamma}$ comprises covariates with parametric 
effects in an appropriate coding.
...
\code{k = 22}. The model is then fitted applying REML by assigning a cumulative multinomial 
distribution to the response calling
...
extremely raw estimate of \code{inclination} is hardly interpretable. Therefore, the authors extend 
the model by a spatial effect, which is modeled by a two dimensional geospline term of the tree 
locations. The tree $x$- and $y$ coordinates are calculated by the centroid positions of tree 
polygons given by the boundary map file, which may be loaded with
<<forest-model-gra-bnd, echo=TRUE, eval=TRUE>>=
data("BeechBnd", package = "R2BayesX")
@
Involving the spatial effect term we now fit the following model
...
GCV(c(fm1, fm2))
@
that clearly indicate a better fit by modeling the spatial effect of tree locations. The summary 
statistics for both models give 
<<summary-forest-model>>=
summary(c(fm1, fm2))
@
where the parametric modeled terms in the second model seem to have a less important effect on tree 
...
estimated spatial effect may be plotted either using a 3d perspective plot, an image/contour plot or
a map effect plot using the boundary file \code{BeechBnd} with
<<forest-spatial-id, echo=TRUE, eval=FALSE>>=
plot(fm2, term = "s(id)", map = BeechBnd, pos = "topleft")
@
as shown in Figure~\ref{fig:forest-spatial}. Summarizing these results identifies a strong influence 
of the spatial effect of the overall model fit, indicating that a clear splitting of locational 
specific covariates and the spatial effect is hardly possible in this example. 
\begin{figure}[!ht]
...
Furthermore, another possibility is to add the installation directory of the binary to the environment 
PATH variable of the operating system. Both options will automatically link the binary with 
\proglang{R} for the upcoming sessions.
