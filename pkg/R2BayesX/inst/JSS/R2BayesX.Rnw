...
\ref{fig:zambia-district-structured-unstructured-kde}. Here, the labels of the effects are extended
by their basis specification \code{mrf} and \code{re} merged by a colon to uniquely identify which
term should be plotted. Note that argument \code{term} could also be specified in terms of the
ordering of the model formula as an \code{integer} vector, e.g., \code{term = c(7, 8)}.
\begin{figure}[!t]
\setkeys{Gin}{width=0.46\textwidth}
\centering
...
We would like to thank Fabian Scheipl for testing the package and giving fruitful feedback. We would
also like to thank Uwe Ligges, who shared his expertise on compiling and checking the \pkg{BayesX}
source code package \pkg{BayesXsrc} for Windows.
