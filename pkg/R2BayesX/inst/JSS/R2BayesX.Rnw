...
\end{eqnarray*}
where \code{memployment1} represents the first level, `working', of covariate \code{memployment}, 
likewise for covariates \code{education}, \code{urban} and \code{gender}. As mentioned in the 
introduction, functions $f_1$ and $f_2$ of continuous covariates \code{agechild} and \code{bmi} are
assumed to have a possibly nonlinear effect on \code{stunting} and are therefore modeled with 
P-splines. Furthermore, the spatial effect is decomposed into a structured effect $f_{str}$, modeled 
by a Gaussian Markov random field, and an unstructured effect $f_{unstr}$, using a random effects 
term for the districts in Zambia.
...
zm <- bayesx(mf, method = "MCMC", iterations = 12000, 
...
terms variances and random effects variances, as well as goodness of fit criteria and other 
