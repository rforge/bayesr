...
may be constructed using function \fct{shp2bnd} or imported with \fct{read.bnd}.
...
\fixme{I feel we need a short paragraph here of maybe 4--8 lines that briefly explains the main
results of the analysis. What do the curves tell us, what information does the map hold?
Is this complemented by the summary output?}
...
software designs including GAMs. The following gives a short summary of some of the larger
projects that have added to CRAN. One of the first implementations of GAMs in \proglang{R} is the
...
GAMs based on GCV, REML and others. Vector generalized additive models (VGAM,
\citealp{R2BayesX:Yee:1996}) for categorical responses are covered by package \pkg{VGAM}
\citep{R2BayesX:Yee:2009}. Another comprehensive package for GAMs, accounting for responses that do
...
By reviewing the above mentioned packages for GAMs, we searched for existing structures that on the
...
\fct{print} & Simple printed display of the initial call and some additional information of the
              fitted model. \\
\fct{summary} & Return an object of class \code{"summary.bayesx"} containing the relevant summary
...
\fct{AIC}, \fct{BIC}, \phantom{123} \fct{DIC}, \fct{GCV} & Compute information criteria,
                                             ...
                                             \\ \hline
\end{tabular}
\caption{\label{tab:funmethods} Functions and methods for objects of class \code{"bayesx"}. More details
...
$\pm \max(|\mathit{pmean}|)$. \fixme{What is pmean?} In many situations the visual impression of the colored map is
...
fm1 <- bayesx(f, family = "cumlogit", method = "REML", data = ForestHealth)
@
...
We now fit the  model \readme{updating the model object directly rather than the formula}:
...
\fixme{Should we increase the size of the rectangles so that the color is more visible?}
...
\fixme{Check the computations of \texttt{range} in these figures, shouldn't they be in sync? Also see above for rectangle size.}
\begin{figure}[!ht]
\setkeys{Gin}{width=0.65\textwidth}
\centering
