...
more interactive and intuitive way to estimate complex regression models, has been in mind for quite 
a while. However, since the development status of the project is already very advanced, and there 
has been more and more recent interest of an \proglang{R} version of BayesX, the compromise of 
creating an interface was seen advantageous, not only because of a faster development time, but also 
offering already familiar users a toolset, that even, beside the possibility to formulate STAR 
models directly on the console, easily allows to use and reanalyze existing code and model output, 
within \proglang{R}. 

The implementation is therefore structured in several independent operating functions that basically 
set up the necessary model components at first, send these to the BayesX binary for processing, read 
the returned output files back into \proglang{R} by conversion to objects of suitable classes, which 
may then be printed, summarized and visualized.

All the functions described in this paper have been added to the CRAN package \pkg{BayesX} 
...
The unified representation of a STAR predictor arises from the fact that all of the above mentioned 
functional types may be specified by a basis function approach, where the vector of function 
evaluations $f_j = (f_j(x_{1j}),\ldots,f_j(x_{nj}))$, with $i = 1,\ldots,n$ observations, can be
written in matrix notation 
\begin{equation} \label{eqn:matnot}
f_j = X_j\beta_j,
\end{equation}
with $X_j$ as a design matrix, that only depends on the prior assumptions about smoothness of $f_j$, and $\beta_j$ are unknown regression coefficients which have to be estimated. Hence, the predictor 
...
In BayesX, estimation of regression coefficients is based on two inferential concepts, Markov chain Monte Carlo simulation techniques corresponding to full Bayesian inference \citep{Brezger+Lang:2006}
