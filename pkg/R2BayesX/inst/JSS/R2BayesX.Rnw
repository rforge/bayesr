...
capabilities for semiparametric regression in \proglang{R} by a comprehensive toolbox comprising in particular more complex response types and alternative inferential procedures such as simulation-based Bayesian inference.
}
...
\ref{fig:zambia-district-structured-unstructured-kde}. Here, argument \code{term} is specified
in terms of the ordering of the model formula as an \code{integer} vector, which is another way how
to select terms for plotting. \fixme{Achim, we need to specify with an integer vector, because BayesX
also exports the total spatial effect and we get 3 plot if we say \code{term = "sx(district)"}.}
\begin{figure}[!t]
\setkeys{Gin}{width=0.46\textwidth}
\centering
<<zambia-district-structured-kde, echo=FALSE, fig=TRUE, width=5, height=4>>=
par(mar = c(4.1, 4.1, 0.4, 1.1))
plot(zm, term = 7, map = FALSE, main = "")
@
<<zambia-district-unstructured-kde, echo=FALSE, fig=TRUE, width=5, height=4>>=
par(mar = c(4.1, 4.1, 0.4, 1.1))
plot(zm, term = 8, map = FALSE, main = "")
@
...
par(mar = c(0, 0, 0, 0))
plot(zm, term = 7, map = ZambiaBnd, swap = TRUE)
@
<<zambia-district-unstructured-samescale, echo=FALSE, fig=TRUE, width=5, height=4>>=
par(mar = c(0, 0, 0, 0))
plot(zm, term = 8, map = ZambiaBnd, swap = TRUE,
...
The \proglang{R} package \pkg{R2BayesX} provides an semi-interactive interface to the standalone
software package \pkg{BayesX}. The presented tools allow the user to specify and estimate STAR
models in a convenient \proglang{R} style that is oriented on commonly used regression model fitting
functions. Therefore, besides adapting already existing functionality from the \pkg{mgcv} package
the package also adds its own \pkg{BayesX} alike smooth term constructor function to provide users
that are more familiar with \pkg{BayesX} an intuitive start as well as accounting for \pkg{BayesX}
specific optional controlling arguments. Moreover, the software design is fully modular since the
user may import already existing \pkg{BayesX} fitted model files and also run \pkg{BayesX}
program files from \proglang{R} separately. For post estimation analysis and graphical inspection,
the package facilitates several (generic) functions that produce summary statistics, extract fitted
model term objects, generate 2d, 3d, image and map effect plots, amongst others. The major
functionality of the package is demonstrated on two datasets including examples for MCMC, REML and
stepwise estimation.
