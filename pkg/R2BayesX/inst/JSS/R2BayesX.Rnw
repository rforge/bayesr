...
@
and the model formula simply specified by
...
In BayesX, estimation of regression parameters is based on three inferential concepts: \bigskip \\
\textit{Full Bayesian inference via MCMC} \medskip \\
...
to obtain rapidly mixing, well-behaved sampling schemes without the need for manual tuning. \bigskip 
\\
\textit{Inference via a mixed model representation} \medskip \\
...
perspective. \bigskip \\
\textit{Penalized likelihood with variable selection} \medskip \\
...
The article covering MCMC methodology is \citet{Brezger+Lang:2006}, inference based on a mixed model
representation is described in \citet{Fahrmeir+Kneib+Lang:2004}, estimation procedures including
variable selection in \citet{Belitz+Lang:2008}. A thorough (and for most practical purposes 
sufficient) introduction into the regression models supported by the program is provided in the 
BayesX methodology manual \citep{Belitz+Brezger+Kneib+Lang:2011}. 
...
\citep[see][]{Chambers+Hastie:1992}.  Note that the default specification for argument 
...
default is \code{family = "gaussian"}. An overview of all available distributions for the different 
inferential concepts is given in Table~\ref{tab:familiy}.
\begin{table}[h!]
\begin{center}
...
\end{center}
\end{table}
Argument \code{methods} determines the inferential method used for estimation. Options currently 
are: Markov chain Monte Carlo simulation - \code{"MCMC"}, mixed model based estimation using 
restricted maximum likelihood/marginal likelihood - \code{"REML"} and penalized likelihood including 
model selection - \code{"STEP"}. The last argument specifies several parameters controlling the 
processing of the BayesX binary that are arranged by function \fct{bayesx.control}. 
...
\subsubsection[Implementation details]{Implementation details} \label{ssec:implementation}
...
\end{Sinput}
to run a BayesX program file from a specified directory. The model output files may be imported 
...
\end{Sinput}
Within \fct{s}, the covariates used for the model term are set with the dots \code{...} argument. 
\code{k} controls the dimension of the basis used for smooth terms. Argument \code{bs} choses the 
basis/type of the term, possible options are shown in Table~\ref{tab:terms}. Setting argument 
\code{m} is only meaningful if \code{bs = "ps"}, which controls the degree of the B-spline basis 
functions and the order of the difference penalty, e.g. \code{m} may be set to \code{m = c(3, 2)}. 
A numeric or a factor variable may be provided to argument \code{by} to estimate varying effects of
...
hyperpriors $a$ and $b$ for the variance parameter for a P-spline term, the user may type 
\code{xt = list(a = 0.0001, b = 0.0001)}. Information about all possible extra arguments for a 
particular term can be looked up using function \fct{bayesx.term.options}. \
...
r(id, by = NA, xt = NULL)
\end{Sinput}
and are used in the same fashion as described for function \fct{s}, where argument \code{id} is the 
unit or cluster identification covariate the random effects should be estimated for.
...
\code{"gs"} & geosplines based on two-dimensional P-splines with first order random walk penalty: 
              ...
              e.g. \code{xt = list(map = MapBnd)}. \\ \hline
\code{"mrf"} & Markov random fields: defines a Markov random field prior for a spatial covariate, 
               ...
               file format within the \code{xt} argument of function \fct{s}, e.g. 
