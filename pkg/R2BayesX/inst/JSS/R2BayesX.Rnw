...
(see Section~\ref{subsec:zambia}) using Markov chain Monte Carlo (MCMC) simulation. In this 
analysis, the main interest is on modeling the dependence of stunting of newborn children on 
covariates including the age of the child, the body mass index of the mother and the district the 
child lives in. The model is given by
\begin{equation*}
...
\end{equation*} 
where functions $f_1$ and $f_2$ of continuous covariates \code{agechild} and \code{bmi} have 
...
@
and the model formula is simply specified by
...
\end{equation}
where $h$ is a known link function and $\boldsymbol{\gamma}$ are unknown regression parameters. In 
...
is considered an unknown constant which is determined via restricted maximum likelihood (REML).
...
program needs to be installed and linked to \proglang{R}. The recommended option on UNIX and Windows 
systems is to auto - compile/install the BayesX command line binary within \proglang{R} by calling 
the function \fct{install.bayesx}. Therefore package \pkg{BayesX} needs to be loaded.
...
install.bayesx(inst.dir = "path/to/installation/directory", 
  source.dir = NULL)
@
where argument \code{inst.dir} is a path to a valid directory with user writing permissions. If 
\code{source.dir = NULL}, the necessary installation files will automatically tried to be downloaded 
from the \href{http://www.stat.uni-muenchen.de/~bayesx}{BayesX homepage}, otherwise 
...
installation process as described in the Appendix, see Subsection~\ref{appendix:wininstall}, or to 
compile the sources as shown in Subsection~\ref{appendix:otherinstall}, respectively.
...
the code above to the \proglang{R} startup profile site once, also see \ref{appendix:linking}. 
...
are arranged by function \fct{bayesx.control}. 
...
included in the model formula by \code{s(bmi)}, a term with two covariates is then be specified 
e.g. with \code{s(bmi, district)}. \code{k} controls the dimension of the basis used for smooth 
...
\end{center}
\end{table}
Setting argument \code{m} is only meaningful for P-splines, i.e. \code{bs = "ps"}, which controls 
...
numeric or a factor variable can be provided to argument \code{by} to estimate varying effects of 
covariates. Argument \code{xt} is used to specify special controlling parameters or additional 
geographical information for the model term. The argument must be a named list, e.g. to set the 
hyperpriors $a$ and $b$ for the variance parameter for the P-spline term for \code{bmi}, the user 
may type \code{s(bmi, bs = "ps", xt = list(a = 0.0001, b = 0.0001))}. For supplying additional 
boundary or graph files (see function \fct{read.bnd} and \fct{read.gra}), that are used to compute
suitable neighborhood penalty matrices for terms using Markov random field priors, or to calculate 
the centroids of particular regions for geosplines and geokriging terms, the name of the list 
argument is \code{map}. I.e. the necessary boundary file \code{ZambiaBnd} for the geokriging term in 
...
term for the districts in Zambia with \code{r(district)}. Here, argument \code{by} takes continuous 
covariates for which random slopes may be estimated. Argument \code{xt} is used in the same fashion 
as described for function \fct{s}, i.e. similar to the above, hyperpriors $a$ and $b$ for the 
variance parameter are set e.g. with \code{r(district, xt = list(a = 0.0001, b = 0.0001))}.
...
Function \fct{run.bayesx} is used to run a BayesX program file. The arguments of 
\fct{run.bayesx} are
\begin{Sinput}
...
Model output files are imported using function \fct{read.bayesx.output}
\begin{Sinput}
...
covariates is usually much smaller than the total number of observations. I.e., the output files 
...
\proglang{R}'s memory allocation. Therefore, we store the data on disc in the temporary folder of 
...
\pkg{BayesX}, ensuring straightforward traceability of the code presented. In the first example, a 
Gaussian additive regression model is estimated using Markov chain Monte Carlo simulation. The 
second example covers restricted maximum likelihood estimation, where a cumulative multinomial 
distribution is assigned for the response variable. The last example illustrates the approach of the 
stepwise algorithm for model and variable selection.
...
Following \citet{Kandala+Lang+Klasen+Fahrmeir:2001}, we estimate a Gaussian additive regression 
model with predictor
\begin{eqnarray*}
\eta &=& \gamma_0 + \gamma_1\texttt{memployment} + \gamma_2\texttt{education} + 
         \gamma_3\texttt{urban} + \gamma_4\texttt{gender} + \\
...
Section~\ref{sec:overview}, for any term in the model, the samples of the parameters are 
