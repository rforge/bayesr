...
BayesX. This means, some of the model formulas specified within the \pkg{mgcv} main model 
...
of the estimated effect for covariate \code{x2} of the two models is shown in 
...
which may be adopted in the same manner. 
...
into the declared output directory, which is per default the temporary directory of the running
...
listed in table~\ref{tab:funmethods}, is available. Function \fct{plot} either provides model 
diagnostic plots or plots of estimated term effects, futhermore, for MCMC models, sampling paths of
coefficients and variances may be drawn. 
\setkeys{Gin}{width=0.49\textwidth}
<<plot-illustration-model-samples-01, echo=TRUE, eval=FALSE, fig=FALSE, width=5, height=4>>=
plot(b2, term = "s(x0)", which = "var-samples")
@
\begin{figure}[t!]
...
@
\end{center}
\caption{\label{fig:illustration} Illustration example: plotting the sampling paths of the 
coefficients of term \code{s(x2)} of model \code{b2}.}
\end{figure}
\begin{table}[htbp]
\begin{center}
\begin{tabular}{|l|p{10cm}|}
