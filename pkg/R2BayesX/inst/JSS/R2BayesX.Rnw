...
issues: First, the interfacing function calls needed to be as practical as possible and oriented on
\proglang{R}'s standards for regression model fitting functions, and secondly, to provide a set of
functions and methods for an easy representation of fitted model objects, that would enhance the
...
\pkg{SemiPar} \citep{R2BayesX:Wand:2010} and its dependent version for adaptive P-splines
\pkg{AdaptFit} \citep{R2BayesX:Krivobokova}. The package \pkg{spikeSlabGAM} applies Bayesian
variable selection, model choice and regularization for GAMM's \citep{R2BayesX:Scheipl:2011}.
