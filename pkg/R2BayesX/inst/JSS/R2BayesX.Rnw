...
\citep{Kneib+Heinzl+Brezger:2011}, which previously provided only functions for exploring estimation 
results. 
...
additive predictor, given by
\begin{equation} \label{eqn:structadd}
\eta = f_1(z_1) + \ldots + f_p(z_p),
\end{equation}
where covariates $z_j$, $j = 1,\ldots,p$, are of generic type and dimension, and $f_j$ are possibly 
smooth functions comprising effects (and combination of effects) as e.g. presented in 
table~\ref{tab:funtypes}.
\begin{table}[h]
\begin{center}
\begin{tabular}{|l|l|}
\hline 
Function                      & Description \\ \hline
$f_j(z_j) = u^{\prime}\gamma$ & linear effects, with or without regularization. \\ \hline
$f_j(z_j) = f(x)$             & nonlinear effects of continuous covariate $x$. \\ \hline
$f_j(z_j) = f(x_1,x_2)$       & two-dimensional surfaces. \\ \hline
$f_j(z_j) = f_{spat}(s)$      & spatially correlated effects. \\ \hline
$f_j(z_j) = x_1f(x_2)$        & varying coefficients. \\ \hline
$f_j(z_j) = x_1f_{spat}(s)$ or $f_j(z_j) = x_1f(x_2, x_3)$ & spatially varying effects. \\ \hline
$f_j(z_j) = b_c$              & random intercepts with cluster index $c$. \\ \hline
$f_j(z_j) = x \, b_c$         & random slopes with cluster index $c$. \\ \hline
\end{tabular} 
\caption{\label{tab:funtypes} Examples of possible function types of a structured additive 
predictor.}
\end{center}
\end{table}
...
\citep{Brezger+Lang:2006} \citep{Belitz+Lang:2008} \citep{Fahrmeir+Kneib+Lang:2004}
