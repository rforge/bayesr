...
about the district neighborhood structure, which is enclosed in 
...
generic method implemented for objects of class \code{"bnd"}, which in principle calls function 
...
summary(zm)
@
provides summary statistics of the fitted-model object, which typically include estimates of linear 
effects, smooth terms variances and random effects variances, as well as goodness of fit criteria.
\begin{figure}[t!]
\setkeys{Gin}{width=0.49\textwidth}
\begin{center}
