...
additional map boundary or graph file, which must be provided within \code{xt}, then 
\begin{Sinput}
s(id, bs = "mrf", xt = list(map = SomeMap)) 
\end{Sinput} 
constructs the setup for this type of model term.

After the execution of computations with the BayesX binary, the model output files are written into 
...
and more, listed in table~\ref{tab:funmethods}, are available.
\begin{table}[htbp]
\begin{center}
\begin{tabular}{|l|p{10cm}|}
\hline
