...
One of the major problems of designing the interface was to choose a comfortable translation method
of \proglang{R} specific syntax for setting up STAR models in BayesX. Therefore, the interface is 
orientated on the recommended CRAN package \pkg{mgcv} \citep{Wood:2011} since \pkg{mgcv} already 
provides functionality which easily allows to set up new model terms within \proglang{R}'s formula 
language, and secondly, the package covers most of the implemented model terms in BayesX. This 
means 
