...
\fixme{Niki, I feel we need a short paragraph here of maybe 4--8 lines that briefly explains the main
...
\fixme{Niki, why do we use na.fail by default instead of \code{getOption("na.action")}?}
...
\end{figure}
The kernel densities reveal the general form of the random effects distributions.
...
$\pm \max(|\mathit{pmean}|)$. \fixme{Niki, what is pmean? Clarify.} In many situations the visual impression of the colored map is
