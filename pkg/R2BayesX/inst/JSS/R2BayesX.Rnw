...
\item apply a set of extractor functions and methods on BayesX fitted-model objects, e.g. producing 
  high level graphics of estimated effects and model diagnostic plots or summary statistics,
...
\texttt{stunting}_i = \gamma_0 + f_1(\texttt{agechild}_i) + f_2(\texttt{bmi}_i) + 
  f_{geo}(\texttt{district}_i) + \varepsilon_i 
\end{equation*} 
with $\varepsilon_i \sim N(0, \sigma^2)$, where functions $f_1$ and $f_2$ are possibly nonlinear and 
$f_{geo}$ represents a spatially correlated effect. Therefore, the data together with a boundary map 
file of the districts in Zambia is loaded with    
...
plot(b, term = "s(district)", image = T, grid = 100, at = c(-0.4, 0, 0.4),
  image.map = ZambiaBnd, outscale = 0.1, linear = FALSE, zlim = c(-1, 1),
  extrap = TRUE, range = c(-0.5, 0.5), lrange = c(-0.6, 0.6), swap = TRUE)
@
\setkeys{Gin}{width=0.46\textwidth}
...
program needs to be installed and linked to \proglang{R}. This may be accomplished differently and 
system specific: One possibility is to install BayesX "by hand", which is described in 
Subsections~\ref{subsec:wininstall} and \ref{subsec:otherinstall}. Another alternative is the 
installation from \proglang{R}, as explained in Subsection~\ref{subsec:Rinstall}. The last 
...
command line binary \textbf{must} be set at the beginning of every new \proglang{R} session by
...
directory of the running \proglang{R} installation. The path to this directory may be instantly 
returned by
...
for the upcoming sessions. Afterwards, if package \pkg{BayesX} is loaded, the function call
<<install-bayesx, echo=TRUE, eval=FALSE>>=
...
\subsection[Interface functions]{Interface functions} \label{subsec:interface}
...
argument \code{family}, which per default is \code{family = "gaussian"}. An overview of all 
available distributions is given in table~\ref{tab:familiy}.
\begin{table}[h!]
\begin{center}
