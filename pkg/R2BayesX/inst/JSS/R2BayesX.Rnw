...
the user may analyze models in both settings by changing the function call, only. 

However, before STAR models can be fitted with BayesX, the binary command line version of the 
