...
\readme{Achim, added some more info on geokriging here.}
...
\fixme{Niki, some more details.} \readme{Achim, more details here}
...
\readme{Achim, added more info, hope that is sort of what you had in mind?}

Objects of class \class{bayesx} returned either from function \fct{bayesx} or
\fct{read.bayesx.output} have a generic plotting method. Depending on the structure of the
...
using a divergent color legend where the \code{range} is set \code{symmetric}al, e.g.\ according to
