...
on the recommended CRAN package \pkg{mgcv} \citep{Wood:2011}, since \pkg{mgcv} already features
functionality, which easily allows to set up new model terms within \proglang{R}'s formula language, 
and secondly, the package covers a number of the model terms implemented in BayesX. This means, some 
of the model formulas specified for use within the \pkg{mgcv} model fitting-function \fct{gam}, may 
also be used in \pkg{BayesX} and vice versa. Hence, users primarily familiar with \pkg{mgcv} and its 
GAM formula definition may use the interface with little additional effort, only. This is 
illustrated in the following, where some data is simulated and models are estimated with \fct{gam}, 
using generalized cross validation (GCV) for estimation, and the \pkg{BayesX} interface main 
model-fitting function \fct{bayesx} applying MCMC: 
...
are arranged by function \fct{bayesx.control}, e.g. the choice between the three inferential 
concepts, set within argument \code{method}, which are Markov chain Monte Carlo simulation 
...
predictor with some response \code{y}, covariate \code{x1} with a possibly smooth effect modeled by 
a P-spline, covariate \code{id} representing some cluster index modeled as random effect and 
covariate \code{x2} entering linearly, is e.g. specified by
\begin{Sinput}
