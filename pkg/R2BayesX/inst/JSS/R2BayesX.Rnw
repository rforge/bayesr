...
Section~\ref{sec:overview}. In Section~\ref{sec:illustrations}, the interface usability is further
...
range <- lrange <- c(-1 * range, range)
plot(zm, term = "r(district)", map = ZambiaBnd, 
  pos = "topleft", range = range, lrange = lrange)
@ %$
