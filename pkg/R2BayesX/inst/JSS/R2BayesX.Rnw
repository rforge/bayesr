...
\fct{cprob} & extract contour probabilities of a particular P-spline term, only meaningful if
              ...
              \code{s(bmi, bs = "ps", xt = list(contourprob = 4))}. \\
...
\fct{AIC}, \fct{BIC}, \fct{DIC}, \fct{GCV} & computes information criteria, availability is
                                             dependent on the \code{method} used. \\
...
\code{"gk"}, \code{"geokriging"} & geokriging with stationary Gaussian random fields: estimates a 
              ...
              function \fct{s}, e.g. \code{xt = list(map = MapBnd)}. \\
\code{"gs"}, \code{"geospline"} & geosplines based on two-dimensional P-splines with first order 
              ...
              \code{xt} argument of function \fct{s}, e.g. \code{xt = list(map = MapBnd)}. \\
...
\code{stunting}    & standardised Z-score for stunting. \\
\code{bmi}         & body mass index of the mother. \\
...
\code{education}   & mother's educational status with categories for complete primary but incomplete
                     ...
                     no education or incomplete primary `minimum secondary'. \\
...
\code{defoliation} & percentage of tree defoliation in three ordinal
                     ...
                     defoliation < 50\%' and 2 == `defoliation $\geq$ 50\%'. \\
