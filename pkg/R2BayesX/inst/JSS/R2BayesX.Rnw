...
before a description of the necessary installation process and the implementation is provided in 
Section~\ref{sec:installation} and \ref{sec:implementation} respectively. In 
...
in Subsections~\ref{subsec:wininstall} and \ref{subsec:otherinstall}. Another alternative is the 
installation directly from \proglang{R}, as explained in Subsection~\ref{subsec:Rinstall}. The last 
Subsection~\ref{subsec:linking} describes how to essentially link the command line binary with 
\proglang{R}. All the necessary files for installation may be found at 
...
\subsection[Installing from R - Windows/UNIX]{Installing from \proglang{R} - Windows/UNIX} 
\label{subsec:Rinstall}
The recommended option on UNIX and Windows systems is to install the BayesX command line binary 
...
install.bayesx(inst.dir = "path/to/installation/directory", 
  source.dir = NULL)
@
where argument \code{inst.dir} must be a valid directory path. If \code{source.dir = NULL}, the BayesX 
source code will automatically be downloaded from the BayesX homepage, otherwise \code{source.dir} 
specifies the path where \fct{install.bayesx} may find the Windows installer 
\href{http://www.stat.uni-muenchen.de/~bayesx/install/BayesX_windows.exe}{BayesX\_windows.exe} or 
the packed sources 
\href{http://www.stat.uni-muenchen.de/~bayesx/install/bayesxsource.zip}{\code{bayesxsource.zip}}. 
Depending on the operating system, function \fct{install.bayesx} will then try to execute either the 
Windows installation process from Subsection~\ref{subsec:wininstall}, or to compile the sources as 
described in Subsection~\ref{subsec:otherinstall}.


\subsection[Linking the BayesX binary with R]{Linking the BayesX binary with \proglang{R}} 
\label{subsec:linking}
After the successful installation/compilation, the full path to the 
...
@
However, it is recommended to add the code above in the \proglang{R} startup profile site once. In 
...
<<check-install-bayesx, echo=TRUE, eval=TRUE>>=
...
The sequence of interface functions used during a typical \proglang{R} session may be illustrated 
with the following: At first, models are specified and estimated with the main model-fitting 
function, as described in the next subsection. Thereby, the model formulas used may involve a 
number of special additive terms, which are presented in Subsection~\ref{subsec:addterms}. In 
Subsection~\ref{subsec:extract}, the multiple extractor functions and methods used for post 
estimation analysis are briefly explained, before an additional example for handling large datasets
is given in Subsection~\ref{subsec:largedata}. Note that for all functions presented in this paper, 
extensive examples of the usage are also provided in the function documentation of package 
\pkg{BayesX}. 
...
\end{center}
\end{table}
The object returned with function \fct{bayesx} is of class \code{"bayesx"}, which is supported by
several functions and methods, such as \fct{plot} or \fct{summary}, shown in 
...
the interfacing functions, that are basically called within \fct{bayesx}, may also be used 
...
might be a need for importing stored BayesX output files into \proglang{R} for further analysis. 
Therefore, users may call function \fct{run.bayesx}, with arguments
\begin{Sinput}
...
The usage of function \fct{s} within \pkg{BayesX} is in principle similar to package \pkg{mgcv}. 
