...
\begin{equation*}
\texttt{stunting}_i = \frac{\texttt{AI}_i - \texttt{MAI}}{\sigma},
\end{equation*}
...
f <- stunting ~ memployment + education + urban + gender +
  sx(bmi) + sx(agechild) + sx(district, bs = "mrf", map = ZambiaBnd) + 
  r(district)
@
...
plot(zm, term = c("sx(bmi)", "sx(agechild)"))
@
\begin{figure}[!ht]
\setkeys{Gin}{width=0.46\textwidth}
\centering
...
for term \code{sx(bmi)} are displayed with
<<zambia-bmi-coef-samples, echo=TRUE, eval=FALSE, fig=FALSE>>=
plot(zm, term = "sx(bmi)", which = "coef-samples")
@
...
par(mar = c(4.1, 4.1, 0.1, 1.1))
plot(zm, term = "sx(bmi)", which = "var-samples",
  acf = TRUE, main = "")
@
...
of the samples of the variance parameter of term \code{sx(bmi)}, left panel, maximum autocorrelation
...
<<forest-spatial-id, echo=TRUE, eval=FALSE>>=
plot(fm2, term = "sx(id)", map = BeechBnd, pos = "topleft")
@
...
f <- stunting ~ memployment + urban + gender +
  sx(education, bs = "factor") + sx(bmi) + sx(agechild) +
