...
regression models. To gain improved computational performance of the supported models, the 
algorithms implemented utilize efficient (sparse) matrix architectures and are written in a 
...
with little additional effort, only. This is illustrated in the following, where some gaussian data 
is simulated and models are estimated with \fct{gam}, using generalized cross validation (GCV) for 
...
consistent way of smooth terms specifications in GAM and STAR formulas. Furthermore, BayesX supports 
...
\begin{center}
<<forest-spatial-id, echo=FALSE, fig=TRUE, width=7, height=5>>=
