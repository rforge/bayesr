...
is modeled by a two-dimensional P-spline based on centroid coordinates (geospline) of the districts 
in Zambia. To estimate the model with BayesX from \proglang{R}, the data together with a boundary 
map file of the districts in Zambia is loaded with    
...
@
In Figure~\ref{fig:illustration}, some visualization examples of the estimated effects produced by 
the versatile implemented plotting functions are shown.
\begin{figure}[t!]
...
implementation is given in Section~\ref{sec:overview}. In Section~\ref{sec:illustrations}, the 
interface usability is further illustrated with the childhood malnutrition data of Zambia and a 
...
\item \textit{Penalized likelihood with variable selection} \\
...
downloaded from the \href{http://www.stat.uni-muenchen.de/~bayesx}{BayesX homepage}, otherwise 
...
\citep[see][]{Chambers+Hastie:1992}. However, the object supplied to argument data is not 
...
functions and methods, such as \fct{plot} and \fct{summary}, listed in Table~\ref{tab:funmethods}. 
\begin{table}[t!]
\begin{center}
...
However, only a reduced list of arguments and options is available using the interface. The 
...
included in the model formula by \code{s(bmi)}, a term with two covariates may then be specified 
...
numeric or a factor variable may be provided to argument \code{by} to estimate varying effects of 
...
argument is \code{map}. I.e. the necessary boundary file \code{ZambiaBnd} for the geospline term in 
the introduction is provided with \code{s(district, bs = "gs", xt = list(map = ZambiaBnd))}. 
Information about all possible extra arguments for a particular term can be looked up using function 
\fct{bayesx.term.options}. 
...
estimated for. E.g. the model formula from the introduction may be extended by a random effects term 
for the districts in Zambia with \code{r(district)}. Here, argument \code{by} takes continuous 
...
the interfacing functions that are consecutively called within \fct{bayesx} may also be used 
independently. This may be meaningful for two reasons: First, users may want to use already existing 
...
To run a BayesX program file users may call function \fct{run.bayesx}. The arguments of 
\fct{run.bayesx} are
\begin{Sinput}
...
informations may be printed to the \proglang{R} console if \code{verbose = TRUE}. Argument 
...
Model output files may be imported using function \fct{read.bayesx.output}
\begin{Sinput}
...
numerically efficient algorithms including sparse matrix computations which allow for estimating
models using large datasets. There may be situations where it is difficult to load particular 
datasets into \proglang{R}, because of memory problems etc., 

To exploit this within \proglang{R} using a
...
\section[Manual installation of BayesX]{Manual installation of BayesX} \label{appendix:minstall}
...
options(bayesx.bin = "path/to/BayesX")
\end{Sinput}
to \code{Rprofile.site} will 
tell \proglang{R} where to find the binary on startup. On Windows systems \code{"path/to/BayesX"}, 
may be specified e.g. with \code{"C:/BayesX/commandline/bayesx.exe"}, depending on the location of
the installation of the binary.  
 
However, another possibility is to add the installation directory of the binary to the environment 
