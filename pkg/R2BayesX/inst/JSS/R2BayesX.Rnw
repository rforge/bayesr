...
f <- fitted(zm, term = "s(district)")$Mean
plot(density(f, n = length(f), from = min(f), to = max(f)), main = "")
abline(v = R2BayesX:::get.kde.range(f), lty = 2)
@ 
<<zambia-district-unstructured-kde, echo=FALSE, fig=TRUE, width=5, height=4>>=
par(mar = c(4.1, 4.1, 0.4, 1.1))
f <- fitted(zm, term = "r(district)")$Mean
plot(density(f, n = length(f), from = min(f), to = max(f)), main = "")
abline(v = R2BayesX:::get.kde.range(f), lty = 2)
@ 
\\[2ex] 
...
par(mar = c(0, 0, 0, 0))
f <- fitted(zm, term = "s(district)")$Mean
frange <- c(-1 * max(abs(f)), max(abs(f)))
plot(zm, term = "r(district)", map = ZambiaBnd, 
  pos = "topleft", width = 0.6, height = 0.2, 
  distance.labels = 2L, swap = TRUE, 
  density = 20, angle = 90, lrange = frange, 
  range = R2BayesX:::get.kde.range(f))
@ %$
...
panel. Kernel density estimates of the mean of the structured effect are shown in the middle left 
panel, of the unstructured spatial effect in the middle right panel respectively. The 5\% and 95\% 
quantiles of the kernel density estimate are represented by the dashed lines. Lower panel, 
unstructured spatial effect using the legend scaling of the structured effect.}
\end{figure}
...
@
\\[2ex] \setkeys{Gin}{width=0.46\textwidth}
<<forest-spatial-id-kde, echo=FALSE, fig=TRUE, width=5, height=4>>=
par(mar = c(4.1, 4.1, 0.4, 1.1))
f <- fitted(fm2, term = "s(id)")$Estimate
plot(density(f, n = length(f), from = min(f), to = max(f)), main = "")
abline(v = R2BayesX:::get.kde.range(f), lty = 2)
@ %$
\caption{\label{fig:forest-spatial} Forest damage: Estimate of the spatial effect together with the
kernel density estimate of the effect. The 5\% and 95\% quantiles of the kernel density estimate are 
