...
\fixme{I have rewritten the section up to this point but did not modify the subsequent
text. My feeling is that a table explaining how arguments are mapped would still be useful
...
Note that the defaults of \fct{s}, as provided by \pkg{mgcv}, are different from the default values
...
<<appendix-s, echo=TRUE, eval=TRUE>>=
...
To generate a \pkg{BayesX} term with the same base setting as provided from function \fct{sx}, the
user would need to specify argument \code{k} additionally with
<<appendix-s, echo=TRUE, eval=TRUE>>=
...
penalty is set with \code{s(mbmi, bs = "ps", m = c(2, 2))} (note that argument \code{m} is slightly different than argument \code{degree} in function \fct{sx} using P-splines, since \code{m[1]}
specifies the order of the spline, see also the manual of \fct{s}). Argument \code{by} is used in
the same way as for \fct{sx}. The additional parameters that are specified by argument
...
\code{ZambiaBnd} is supplied with \code{s(district, bs = "mrf", xt = list(map = ZambiaBnd))}.

\begin{table}[b!]
\centering
\begin{tabular}{lll|lp{1.5cm}}
\hline
...
\code{"te"}, \fct{te} & \code{nrknots} & \code{7}  & \code{k = nrknots + degree - 1}                & \code{5} \\
...
\end{tabular}
\caption{\label{tab:sx2s} Argument mapping and default specifications of
  functions \fct{sx} and \fct{s} / \fct{te}.}
\end{table}
