...
\begin{itemize}
\item specify and estimate STAR models with \pkg{BayesX} directly from the console (function
  \fct{bayesx}),
...
\begin{itemize}
\item run already existing \pkg{BayesX} input program files from \proglang{R} (function
  \fct{run.bayesx}),
\item automatically import \pkg{BayesX} output files into \proglang{R} (function
...
\proglang{R} syntax into \pkg{BayesX} interpretable commands (see Section~\ref{sec:overview}).

The reminder of this paper is as follows. The next section gives a first example of a common 
\proglang{R} session using \pkg{R2BayesX}. Section~\ref{sec:model} briefly discusses the
methodological background of regression models with a structured additive predictor, before a
description of the package functionality is given in Section~\ref{sec:overview}. In
Section~\ref{sec:illustrations}, the interface usability is further illustrated with the childhood
malnutrition data of Zambia and a dataset on forest health.


\section{Motivating example}

To give an introductory example of the various features of the interface, we estimate a Bayesian
geoadditive regression model for the childhood malnutrition data set in Zambia (see
...
child (\code{age}), the body mass index of the mother (\code{bmi}) and the district the child lives
...
plot(b, term = "sx(district)", map = ZambiaBnd,
  ...
  distance.labels = 2, swap = TRUE, density = 20,
  angle = 90)
@
...
a map effect plot.}
\end{figure}
A plot of the estimated effect for covariate \code{bmi} may then be produced by typing
<<illustration-plot-bmi, echo=TRUE, fig=FALSE, eval=FALSE>>=
plot(b, term = "sx(bmi)")
@
...
plot(b, term = "sx(district)", map = ZambiaBnd, pos = "topleft")
@
