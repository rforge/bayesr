...
For model fitting with package \pkg{R2BayesX}, the binary command-line version of \pkg{BayesX} needs
to be installed and linked to \proglang{R} first.
...
\textbf{Note:} on Windows, function \fct{install.bayesx} will download and execute an installer,
which also installs the GUI version of \pkg{BayesX}.
\end{frame}
